...
- [Безопасное открытие файлов через PWA](docs/ru/PWA_FILE_HANDLING.md)
...
- Установленное Chromium-family desktop PWA открывает `.pine`, `.strategy` и `.saltanat-plugin` только через обязательную локальную проверку; ручной импорт остаётся доступен во всех браузерах.
