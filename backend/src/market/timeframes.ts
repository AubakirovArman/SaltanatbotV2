import type { Timeframe } from "../types.js";

export const timeframes: Timeframe[] = ["1m", "5m", "15m", "30m", "1h", "2h", "4h", "1d", "1w", "1M"];

export const timeframeMs: Record<Timeframe, number> = {
  "1m": 60_000,
  "5m": 300_000,
  "15m": 900_000,
  "30m": 1_800_000,
  "1h": 3_600_000,
  "2h": 7_200_000,
  "4h": 14_400_000,
  "1d": 86_400_000,
  "1w": 604_800_000,
  // 1M is calendar-approximate (30 days). Real months vary 28-31 days; this
  // value is only used for bucket alignment/synthetic bars, not exchange klines.
  "1M": 2_592_000_000
};

export const binanceIntervals: Partial<Record<Timeframe, string>> = {
  "1m": "1m",
  "5m": "5m",
  "15m": "15m",
  "30m": "30m",
  "1h": "1h",
  "2h": "2h",
  "4h": "4h",
  "1d": "1d",
  "1w": "1w",
  "1M": "1M"
};

/** Bybit v5 kline intervals: minutes as numbers, day/week/month as letters. */
export const bybitIntervals: Partial<Record<Timeframe, string>> = {
  "1m": "1",
  "5m": "5",
  "15m": "15",
  "30m": "30",
  "1h": "60",
  "2h": "120",
  "4h": "240",
  "1d": "D",
  "1w": "W",
  "1M": "M"
};

export function alignTime(ts: number, timeframe: Timeframe) {
  return Math.floor(ts / timeframeMs[timeframe]) * timeframeMs[timeframe];
}
