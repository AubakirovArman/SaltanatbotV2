...
- Если SL/TP отклонён после принятия entry, принятый entry не становится `rejected`: сохраняются
  managed state и резерв, automation ставится на паузу и не получает `open_protected`. Отдельный
  best-effort reduce-only emergency close имеет уникальный client/order identity, а его принятие или
  ошибка показываются явно до authenticated execution accounting.
