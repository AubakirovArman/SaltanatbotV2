...
Accepted production now runs PostgreSQL schema 13 from slot `r5a-schema13-66394fd`. The R5.1/schema
13 alert startup behavior described below is accepted and deployed; its acceptance evidence is
recorded in [R5.1 owner alerts](./evidence/R5_1_OWNER_ALERTS.md). See
...
During a schema-13 cutover, start the API first while the research worker remains stopped. Only
...
aggregate heartbeat/metrics and correct the exact alert service. A restored owner event stream
