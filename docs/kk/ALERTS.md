...
Қабылданған R5.3a релизінде жеткізу тек `in-app`: `telegram` каналын сұраған
screener ережесі басқа кез келген қолдау көрсетілмейтін жеткізу каналы сияқты
айқын `400` қатесімен қабылданбайды. Төменде сипатталған R5.3b-1 инкременті
(жұмыс үстінде) осы тексеруді кеңейтеді: `telegram` каналы price-threshold
және screener ережелерінің екеуіне де қабылданады.
...
## Telegram арқылы жеткізу және чат байланыстыру (R5.3b-1, жұмыс үстінде)
...
notification worker қосады. Бұл инкремент **жұмыс үстінде және қабылданған
релиз емес**: production әлі қабылданған R5.3a слотын орындайды, онда
`telegram` каналы `400` қайтарады. Барлық алерт мүмкіндіктері сияқты бұл —
...
binding/revoke/delivery flow — жоғарыда сипатталған R5.3b-1 инкременті
(жұмыс үстінде); кеңейтілген кіріс бот командалары R5.3b-2-де қалады.
