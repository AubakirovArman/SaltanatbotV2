...
schema 15 нұсқасына cutover жасады, ал owner-дың белсенді байланыстыруы
...
статикалық жауаптан басқа бот командалары — төмендегі
[Telegram paper командалары](#telegram-paper-командалары-r53b-2)
бөлімінде сипатталған, қабылданған R5.3b-2 релизі.
...
жазылады, ал басқа кез келген приват хабар белгісіз команда туралы `/help`
нұсқайтын статикалық жауап алады. Сақталатын ingress жолдары тек нормаланған —
...
## Telegram paper командалары (R5.3b-2)

R5.3b-2 байланыстырылған приват чатты оқу командаларымен және paper
роботтарды екі қадамдық fenced басқарумен кеңейтеді. Инкремент **қабылданды
және деплой жасалды**: production қорғалған `r5e-schema16-17e12f1`
slot-ынан PostgreSQL schema 16 нұсқасында жұмыс істейді (16-миграция
`telegram_command_bridge` `telegram_command_replies` және
`telegram_confirmations` кестелерін қосады), командалар бот токені файлы
орнатылған соң жеткізумен бірге белсендіріледі, ал қабылдау мен cutover
жазбасы — [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md).
Worker бұрынғыдай HTTP listener ашпайды
және сауда SQLite-ін ешқашан ашпайды: әрбір paper жауап бар API-процестегі
fenced орындаушы қолданатын бір durable executor командасынан туады.
Барлығы paper-only зерттеу болып қалады — ешбір команда тірі саудаға жете
алмайды, — ал administrator-лар төмендегі лимиттердің ешқайсысын айналып
өтпейді.

Әрбір команда тек белсенді байланыстыруы бар приват чатта жұмыс істейді.
Worker жалғыз белсенді байланыстыруды хэштелген чат өткізбесі бойынша
резолвтайды (екі түрлі owner белсенді байланыстырған чат екіұшты ретінде
қабылданбайды), `users.status = 'active'` күйін қайта тексереді және
owner-дың ағымдағы authorization revision мәнін оқиды; кез келген сәтсіздік
tenant деректерін ашпайтын статикалық fail-closed жауап алады. Бір чаттан
командалардың бар лимиті (6/мин) қолданылады.

| Команда | Жауап |
| --- | --- |
| `/help` | Статикалық команда анықтамасы |
| `/balance` | Default портфель снапшоты: available/reserved капитал, evidence-aware equity (адал `unavailable`, нөл емес), realized PnL және 8 таңбалы handle-мен ең көбі 20 робот |
| `/daily` | Ағымдағы UTC тәулігінің realized PnL |
| `/profit` | Жалпы realized PnL |
| `/performance` | Робот бойынша realized PnL және шектелген win/loss санақтары |
| `/trades <robot>` | 8 таңбалы handle бойынша бір роботтың соңғы ≤10 fill-і |
| `/alerts` | PostgreSQL-ден тікелей оқу: ≤10 қосулы ереже және ≤5 соңғы оқиға |
| `/pause`, `/resume`, `/stop <robot>` | Екі қадамдық fenced басқару (төменде) |
| `/confirm <token>` | Бір реттік растау токенін тұтыну |

### Екі durable фаза

A фазасы update-ті журналдайтын сол ingress батч транзакциясында
орындалады: paper команда `telegram:<botFingerprint>:<updateId>`
idempotency key-і бар дәл бір durable executor командасын және
`telegram_command_replies` кестесіне pending жолды кезекке қояды.
`(owner, idempotency key)` бірегейлігі қайталанған немесе crash-тан кейін
қайта жеткізілген update-тің рестартқа/takeover-ге дейін де, кейін де сол
бір durable командаға келуін қамтамасыз етеді. Мұздатылған
`executor_commands` кестесі DDL алмайды: Telegram шығу тегі — payload
ішіндегі `origin` маркері мен idempotency key префиксі; telegram-тектес
командалар authorization epoch 0-мен детерминистік синтетикалық session
digest алып жүреді, ал орындаушы apply кезінде durable authorization
revision-ды қайта дәлелдейді. Ingress cursor орындаушыны ешқашан күтпейді.

B фазасы — delivery lane қасындағы шектелген replies lane
(`NOTIFICATION_REPLIES_POLL_INTERVAL_MS`). Ол pending жауап жолдарын
executor командаларымен қосады; терминалды (`applied`/`rejected`) команда
форматталады (rejected тек қауіпсіз қате кодтарына түседі), ал 10 минуттан
асқан терминалды емес команда бір «timed out» жауабын алады. Байланыстыру
(белсенді, сол revision, чат орнында) мен белсенді owner **әрбір** жіберу
алдында settle транзакциясының ішінде қайта дәлелденеді; кері қайтарылған
байланыстыру жауапты толық басады. `replied_at` сыртқы жіберуден БҰРЫН
тұрақты бекітіледі, сондықтан әрбір жауап — **at-most-once**: fence пен
жіберу арасындағы crash сол бір хабарды қайталамай жоғалтады; күтудегі
басқару командасы үшін жіберілмеген токен жай мерзімі өтіп кетеді де,
команданы қайта беруге болады. Жауаптар delivery lane-нің
global/per-chat/per-owner жіберу бюджеттерін бөліседі.

Екі оқу kind-і — `paper-portfolio.snapshot` пен `paper-robot.trades` —
қатаң read-only және басқа executor командалары сияқты сол lease/fence
жолымен өтеді; `/balance`, `/daily`, `/profit` және `/performance` бір
снапшот нәтижесінен форматталады.

### Екі қадамдық басқару және растау токендері

1. `/pause <robot>` (немесе `/resume`, `/stop`) A фазасында confirm-target
   snapshot командасын кезекке қояды.
2. Оның терминалды нәтижесінде replies lane 8 таңбалы handle-ді owner
   роботтары бойынша резолвтайды — сәйкестік болмаса қолжетімді
   handle-дермен жауап береді, екі және одан көп сәйкестік екіұшты ретінде
   қабылданбайды — және бір реттік растау шығарады: тек SHA-256 хэші
   сақталатын 16 таңбалы base32 токен, қасында бекітілген байланыстыру
   кортежі, чат өткізбесі, authorization revision және шығару сәтінде
   байқалған portfolio/ledger/robot revision-дар. Шикі токен дәл бір рет —
   жауап ішінде — өмір сүреді; ол 120 секундтан кейін істен шығады және бір
   owner-да ең көбі 3 тұтынылмаған растау болады.
3. `/confirm <token>` токенді хэштеп, тұтынылмаған, мерзімі өтпеген жолды A
   фазасының ішінде құлыптайды (`FOR UPDATE`), содан кейін чат өткізбесін,
   owner-ды, байланыстырудың дәл id/revision мәндерін және сол authorization
   revision-ды қайта дәлелдейді. Кез келген сәйкессіздік — токенді
   **тұтынбайтын** бір біркелкі бас тарту. Тұтыну мен durable
   `paper-robot.action` командасы бір транзакцияда коммиттеледі; орындаушы
   әрекетті бекітілген оптимистік fence-тер астында қолданады. Жарамсыз
   токендер байланыстыру кодтарымен бірдей чат әрекет бюджетін жағады.
4. Replies lane терминалды әрекет командасына applied нәтижесімен немесе
   қауіпсіз тұжырымдалған бас тартумен жауап береді.

### Команда лимиттері және retention

| Шекара | Лимит |
| --- | ---: |
| Растау токенінің TTL | 120 с |
| Owner-дың тұтынылмаған растаулары | 3 |
| Бір чаттан растау/код әрекеттері | 5 / 10 мин |
| Бір чаттан өңделген командалар | 6 / мин |
| Снапшот жауабындағы роботтар | 20 |
| `/trades` жауабындағы fill-дер | 10 |
| `/alerts` жауабындағы ережелер / оқиғалар | 10 / 5 |
| Терминалды емес команданың жауап timeout-ы | 10 мин |

Retention тұтынылған немесе мерзімі өткен растауларды 2 күннен кейін және
жауап берілген жауап жолдарын 7 күннен кейін басқа алерт тарихы сияқты
шектелген `SKIP LOCKED` стадияларымен өшіреді; жауап жолы өз executor
командасына `ON DELETE CASCADE` арқылы ереді, сондықтан executor
командаларының retention-ы оларды жетім қалдырмайды.
...
релизі; кеңейтілген кіріс бот командалары — жоғарыдағы
[Telegram paper командалары](#telegram-paper-командалары-r53b-2)
бөлімінде сипатталған, қабылданған R5.3b-2 релизі.
