...
- Сенсорлық экранда бір саусақ history-ді жылжытады, ал екі саусақ жанасу ортасының маңында chart-ты бір уақытта zoom және pan жасайды. Бір саусақты көтергеннен кейін қалған саусақ pan-ды бірден жалғастырады; page scroll/zoom тек Canvas үстінде тоқтатылады. 40% төменгі шегіне жеткеннен кейін саусақтарды одан әрі жақындату қауіпсіз no-op болады және интерфейсті қайта іске қоспайды. Coarse pointer құрылғысында қысқа gesture hint көрсетіліп, scale/reset батырмалары 48 px-ге үлкейеді.
- Тар экранда chart overlay panel-дерсіз ашылады. Markets және instrument батырмалары өзара exclusive төменгі modal sheet ашады: symbol таңдалғанда list жабылады, `Escape`, backdrop және close батырмасы focus-ты trigger-ге қайтарады. Sheet биіктігі browser chrome мен device safe area-ны ескереді.
- Телефондағы толық drawing жиыны **Drawing tools** батырмасымен ашылады. Төменгі sheet іздеу мен топтарды қолдайды; chart жанында active tool, undo, redo, selected object delete және object tree қалады. Cursor режиміндегі long press inspect/crosshair ашады, ал екінші touch басталса gesture жалған drawing anchor қоспай pinch режиміне өтеді.
...
## Мәтіндік жазба және параллель канал

**Мәтіндік жазба** құралы бір click арқылы нақты уақыт пен бағаға байланады, сондықтан жазба zoom және pan кезінде деректермен бірге жылжиды. Орнатқан соң шағын editor ашылады: жаңа жолдары бар 500 таңбаға дейінгі мәтін, `Ctrl+Enter` сақтайды, `Esc` бас тартады. Қайта редакциялау double click, телефонда long press немесе object tree арқылы ашылады; object tree-де жазба өз мәтінінің басымен белгіленеді. Жазбамен бірге ақпараттық метадеректер сақталады — жасау сәтіндегі автор логині мен жасалу уақыты. Бұл workspace client құжатының анықтамалық өрістері, сервер тексеретін қолтаңба емес; логинсіз жергілікті сессияда автор жазылмайды.

**Параллель канал** құралы үш click арқылы салынады: алғашқы екеуі базалық сызықты, үшіншісі параллель сызықтың баға ығысуы ретінде канал енін береді. Сызықтар арасында translucent толтыру салынады, ортасында `Δ` ен белгісі баға бірлігімен көрсетіледі. Body drag екі сызықты біртұтас жылжытады; базалық сызық ұшын сүйреу енін сақтап көлбеуін өзгертеді; жеке width anchor тек енін өзгертеді. Екі құрал да телефонда ортақ төменгі **Drawing tools** тізімінен қолжетімді, басқа drawing-термен бірге workspace-те сақталады және `.saltanat-workspace.json` экспортына кіреді.
...
- **Four different markets** action бірден 2×2 grid жасайды: primary symbol сақталып, қалған pane-дер available major pair-лармен BTC/ETH/SOL/BNB priority бойынша толтырылады. Тек symbol link ажырайды; timeframe, indicator, compare, drawing және басқа link setting сақталады. Нәтиже reload-тан кейін автоматты қалпына келеді.
- 2×2 торында `2`, `3`, `4` pane-дері chart үстінде жеке symbol, timeframe және chart type select көрсетеді. Басқа pair, interval не chart type таңдау тек сол field-ті автоматты unlink жасайды және primary chart өзгермейді. Chart type жанындағы chain primary representation мен кейінгі synchronization-ды қайтарады.
...
- Жоғарғы панельдегі symbol, timeframe және chart type, command palette және сандық timeframe shortcut-тары active pane-ге қолданылады. Secondary chart symbol, timeframe немесе chart type қолмен өзгертілсе, primary chart өзгермей, тек сол field link-тен ажырайды; chain батырмасы synchronization-ды қайта қосады.
...
- Layout menu толық ARIA keyboard contract қолданады: `ArrowUp`/`ArrowDown`, `Home`/`End` focus ауыстырады, `Enter` action орындайды, `Escape` menu-ді жауып trigger-ге focus қайтарады.
- Chain батырмалары symbol, timeframe, chart type, crosshair және visible time range-ті primary chart-пен бөлек байланыстырады. Linked zoom/pan absolute UTC boundary жібереді, сондықтан әртүрлі timeframe candle index бойынша drift жасамайды; loaded history-ден тыс range chart-ты секіртпейді.
...
## Орнату және offline режимі

Production нұсқасын HTTPS немесе localhost арқылы қолдайтын browser-ден app ретінде орнатуға
болады. Желі жоқ кезде тек сақталған static интерфейс ашылады. API, authorization, quote, order
book, trade flow және сауда command-тары әрқашан network талап етеді, service worker cache-іне
жазылмайды және кейін автоматты жіберілмейді. Offline интерфейстің ашылуы price өзекті немесе
execution қолжетімді дегенді білдірмейді.
