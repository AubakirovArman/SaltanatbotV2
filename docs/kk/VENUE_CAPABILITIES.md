...
| --- | --- |
| Графиктер | Binance/Bybit public candle және Hyperliquid first-DEX perpetual last-trade candle; market type/price source шектеулері анық көрсетіледі |
...
| Hyperliquid | Public HyperCore `/info`, L2/funding, first-DEX perpetual candle history және public candle/L2/trade streams тіркелген | Әр `l2Book` protocol sequence/checksum жоқ atomic block snapshot. Тек continuous research, route-ready books ішіне кірмейді | Ортақ continuous research, ordinary first-DEX perpetual chart selector және optional paper-robot market-data source | Wallet, address, signing, `/exchange`, HyperEVM және order коды жоқ |
