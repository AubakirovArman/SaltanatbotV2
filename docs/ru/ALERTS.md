...
изменению». Инкремент завершён в этом дереве, но **ещё не принят**; статус
приёмки ведётся в [IMPLEMENTATION_STATUS.md](../IMPLEMENTATION_STATUS.md).
...
Доставка — только `in-app` до R5.3b. Screener-правило с каналом `telegram`
отклоняется явным `400`, как и любой другой неподдерживаемый канал доставки.
...
kind `screener`. R5.3 добавит отдельный notification worker и Telegram
binding/revoke/delivery.
