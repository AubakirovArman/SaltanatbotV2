...
В релизе R5.3a доставка была только `in-app`: screener-правило с каналом
`telegram` отклонялось явным `400`, как и любой другой неподдерживаемый
канал. Принятый релиз R5.3b-1 (см. ниже) расширяет эту проверку: канал
`telegram` теперь принимается и для price-threshold, и для screener-правил.
...
## Доставка в Telegram и привязка чата (R5.3b-1)
...
владельцу через одноразовые коды. Инкремент **принят и задеплоен**:
production работает на PostgreSQL schema 15 из защищённого slot
`r5d-schema15-cd34ec8`, и уведомление доставляется в `telegram`, когда у
владельца есть активная привязка; запись приёмки и cutover —
[R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md).
Как и вся система алертов, это исключительно
...
binding/revoke/delivery — это описанный выше принятый релиз R5.3b-1;
