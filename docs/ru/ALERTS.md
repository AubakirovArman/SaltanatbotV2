...
production выполнил cutover на PostgreSQL schema 15 из защищённого slot
...
`/start`, `/bind` и статичного ответа-заглушки, — это принятый релиз
R5.3b-2, описанный в разделе
[Telegram paper-команды](#telegram-paper-команды-r53b-2).
...
статичный ответ о неизвестной команде с подсказкой `/help`. Хранимые
ingress-строки только нормализованы — хэшированный отпечаток чата, kind и
outcome — никогда текст сообщения или сырой chat id.
...
## Telegram paper-команды (R5.3b-2)

R5.3b-2 расширяет привязанный приватный чат командами чтения и двухшаговым
fenced-управлением paper-роботами. Инкремент **принят и задеплоен**:
production работает на PostgreSQL schema 16 из защищённого slot
`r5e-schema16-17e12f1` (миграция 16 `telegram_command_bridge` добавляет
таблицы `telegram_command_replies` и `telegram_confirmations`), команды
активируются вместе с доставкой после установки файла токена бота, а
запись приёмки и cutover —
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md).
Worker по-прежнему не открывает HTTP-listener и никогда не
открывает торговый SQLite: каждый paper-ответ порождается одной durable
executor-командой, которую применяет существующий fenced-исполнитель
процесса API. Всё остаётся paper-only research — ни одна команда не
достигает живой торговли, — а администраторы не обходят ни один из лимитов
ниже.

Каждая команда работает только в приватном чате с активной привязкой.
Worker резолвит единственную активную привязку по хэшированному отпечатку
чата (чат, активно привязанный двумя разными владельцами, отклоняется как
неоднозначный), повторно проверяет `users.status = 'active'` и читает
текущую authorization revision владельца; любой сбой получает статичный
fail-closed ответ без tenant-данных. Действует существующий лимит команд
на чат (6/мин).

| Команда | Ответ |
| --- | --- |
| `/help` | Статичный справочник команд |
| `/balance` | Снапшот default-портфеля: available/reserved капитал, evidence-aware equity (честное `unavailable`, не ноль), realized PnL и до 20 роботов с 8-символьными handle |
| `/daily` | Realized PnL за текущие сутки UTC |
| `/profit` | Суммарный realized PnL |
| `/performance` | Realized PnL по роботам с ограниченными счётчиками win/loss |
| `/trades <robot>` | Последние ≤10 fills одного робота по 8-символьному handle |
| `/alerts` | Прямое чтение PostgreSQL: ≤10 включённых правил и ≤5 недавних событий |
| `/pause`, `/resume`, `/stop <robot>` | Двухшаговое fenced-управление (ниже) |
| `/confirm <token>` | Потребление одноразового токена подтверждения |

### Две durable фазы

Фаза A выполняется в той же транзакции батча ingress, которая журналирует
update: paper-команда ставит в очередь ровно одну durable executor-команду
с idempotency key `telegram:<botFingerprint>:<updateId>` и pending-строку в
`telegram_command_replies`. Уникальность `(owner, idempotency key)`
гарантирует, что повторённый или доставленный после crash update приходит к
той же единственной durable-команде до и после рестарта или takeover.
Замороженная таблица `executor_commands` не получает DDL: происхождение
Telegram — это маркер `origin` в payload и префикс idempotency key; команды
telegram-происхождения несут детерминированный синтетический session digest
с authorization epoch 0, а исполнитель при apply повторно доказывает
durable authorization revision. Cursor ingress никогда не ждёт исполнителя.

Фаза B — ограниченная replies lane рядом с delivery lane
(`NOTIFICATION_REPLIES_POLL_INTERVAL_MS`). Она соединяет pending-строки
ответов с executor-командами; терминальная команда (`applied`/`rejected`)
форматируется (rejected отображается только в безопасные коды ошибок), а
нетерминальная команда старше 10 минут получает один ответ «timed out».
Привязка (активна, та же revision, чат на месте) и активный владелец
повторно доказываются внутри settle-транзакции перед **каждой** отправкой;
отозванная привязка полностью подавляет ответ. `replied_at` фиксируется
долговечно до внешней отправки, поэтому каждый ответ — **at-most-once**:
crash между fence и отправкой теряет это одно сообщение и никогда его не
дублирует; для ожидающей команды управления неотправленный токен просто
истекает, и команду можно повторить. Ответы делят
global/per-chat/per-owner бюджеты отправки delivery lane.

Два kind чтения — `paper-portfolio.snapshot` и `paper-robot.trades` —
строго read-only и проходят тот же lease/fence-путь, что и остальные
executor-команды; `/balance`, `/daily`, `/profit` и `/performance`
форматируются из одного результата снапшота.

### Двухшаговое управление и токены подтверждения

1. `/pause <robot>` (или `/resume`, `/stop`) в фазе A ставит в очередь
   confirm-target snapshot-команду.
2. На её терминальном результате replies lane резолвит 8-символьный handle
   по роботам владельца — при отсутствии совпадений отвечает списком
   доступных handle, два и более совпадения отклоняются как
   неоднозначные — и выпускает одноразовое подтверждение: 16-символьный
   base32-токен, хранимый только как SHA-256, рядом с зафиксированным
   кортежем привязки, отпечатком чата, authorization revision и
   portfolio/ledger/robot revision на момент выпуска. Сырой токен
   существует ровно один раз — внутри ответа; он истекает через 120 секунд,
   и на владельца одновременно не больше 3 неиспользованных подтверждений.
3. `/confirm <token>` хэширует токен и блокирует неиспользованную
   неистёкшую строку (`FOR UPDATE`) внутри фазы A, затем повторно
   доказывает отпечаток чата, владельца, точные id и revision привязки и ту
   же authorization revision. Любое несовпадение — один одинаковый отказ,
   который **не** потребляет токен. Потребление и durable-команда
   `paper-robot.action` коммитятся в одной транзакции; исполнитель
   применяет действие под зафиксированными оптимистичными fence. Неверные
   токены сжигают тот же бюджет попыток чата, что и коды привязки.
4. Replies lane отвечает на терминальную команду действия applied-исходом
   или безопасно сформулированным отказом.

### Лимиты команд и retention

| Граница | Лимит |
| --- | ---: |
| TTL токена подтверждения | 120 с |
| Неиспользованные подтверждения владельца | 3 |
| Попытки подтверждения/кода привязки из чата | 5 / 10 мин |
| Обработанные команды из чата | 6 / мин |
| Роботы в ответе снапшота | 20 |
| Fills в ответе `/trades` | 10 |
| Правила / события в ответе `/alerts` | 10 / 5 |
| Timeout ответа нетерминальной команды | 10 мин |

Retention удаляет использованные или истёкшие подтверждения через 2 дня и
отвеченные строки ответов через 7 дней теми же ограниченными `SKIP LOCKED`
стадиями, что и остальную историю алертов; строка ответа также следует за
своей executor-командой через `ON DELETE CASCADE`, поэтому retention
executor-команд не оставляет сирот.
...
расширенные входящие команды бота — это принятый релиз R5.3b-2, описанный
в разделе [Telegram paper-команды](#telegram-paper-команды-r53b-2).
