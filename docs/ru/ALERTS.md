...
`/start`, `/bind` и статичного ответа-заглушки, — это инкремент R5.3b-2 в
работе, описанный в разделе
[Telegram paper-команды](#telegram-paper-команды-r53b-2-в-работе).
...
## Telegram paper-команды (R5.3b-2, в работе)

R5.3b-2 расширяет привязанный приватный чат командами чтения и двухшаговым
fenced-управлением paper-роботами. Инкремент **в работе и не принят**:
кандидатная миграция 16 `telegram_command_bridge` добавляет таблицы
`telegram_command_replies` и `telegram_confirmations`, production cutover не
выполнялся. Worker по-прежнему не открывает HTTP-listener и никогда не
...
расширенные входящие команды бота — это инкремент R5.3b-2 в работе,
описанный в разделе
[Telegram paper-команды](#telegram-paper-команды-r53b-2-в-работе).
