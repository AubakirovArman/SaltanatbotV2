...
## DCA paper-роботы (R6, в работе)

Статус R6: реализовано на `main`, **не принято и не развёрнуто**. Принятое production-evidence
выше по-прежнему описывает R4; release gate из [RELEASING.md](../RELEASING.md) — exact-commit CI,
защищённый слот, paired recovery rehearsal и cutover — для R6 ещё не выполнялся. R6 не меняет
схемы PostgreSQL и SQLite: DCA-роботы используют существующие типы paper-событий и settings
snapshots, а каждый ledger до R6 воспроизводится байт-в-байт без изменений.
