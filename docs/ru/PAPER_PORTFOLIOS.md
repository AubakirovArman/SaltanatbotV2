...
## Multi-leg paper-интенты (R8, принят)

Статус R8: **принят и развёрнут**. Release gate R8 из [RELEASING.md](../RELEASING.md) —
exact-commit CI run `29639908389` (`6/6`) на commit
`69621f8107a713031f768320e9dc496010234100`, защищённый слот `r8a-schema16-69621f8`, paired
репетиция миграции v9→v10 на копии production `trading.db`, paired recovery и cutover —
пройден; см. записанное
[R8 acceptance evidence](../evidence/R8_MULTI_LEG_PAPER_INTENTS.md). Production работает на
неизменной PostgreSQL schema 16 и trading SQLite schema 10 — первой SQLite-миграции после R4.
...
### Trading SQLite миграция v10 (применена в production)
...
ledgers воспроизводятся без изменений. Миграция применена в production при принятом cutover R8:
`scripts/rehearse-trading-migration.mjs` сначала мигрировал **копию** production `trading.db`
(`fromVersion 9 → toVersion 10`, применена ровно `owner_scoped_paper_multi_leg`), не касаясь
рабочего data directory, затем рестарт через слот `r8a-schema16-69621f8` применил v10 на
старте (`PRAGMA user_version` = 10). См. [Migration notes](../MIGRATIONS.md).
