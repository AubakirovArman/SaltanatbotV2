...
Статус: active; реализация и часть целевых изолированных проверок выполнены.
Полные build/browser/recovery gates, production cutover и release evidence ещё
не завершены.
...
deduplication ID. Long-poll consumer takeover, lease loss, duplicate/replayed и
out-of-order updates, crash до/после durable cursor, Telegram timeout/`429`,
истечение/перебор/replay кодов, revoke race и cross-owner команды покрыты
failure tests; один `update_id` создаёт не более одной durable paper mutation
до и после restart или consumer takeover.
...
- измерить и настроить уже реализованный process-wide API admission, затем
  добавить недостающие global caps для WebSocket, robots, jobs, alerts,
  screener и L2;
...
3. R3.1 и R3.2 приняты и развёрнуты; schema-10 API-first cutover, защищённый
   ...
   evidence.
4. Завершить активный R3.3 + O1 в изоляции: принять hardening recovery tooling,
   прогнать полную PostgreSQL integration matrix, собрать frontend вне
   production checkout и закрыть Chromium, Firefox, visual, PWA, bundle,
   architecture и documentation gates. Синхронизировать generated API index,
   schema/configuration/security docs с защищёнными onboarding/operations
   routes; добавить обещанные ссылки на документацию и одно следующее действие
   в принятых empty-state journeys.
5. До любой production-мутации зафиксировать точные project root, units,
   Compose/container, порты, DB names и data paths; создать парную schema-10
   PostgreSQL/SQLite generation, проверить её и выполнить replacement-only
   restore drill с реально доказанными matching `pg_dump`/`pg_restore`.
   Последняя verified recovery generation сохраняется и показывается в
   admin-метриках только после успешной проверки, а не как недоказанный
   placeholder. Любое несовпадение identity или конфликт с чужим ресурсом
   останавливает релиз.
6. После зелёных локальных gates зафиксировать принятый инкремент в `main`,
   проверить GitHub Actions, собрать protected exact-commit release, мигрировать
   только project database до schema 11, перезапустить только project API/worker,
   переключить только защищённый frontend slot и выполнить
   owner/onboarding/readiness/backup smoke на порту 4180.
7. После принятия R3 перейти к paper-account contract R4; UI «Запущено»
   подключать только к каноническому ledger, не к ещё одному временно
   авторитетному browser/database state.

### 18.4. Покрытие ключевых выводов свежего аудита

| Вывод аудита | Где закрывается |
| --- | --- |
| Алерты умирают вместе с вкладкой; мало типов условий | R5.1–R5.3: server scheduler, indicator/drawing alerts, outbox и notification worker |
| Нет DCA и Grid | R6 и R7 поверх общего R4 paper ledger |
| Нет аналитики запущенных роботов | R4.1–R4.2: reservations, reconciliation, equity/PnL/drawdown/journal |
