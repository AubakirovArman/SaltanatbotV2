...
| R3 | active | R3.1 развёрнут; R3.2 release candidate прошёл backend/frontend/PostgreSQL/Chromium/Firefox/visual gates и имеет schema-9 backup/rollback slot | production cutover R3.2, затем R3.3 onboarding/PWA boundary | [R3.1 evidence](../evidence/R3_1_IDENTITY_CONTROL_PLANE.md) + [R3.2 evidence](../evidence/R3_2_WORKSPACE_WORKFLOW.md) |
...
Статус: R3.1 завершён и развёрнут; R3.2 release candidate принят локально,
production cutover ожидает commit/push/green CI. Baseline: PostgreSQL auth,
...
archive/purge и явное разрешение конфликтов. Remaining delta: безопасно
развернуть schema v10 и затем реализовать onboarding без доступа администратора
к tenant-owned product data. Evidence:
...
Статус: release candidate принят локально; production cutover pending.
...
строго последовательный: R3.2 → R3.3 с соответствующим срезом O1 → R4 → R5 →
R6 → R7 → R8 → R9 → R10A → R10B → R11 → R12. Оставшийся ручной device/AT
...
3. R3.1 принят и развёрнут. R3.2 прошёл локальные release gates; следующий шаг —
   commit/push, green CI и контролируемый API-first schema-10 cutover с
   проверенным schema-9 backup/rollback slot.
4. Включить требуемый срез O1 в R3.3: безопасный replacement-restore,
