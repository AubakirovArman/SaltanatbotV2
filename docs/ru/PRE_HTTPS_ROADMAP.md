...
Текущий production — принятый R4 на PostgreSQL schema 12. Описанный ниже R5.1
для schema 13 существует только как implementation candidate: он ещё не принят
и не развёрнут.
...
| R4 | done | schema-12 fenced commands, SQLite-9 canonical portfolios/epochs/reservations, versioned metrics, bounded Running journal/curve и extended recovery inventory приняты и развёрнуты | сопровождение защищённого слота `r4c-schema12-bb455fa` | [accepted R4 evidence](../evidence/R4_PAPER_PORTFOLIOS.md) |
| R5 | active | R5.1 generic owner price alerts реализованы как schema-13 implementation candidate; production остаётся R4/schema12 | принять R5.1; реализовать R5.2 technical screener и R5.3 notification worker/Telegram | migration/cursor/multi-tab/mobile gates + alert/chart parity + delivery drill |
...
Production позже перешёл на принятый R4 schema 12/schema 9. R5.1 теперь
существует как непринятый implementation candidate, а R5.2/R5.3 остаются
...
Статус: завершён, принят и развёрнут. Production использует PostgreSQL schema
12 и trading SQLite schema 9 из защищённого слота `r4c-schema12-bb455fa` на
коммите `bb455facdfe5a1b3cabe15490c86c299ea684ee7`; exact-SHA GitHub Actions
run `29560112312` прошёл 6/6 jobs. Runtime остаётся
...
Статус: R5.1 существует как implementation candidate, но ещё не принят и не
развёрнут; production остаётся R4 на PostgreSQL schema 12. R5.2 и R5.3 не
реализованы и остаются pending.
...
- PostgreSQL schema 13 хранит owner-scoped rules, immutable revisions, state,
  ...
  bounded retention; production schema 12 пока не мигрировала;
...
- до принятия обязательны exact upgrade/recovery, browser-closed, multi-tab,
  forward-cursor, desktop/mobile accessibility и visual gates.
...
- расширять schema-13 candidate дальше generic price threshold можно только
...
R5.1 candidate gate: schema 12→13/checksum/no-op migration, real unprivileged
...
accessibility/visual checks. Эти проверки делают candidate проверяемым, но сами
по себе не означают accepted/deployed.
...
R5.1 candidate уже содержит собственные beta limits 100/200/400/480,
scheduler admission 4/16/8 и retention 2/30 дней, но они ещё не приняты в
production и не имеют integrated 100-user evidence. R11 остаётся pending и не
...
- измерить и настроить уже реализованный process-wide API admission, проверить
  candidate alert cap под общей нагрузкой, затем добавить недостающие global
...
| R5.1 | implementation candidate: PostgreSQL generic owner `price-threshold` rules/events/in-app outbox и bounded scheduler; не accepted/deployed | schema-13 upgrade + cursor/multi-tab/mobile + closed-browser restart/dedupe |
...
2. Проверить R5.1 implementation candidate по schema-13 upgrade, owner
   isolation, forward-cursor, multi-tab, mobile и recovery gates; до exact-SHA
   acceptance/cutover evidence не называть его развёрнутым.
...
| --- | --- |
| Алерты умирают вместе с вкладкой; мало типов условий | Generic `price-threshold` закрыт только в R5.1 candidate; его release gate ещё не принят. Indicator/drawing/screener alerts остаются R5.2, notification worker — R5.3 |
