...
| R5 | pending | research alert/outbox slices, arbitrage screener, canonical indicators; следующий pre-HTTPS инкремент | unified alerts, technical screener MVP, notification worker и Telegram | scheduler/outbox crash tests + alert/chart parity |
...
Production позже перешёл на принятый R4 schema 12/schema 9; следующий pending
инкремент — R5. Baseline: PostgreSQL auth,
...
Статус: следующий pending pre-HTTPS инкремент; реализация ещё не началась.
Baseline: отдельные research/arbitrage alert rules,
...
2. До реализации зафиксировать R5 contracts для alerts, технического скринера
   и notifications: tenant ownership, scheduler/outbox authority и явные
   paper-only Telegram capabilities.
3. Поставить R5.1–R5.3 с chart/screener parity, closed-browser restart/dedupe,
   bounded delivery и isolated recovery evidence.
