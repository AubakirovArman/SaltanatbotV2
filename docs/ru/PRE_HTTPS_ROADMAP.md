...
Текущий production — принятый релиз R6 (общий paper execution contract и
DCA paper-робот) на PostgreSQL schema 16 из защищённого слота
`r6a-schema16-e2411ab` на коммите
`e2411ab2f0b4540200089af8128304f71d3f73e0` без миграции. Описанные ниже
R5.1, R5.2.1, R5.3a, R5.3b-1, R5.3b-2, chart research tools и R6 приняты и
развёрнуты; R5 и R6 завершены, следующий pending инкремент — R7.
...
R5.3b-2 schema 16 и релиз R5 chart research tools, а теперь на принятый R6
DCA paper-робот на неизменной schema 16. Baseline: PostgreSQL auth,
...
schema 16 и релиз R5 chart research tools (слот `r5f-schema16-2ff6101`),
а теперь на принятый R6 DCA paper-робот (слот `r6a-schema16-e2411ab`,
schema 16 без изменений). Runtime остаётся
...
Production позже перешёл на принятый R6 (слот `r6a-schema16-e2411ab`,
...
`29633743310` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на порту
4180. Accepted evidence:
...
строго последовательный от следующего pending инкремента: R7 → R8 →
R9 → R10A → R10B → R11 →
...
9. Следующий pending инкремент — R7 (Grid paper-робот на том же paper
   ledger и state machine); не допускать код более поздних релизов в `main`
   или production до полной приёмки R7.
...
| Нет DCA и Grid | DCA закрыт принятым и развёрнутым R6 поверх общего R4 paper ledger; Grid остаётся в R7 |
