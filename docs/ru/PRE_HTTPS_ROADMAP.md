...
Текущий production — принятый R5.3b-1 Telegram delivery worker и chat binding
на PostgreSQL schema 15 из защищённого слота `r5d-schema15-cd34ec8` на коммите
`cd34ec8d11810a652bf087718f498dcece3b75fa`. Описанные ниже R5.1, R5.2.1,
R5.3a и R5.3b-1 приняты и развёрнуты; R5 в целом остаётся active из-за
pending R5.3b-2 Telegram-команд и chart research tools.
...
- paper-роботы, paper-портфель и in-app исследовательские алерты; принятый
  R5.3b-1 добавил Telegram delivery и owner-scoped chat binding, а входящие
  Telegram-команды остаются будущим scope R5.3b-2;
...
| R5 | active | R5.1 generic owner price alerts, R5.2.1 technical screener MVP, R5.3a screen→alert promotion и R5.3b-1 Telegram delivery/chat binding приняты и развёрнуты на schema 15 (слот `r5d-schema15-cd34ec8`, миграция 14→15, notification worker намеренно idle без bot token) | реализовать R5.3b-2 Telegram-команды и chart research tools | [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md) + [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md) + [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) + [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md) + remaining R5.3b-2 command drill |
...
R5.1 schema 13, R5.2.1 schema 14 и R5.3a schema 14, а теперь на принятый
R5.3b-1 на schema 15; R5.3b-2 остаётся pending. Baseline: PostgreSQL auth,
...
Статус: R5.1, R5.2.1 technical screener MVP, R5.3a screen→alert promotion и
R5.3b-1 Telegram delivery worker с chat binding приняты и развёрнуты;
production работает на PostgreSQL schema 15 и неизменной trading SQLite
schema 9 из защищённого слота `r5d-schema15-cd34ec8` на коммите
`cd34ec8d11810a652bf087718f498dcece3b75fa`; exact-SHA GitHub Actions run
`29622330910` прошёл 6/6 jobs
...
[R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md)). Исторически
...
`20be5b1d2fb87df38cc298953dfe7a2f414dd831`, а R5.3a — со слота
`r5c-schema14-86712ba` на коммите
`86712bac3293ac8d746b638218eb66995d8e5edb`. R5.3b-2 Telegram-команды и
chart research tools остаются pending, поэтому R5 в целом остаётся active.
...
### Remaining R5.2 alert types и R5.3b-2 команды

Следующие типы алертов не входят в принятые R5.1/R5.2.1/R5.3a/R5.3b-1 и
остаются pending:
...
Remaining R5.3b-2 — Telegram-команды в paper-only режиме:
...
Полный критерий R5: R5.1, R5.2.1, R5.3a и R5.3b-1 отдельно приняты, значение
screener совпадает с графиком на той же закрытой свече, а сохранённый screen
производит owner-scoped alert с закрытым браузером и не
...
истечение/перебор/replay binding-кодов и revoke race уже покрыты принятыми
failure tests; remaining R5.3b-2 добавляет cross-owner команды,
confirmation-token evidence и доказательство, что один command `update_id`
...
| R5 | R5.1 alerts, R5.2.1 screener MVP, R5.3a screen→alert promotion и R5.3b-1 Telegram delivery/binding приняты; remaining: R5.3b-2 Telegram-команды и chart research tools | R3–R4 | остаток R5.3b-2 в пределах исходной оценки 5–7 | at-least-once outbox drill пройден; remaining — R5.3b-2 command drill |
...
| R5.3 | R5.3a done: screen→alert promotion (rule kind `screener`) принят и развёрнут без миграции; R5.3b-1 done: notification worker, Telegram delivery и binding/revoke приняты и развёрнуты на schema 15; remaining R5.3b-2: paper-only команды с one-time confirmations через fenced executor | [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) и [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md) зафиксированы; remaining — R5.3b-2 command drill |
...
6. Реализовать remaining R5.3b-2 Telegram read и paper-control команды с
   one-time confirmations через fenced executor, затем получить command и
   isolated recovery evidence.
7. Не допускать R6 и более поздний код в `main` или production до полной
   приёмки R5.
...
| --- | --- |
| Алерты умирают вместе с вкладкой; мало типов условий | Generic `price-threshold` закрыт принятым и развёрнутым R5.1, screener alerts — принятым и развёрнутым R5.3a, notification worker и Telegram delivery — принятым и развёрнутым R5.3b-1. Indicator/drawing alerts остаются R5.2, Telegram-команды — R5.3b-2 |
