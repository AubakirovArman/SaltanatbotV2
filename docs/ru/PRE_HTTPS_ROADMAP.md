...
Текущий production — принятый R5.3a screener alert promotion на неизменной
PostgreSQL schema 14 из защищённого слота `r5c-schema14-86712ba` на коммите
`86712bac3293ac8d746b638218eb66995d8e5edb` (релиз без миграции). Описанные
ниже R5.1, R5.2.1 и R5.3a приняты и развёрнуты; R5 в целом остаётся active
из-за pending R5.3b notification worker и chart research tools.
...
- paper-роботы, paper-портфель и in-app исследовательские алерты; Telegram
  остаётся разрешённым будущим scope R5.3b, а не текущей возможностью алертов;
...
| R5 | active | R5.1 generic owner price alerts, R5.2.1 technical screener MVP и R5.3a screen→alert promotion приняты и развёрнуты на неизменной schema 14 (слот `r5c-schema14-86712ba`, релиз без миграции, chart/screener parity gate пройден) | реализовать R5.3b notification worker/Telegram и chart research tools | [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md) + [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md) + [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) + delivery drill |
...
R5.1 schema 13 и R5.2.1 schema 14, а теперь на принятый R5.3a на неизменной
schema 14; R5.3b остаётся pending. Baseline: PostgreSQL auth,
...
Статус: R5.1, R5.2.1 technical screener MVP и R5.3a screen→alert promotion
приняты и развёрнуты; production работает на неизменных PostgreSQL schema 14
и trading SQLite schema 9 из защищённого слота `r5c-schema14-86712ba` на
коммите `86712bac3293ac8d746b638218eb66995d8e5edb`; exact-SHA GitHub Actions
run `29590401183` прошёл 6/6 jobs
([R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md),
[R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md),
[R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md)). Исторически R5.1 был
развёрнут со слота `r5a-schema13-66394fd` на коммите
`66394fd38765d8da36174411cecd95a33fda1ea0`, а R5.2.1 — со слота
`r5b-schema14-20be5b1` на коммите
`20be5b1d2fb87df38cc298953dfe7a2f414dd831`. R5.3b notification worker и
...
- собственные caps — 5 enabled правил на owner и 40 active глобально — внутри
  общих alert-квот; `telegram` delivery на screener-правиле отклоняется с
  `400 unsupported_alert_delivery_channel` до R5.3b;
...
### Remaining R5.2 alert types и R5.3b delivery

Следующие типы алертов не входят в принятые R5.1/R5.2.1/R5.3a и остаются
pending:
...
- реализованные evaluation lanes остаются вне API request path; отдельный
  R5.3b delivery worker добавляет retry/backoff и dead-letter, не получая
  trading authority;
- принятые алерты содержат только in-app history. Telegram относится к R5.3b;
  Web Push отложен до HTTPS;
...
Полный критерий R5: R5.1, R5.2.1 и R5.3a отдельно приняты, значение screener
совпадает с графиком на той же закрытой свече, а сохранённый screen
производит owner-scoped alert с закрытым браузером и не
раскрывает чужие presets/rules/deliveries; R5.3b допускает повтор внешней
...
runs, а принятый R5.3a — caps 5 screener-правил на owner/40 глобально; они
приняты в production, но не имеют integrated 100-user evidence. R11
остаётся pending и не может быть заменён размером текущего сервера или
...
- измерить и настроить уже реализованный process-wide API admission, проверить
  принятые R5.1 alert и R5.2.1 screener-preset caps под общей нагрузкой, затем
  добавить недостающие global caps для WebSocket, robots, jobs и L2;
...
| R5 | R5.1 alerts, R5.2.1 screener MVP и R5.3a screen→alert promotion приняты; remaining: notification worker/Telegram и chart research tools | R3–R4 | остаток R5.3b в пределах исходной оценки 5–7 | at-least-once outbox drill |
...
| R5.3 | R5.3a done: screen→alert promotion (rule kind `screener`) принят и развёрнут без миграции; remaining R5.3b: notification worker, Telegram binding/revoke и paper-only команды | [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) зафиксировано; remaining — at-least-once delivery drill |
...
5. Зафиксировать и реализовать pending contracts R5.3b notification/Telegram,
   затем получить bounded delivery и isolated recovery evidence.
6. Не допускать R6 и более поздний код в `main` или production до полной
   приёмки R5.
...
| --- | --- |
| Алерты умирают вместе с вкладкой; мало типов условий | Generic `price-threshold` закрыт принятым и развёрнутым R5.1, screener alerts — принятым и развёрнутым R5.3a. Indicator/drawing alerts остаются R5.2, notification worker — R5.3b |
