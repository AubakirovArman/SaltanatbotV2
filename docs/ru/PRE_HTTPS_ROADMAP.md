...
Текущий production — принятый R5.2.1 на PostgreSQL schema 14 из защищённого
слота `r5b-schema14-20be5b1` на коммите
`20be5b1d2fb87df38cc298953dfe7a2f414dd831`. Описанные ниже R5.1 и R5.2.1
приняты и развёрнуты; R5 в целом остаётся active из-за pending R5.3,
screen→alert promotion и chart research tools.
...
| R5 | active | R5.1 generic owner price alerts и R5.2.1 technical screener MVP приняты и развёрнуты на schema 14 (слот `r5b-schema14-20be5b1`, chart/screener parity gate пройден) | реализовать R5.3 notification worker/Telegram, screen→alert promotion и chart research tools | [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md) + [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md) + delivery drill |
...
Production позже перешёл на принятый R4 schema 12/schema 9, затем на принятый
R5.1 schema 13, а теперь на принятый R5.2.1 schema 14; R5.3 остаётся
...
schema 13, а затем на принятый R5.2.1 schema 14 (слот
`r5b-schema14-20be5b1`). Runtime остаётся
...
Статус: R5.1 и R5.2.1 technical screener MVP приняты и развёрнуты; production
работает на PostgreSQL schema 14 из защищённого слота `r5b-schema14-20be5b1`
на коммите `20be5b1d2fb87df38cc298953dfe7a2f414dd831`; exact-SHA GitHub
Actions run `29584556266` прошёл 6/6 jobs
([R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md),
[R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md)). Исторически
R5.1 был развёрнут со слота `r5a-schema13-66394fd` на коммите
`66394fd38765d8da36174411cecd95a33fda1ea0`. R5.3, screen→alert promotion и
...
- превращение сохранённого screen в серверный alert остаётся pending: rule
  kind `screener` — зарезервированный placeholder, а не runtime.
...
### Remaining R5.2 alert promotion и R5.3 delivery

Следующие типы алертов не входят в принятые R5.1/R5.2.1 и остаются pending:
...
- расширять принятую схему алертов дальше generic price threshold можно только
...
Полный критерий R5: R5.1 и R5.2.1 отдельно приняты, и значение screener
совпадает с графиком на той же закрытой свече; сохранённый screen может
произвести owner-scoped alert с закрытым браузером и не
...
scheduler admission 4/16/8 и retention 2/30 дней, а принятый R5.2.1 добавляет
caps 40 presets на owner/400 глобально, universe ≤200 и bounded compute-job
runs; они приняты в production, но не имеют integrated 100-user evidence. R11
...
| R5 | R5.1 alerts и R5.2.1 screener MVP приняты; remaining: notification worker/Telegram, screen→alert promotion и chart research tools | R3–R4 | остаток R5.3/promotion в пределах исходной оценки 5–7 | at-least-once outbox drill + promotion evidence |
...
| R5.2 | R5.2.1 done: технический скринер MVP на canonical indicator engine принят и развёрнут на schema 14; screen→alert promotion остаётся pending | chart/screener parity пройден; remaining — promotion evidence |
...
4. Зафиксировать и реализовать pending contracts R5.3 notification/Telegram и
   screen→alert promotion, затем получить bounded delivery и isolated recovery
   evidence.
5. Не допускать R6 и более поздний код в `main` или production до полной
   приёмки R5.
