...
| R3 | active | R3.1 завершён: atomic lifecycle, sessions/revoke, audit, guarded recovery, запрет non-admin live; PostgreSQL owner-scoped workspace CRUD/revisions/409/rollback | R3.2 полный workflow document/quotas/import-export и R3.3 onboarding/PWA boundary | [R3.1 evidence](../evidence/R3_1_IDENTITY_CONTROL_PLANE.md) + workspace isolation/conflict evidence |
...
Статус: R3.1 завершён и развёрнут; R3.2 следующий. Baseline: PostgreSQL auth,
...
quotas/import/export UX и onboarding без доступа администратора к tenant-owned
product data. Evidence R3.1:
[R3.1 identity control plane](../evidence/R3_1_IDENTITY_CONTROL_PLANE.md).
...
### 18.2. Допустимая параллельность

- Публикация в `main` идёт по порядку релизов; незавершённые state machines и
  PostgreSQL migration chains не смешиваются.
- После фиксации API-контракта внутри одного релиза параллельно выполняются
  repository/schema, frontend, тесты и документация.
- После R4 R8 можно готовить рядом с R6/R7, но R7 начинается только после
  принятого общего DCA/paper lifecycle R6.
- R9 использует отдельный тип research jobs и может идти рядом с роботами после
  generic worker contract.
- Формат и bounded collector R10A готовятся заранее; календарный soak может идти
  фоном, но R10B не принимается до corpus gate.
- Метрики, quotas, load fixtures и backup evidence растут в каждом релизе; R11
  только объединяет их в итоговый сценарий.
...
3. R3.1 принят и развёрнут. Следующий кодовый этап — R3.2: полный owner-scoped
   workflow document, явные save/conflict/error состояния, quotas и
   import/export.
4. Параллельно продолжить O1: безопасный replacement-restore, readiness/metrics и
   первые global admission limits.
