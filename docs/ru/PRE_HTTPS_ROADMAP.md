...
Текущий production — принятый релиз R8 (owner-scoped multi-leg paper intents
на общем capital plane) на неизменной PostgreSQL schema 16 и trading SQLite
schema 10 из защищённого слота `r8a-schema16-69621f8` на коммите
`69621f8107a713031f768320e9dc496010234100`; первая с R4 миграция SQLite —
additive `owner_scoped_paper_multi_leg` 9→10; accepted evidence —
...
R6, R7 и R8 приняты и развёрнуты; R5, R6, R7 и R8 завершены, следующий
pending инкремент — R9.
...
R5.3b-2 schema 16, релиз R5 chart research tools, R6 DCA paper-робот и R7
Grid paper-робот, а теперь на принятый R8 multi-leg paper intents на
schema 16 и trading SQLite schema 10.
...
DCA paper-робот (слот `r6a-schema16-e2411ab`) и R7 Grid paper-робот (слот
`r7a-schema16-baf4217`), а теперь на принятый R8 multi-leg paper intents
(слот `r8a-schema16-69621f8`, schema 16 без изменений, additive миграция
trading SQLite 9→10). Runtime
...
на принятый R7 Grid paper-робот (слот `r7a-schema16-baf4217`), а теперь на
принятый R8 multi-leg paper intents (слот `r8a-schema16-69621f8`, schema 16
без изменений, additive миграция trading SQLite 9→10).
...
paper-робот (слот `r7a-schema16-baf4217`), а затем на принятый R8 multi-leg
paper intents (слот `r8a-schema16-69621f8`, schema 16 без изменений, additive
миграция trading SQLite 9→10); runtime
...
без изменений, additive миграция trading SQLite 9→10); runtime остаётся
...
`29639908389` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на порту
...
строго последовательный от следующего pending инкремента: R9 →
R10A → R10B → R11 →
...
11. Следующий pending инкремент — R9 (server multi-market GA pipeline; до
    schema/job API R9.1 закрывается решение D2 — canonical Strategy IR,
    versioned dataset contract и воспроизводимый backtest engine); не
