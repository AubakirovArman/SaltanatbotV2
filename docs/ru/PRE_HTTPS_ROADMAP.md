...
Текущий production — принятый R5.3b-2 Telegram paper-команды поверх fenced
executor на PostgreSQL schema 16 из защищённого слота `r5e-schema16-17e12f1`
на коммите `17e12f17933de5ffb047d63358a05fad8f0211f0`. Описанные ниже R5.1,
R5.2.1, R5.3a, R5.3b-1 и R5.3b-2 приняты и развёрнуты; R5 в целом остаётся
active только из-за pending chart research tools (текстовые заметки и
parallel channel).
...
| R5 | active | R5.1 generic owner price alerts, R5.2.1 technical screener MVP, R5.3a screen→alert promotion, R5.3b-1 Telegram delivery/chat binding и R5.3b-2 Telegram paper-команды приняты и развёрнуты на schema 16 (слот `r5e-schema16-17e12f1`, миграция 15→16, notification worker намеренно idle без bot token; установка token file включает команды без релиза) | реализовать chart research tools (текстовые заметки и parallel channel) | [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md) + [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md) + [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) + [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md) + [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md) |
...
R5.1 schema 13, R5.2.1 schema 14, R5.3a schema 14 и R5.3b-1 schema 15, а
теперь на принятый R5.3b-2 на schema 16. Baseline: PostgreSQL auth,
...
schema 13 и R5.2.1 schema 14, а теперь на принятый R5.3a (слот
`r5c-schema14-86712ba`, schema 14 без изменений). Runtime остаётся
...
Статус: R5.1, R5.2.1 technical screener MVP, R5.3a screen→alert promotion,
R5.3b-1 Telegram delivery worker с chat binding и R5.3b-2 Telegram
paper-команды приняты и развёрнуты; production работает на PostgreSQL
schema 16 и неизменной trading SQLite schema 9 из защищённого слота
`r5e-schema16-17e12f1` на коммите
`17e12f17933de5ffb047d63358a05fad8f0211f0`; exact-SHA GitHub Actions run
`29625979877` прошёл 6/6 jobs
...
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md)). Исторически
R5.1 был развёрнут со слота `r5a-schema13-66394fd` на коммите
...
`86712bac3293ac8d746b638218eb66995d8e5edb`, а R5.3b-1 — со слота
`r5d-schema15-cd34ec8` на коммите
`cd34ec8d11810a652bf087718f498dcece3b75fa`. Chart research tools (текстовые
заметки и parallel channel) остаются единственным pending пунктом, поэтому
R5 в целом остаётся active.
...
R5.1/R5.2.1/R5.3a/R5.3b-1/R5.3b-2, остаются отложенными и не входят в
remaining R5 delta:
...
Полный критерий R5: R5.1, R5.2.1, R5.3a, R5.3b-1 и R5.3b-2 отдельно приняты,
значение screener совпадает с графиком на той же закрытой свече, а
сохранённый screen производит owner-scoped alert с закрытым браузером и не
раскрывает чужие presets/rules/deliveries; принятый R5.3b-1 допускает повтор
внешней Telegram-доставки после crash только с тем же deduplication ID.
...
зафиксированный в
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md). До закрытия R5
остаются только chart research tools (текстовые заметки и parallel channel).
...
| R5 | R5.1 alerts, R5.2.1 screener MVP, R5.3a screen→alert promotion, R5.3b-1 Telegram delivery/binding и R5.3b-2 Telegram-команды приняты; remaining: chart research tools (текстовые заметки и parallel channel) | R3–R4 | остаток chart research tools в пределах исходной оценки 5–7 | at-least-once outbox drill и R5.3b-2 command drill пройдены; remaining — chart research tools |
...
6. Выполнено: R5.3b-2 Telegram read и paper-control команды с one-time
   ...
   [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md).
7. Реализовать remaining chart research tools (текстовые заметки и parallel
   channel) на общем geometry contract и закрыть полный R5 release gate.
8. Не допускать R6 и более поздний код в `main` или production до полной
   приёмки R5.
