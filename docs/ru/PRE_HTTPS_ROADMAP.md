...
Текущий production — принятый релиз R7 (Grid paper-робот на общем execution
contract) на PostgreSQL schema 16 из защищённого слота
`r7a-schema16-baf4217` на коммите
`baf42178d33043fde0965d008aee9f09462df699` без миграции; accepted evidence —
[R7 grid paper robot](../evidence/R7_GRID_PAPER_ROBOT.md). Описанные ниже
R5.1, R5.2.1, R5.3a, R5.3b-1, R5.3b-2, chart research tools, R6 и R7 приняты
и развёрнуты; R5, R6 и R7 завершены, следующий pending инкремент — R8.
...
R5.3b-2 schema 16, релиз R5 chart research tools и R6 DCA paper-робот,
а теперь на принятый R7 Grid paper-робот на неизменной schema 16.
...
schema 16, релиз R5 chart research tools (слот `r5f-schema16-2ff6101`) и R6
DCA paper-робот (слот `r6a-schema16-e2411ab`), а теперь на принятый R7 Grid
paper-робот (слот `r7a-schema16-baf4217`, schema 16 без изменений). Runtime
...
Production позже перешёл на принятый R6 (слот `r6a-schema16-e2411ab`), а
теперь на принятый R7 Grid paper-робот (слот `r7a-schema16-baf4217`,
schema 16 без изменений).
...
`29636312303` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на порту
...
строго последовательный от следующего pending инкремента: R8 →
R9 → R10A → R10B → R11 →
...
10. Следующий pending инкремент — R8 (объединённое multi-leg paper
    исполнение: существующий модуль paperMultiLeg интегрируется с общим
    ledger, capital reservations и market-driven fills); не допускать код
    более поздних релизов в `main` или production до полной приёмки R8.
