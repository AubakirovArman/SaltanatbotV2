...
Текущий production — принятый релиз R9.2 (server GA evolution с lineage,
Pareto ranking и OOS promotion) на PostgreSQL schema 17 — additive миграция
`ga_evolution_lineage` 16→17 — и неизменной trading SQLite schema 10 из
защищённого слота `r9b-schema17-3ed6af1` на коммите
`3ed6af138f197ee985bd8ac998ab58cc8769b83c`; accepted evidence —
[R9.2 GA evolution](../evidence/R9_2_GA_EVOLUTION.md).
Описанные ниже R5.1, R5.2.1, R5.3a, R5.3b-1, R5.3b-2, chart research tools,
R6, R7, R8, R9.1 и R9.2 приняты и развёрнуты; R5, R6, R7 и R8 завершены, R9
продолжается, следующий pending инкремент — R9.3.
...
| R9 | active | R9.1 принят и развёрнут: D2 закрыт ADR 0003 (canonical IR `IR_VERSION 4` + pinned checksum guard, `dataset-v1` fingerprint/embargo split, engine `backtest-core-v1`), generic job-kind registry (screener/backtest перерегистрированы byte-identically, unknown kinds hard-fail), server-owned evaluation `multi-market-eval` с детерминированным `multi-market-eval-v1` result и flow «Evaluate on server» поверх pure ranker (слот `r9a-schema16-4f5bc64`, без миграции); R9.2 принят и развёрнут: пакет `@saltanatbotv2/strategy-generator`, job kind `ga-evolution` с fingerprint-pinned dataset, per-generation lineage, Pareto ranks на OOS objectives, atomic checkpoint и cancel→resume, promotion с обязательным clean OOS report и full provenance, owner-scoped `/api/ga` и server evolution секция studio en/ru/kk (слот `r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17 `ga_evolution_lineage`) | R9.3: strategy gallery (versioned artifact, provenance, safe import, revoke) | [R9.1 evidence](../evidence/R9_1_SERVER_EVALUATION.md) + [R9.2 evidence](../evidence/R9_2_GA_EVOLUTION.md) + R9.3: privacy + reproducible artifact |
...
Grid paper-робот, R8 multi-leg paper intents и R9.1 server multi-market
evaluation, а теперь на принятый R9.2 server GA evolution на schema 17 и
trading SQLite schema 10.
...
`r8a-schema16-69621f8`, additive миграция trading SQLite 9→10) и R9.1 server
multi-market evaluation (слот `r9a-schema16-4f5bc64`, без миграции), а
теперь на принятый R9.2 server GA evolution (слот `r9b-schema17-3ed6af1`,
additive миграция PostgreSQL 16→17 `ga_evolution_lineage`). Runtime
...
trading SQLite 9→10) и принятый R9.1 server multi-market evaluation (слот
`r9a-schema16-4f5bc64`, schema 16 и SQLite 10 без изменений), а теперь на
принятый R9.2 server GA evolution (слот `r9b-schema17-3ed6af1`, additive
миграция PostgreSQL 16→17).
...
9→10) и принятый R9.1 server multi-market evaluation (слот
`r9a-schema16-4f5bc64`, без миграции), а теперь на принятый R9.2 server GA
evolution (слот `r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17);
...
миграции), а теперь на принятый R9.2 server GA evolution (слот
`r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17); runtime остаётся
...
а теперь на принятый R9.2 server GA evolution (слот `r9b-schema17-3ed6af1`,
additive миграция PostgreSQL 16→17 `ga_evolution_lineage`); runtime остаётся
`public-http-paper` на порту 4180 на трёх systemd units.
Accepted evidence:
...
Статус: active — R9.1 и R9.2 приняты и развёрнуты, R9 в целом продолжается.
Уже реализованы bounded seeded parameter GA, structural IR generator,
mutation/crossover/deduplication и pure multi-market ranker на
caller-supplied metrics; принятый R9.1 (ниже) добавил контракты D2 и
server-owned candle evaluation с shared capital-pool OOS секцией, а принятый
R9.2 (ниже) — server GA evolution с lineage, Pareto/OOS promotion и
checkpoint/resume. Remaining delta: strategy gallery (R9.3).
...
| R9 | R9.1 (D2 ADR + job registry + server multi-market evaluation) и R9.2 (server GA evolution: lineage, Pareto/OOS promotion, checkpoint/resume) приняты; остаётся R9.3 gallery | R1 + R4 portfolio metrics + canonical IR/dataset/backtest (D2 закрыт ADR 0003) | 5–8 | seeded OOS/Pareto promotion пройден (byte-identical double run + byte-identical resume); остаётся R9.3 privacy + reproducible artifact |
...
строго последовательный от следующего pending инкремента: R9.3 →
R10A → R10B → R11 →
...
13. Следующий pending инкремент — R9.3 (versioned strategy gallery с
    provenance, safe import и revoke внутри ещё не закрытого R9); не
    допускать код более поздних релизов в `main` или production до полной
    приёмки R9.3.
