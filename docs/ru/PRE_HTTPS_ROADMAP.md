...
Текущий production — принятый релиз R9.3 (versioned strategy gallery с
provenance и безопасным импортом) на PostgreSQL schema 18 — additive
миграция `versioned_strategy_gallery` 17→18 — и неизменной trading SQLite
schema 10 из защищённого слота `r9c-schema18-7afde0d` на коммите
`7afde0d12b350babb01e166a1888d54c225d41ec`; accepted evidence —
[R9.3 strategy gallery](../evidence/R9_3_STRATEGY_GALLERY.md).
Описанные ниже R5.1, R5.2.1, R5.3a, R5.3b-1, R5.3b-2, chart research tools,
R6, R7, R8, R9.1, R9.2 и R9.3 приняты и развёрнуты; R5, R6, R7, R8 и R9
завершены, следующий pending инкремент — R10A.
...
- paper-роботы, paper-портфель и in-app исследовательские алерты; принятый
  R5.3b-1 добавил Telegram delivery и owner-scoped chat binding, а принятый
  R5.3b-2 — paper-only входящие Telegram-команды через fenced executor;
...
| R5 | done | R5.1 generic owner price alerts, R5.2.1 technical screener MVP, R5.3a screen→alert promotion, R5.3b-1 Telegram delivery/chat binding, R5.3b-2 Telegram paper-команды и chart research tools (текстовые заметки и parallel channel на canonical geometry contract) приняты и развёрнуты на schema 16 (слот `r5f-schema16-2ff6101`, без миграции; notification worker намеренно idle без bot token; установка token file включает команды без релиза) | только сопровождение | [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md) + [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md) + [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md) + [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md) + [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md) + [R5 chart tools evidence](../evidence/R5_CHART_RESEARCH_TOOLS.md) |
| R6 | done | `paper-fill-model-v1` как единый источник parity, versioned fill behaviors (`single-position-v1` byte-compatible по умолчанию + explicit conflict cancel; `averaging-v1` для DCA), `dca-params-v1`/`dca-state-v1`, серверный worst-case enforcement с live preview, per-transition idempotency keys и goldenReplay harness приняты и развёрнуты (слот `r6a-schema16-e2411ab`, без миграции) | только сопровождение | [R6 evidence](../evidence/R6_DCA_PAPER_ROBOT.md) |
| R7 | done | `grid-params-v1` (arithmetic/geometric лестница 2–50 уровней, neutral/long/short, outside-range pause/stop, optional stop-loss + cycle cap), pure machine `grid-state-v1` с idempotency keys `grid:<botId>:<epochCycle>:<ordinal>` и consolidated gap batches, journal-deduplicated restart, additive robot kind `grid`, серверный worst-case enforcement с live preview и pre-start level-price preview, раздельные realized grid PnL и inventory PnL приняты и развёрнуты (слот `r7a-schema16-baf4217`, без миграции) | только сопровождение | [R7 evidence](../evidence/R7_GRID_PAPER_ROBOT.md) |
| R8 | done | fenced command kinds `paper-multi-leg.submit`/`paper-multi-leg.kill-switch` поверх переиспользованных fail-closed research builders и общего freshness gate, worst-case reservation на общем capital plane, pure engine с durable journal `mleg:<intentId>:<sequence>` и exactly-once release, combined both-legs PnL с explicit residual exposure, read model `multiLeg` и owner kill switch приняты и развёрнуты (слот `r8a-schema16-69621f8`, additive миграция trading SQLite 9→10) | только сопровождение | [R8 evidence](../evidence/R8_MULTI_LEG_PAPER_INTENTS.md) |
| R9 | done | R9.1 принят и развёрнут: D2 закрыт ADR 0003 (canonical IR `IR_VERSION 4` + pinned checksum guard, `dataset-v1` fingerprint/embargo split, engine `backtest-core-v1`), generic job-kind registry (screener/backtest перерегистрированы byte-identically, unknown kinds hard-fail), server-owned evaluation `multi-market-eval` с детерминированным `multi-market-eval-v1` result и flow «Evaluate on server» поверх pure ranker (слот `r9a-schema16-4f5bc64`, без миграции); R9.2 принят и развёрнут: пакет `@saltanatbotv2/strategy-generator`, job kind `ga-evolution` с fingerprint-pinned dataset, per-generation lineage, Pareto ranks на OOS objectives, atomic checkpoint и cancel→resume, promotion с обязательным clean OOS report и full provenance, owner-scoped `/api/ga` и server evolution секция studio en/ru/kk (слот `r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17 `ga_evolution_lineage`); R9.3 принят и развёрнут: immutable versioned `gallery-artifact-v1` с whitelisting sanitizer, server+client проверкой canonical-JSON SHA-256, полным provenance и documented composite rating, revalidation-gated import, private/unlisted/public visibility и owner revoke-with-reason, шесть маршрутов `/api/gallery` и Community gallery UI en/ru/kk (слот `r9c-schema18-7afde0d`, additive миграция PostgreSQL 17→18 `versioned_strategy_gallery`) | только сопровождение | [R9.1 evidence](../evidence/R9_1_SERVER_EVALUATION.md) + [R9.2 evidence](../evidence/R9_2_GA_EVOLUTION.md) + [R9.3 evidence](../evidence/R9_3_STRATEGY_GALLERY.md) |
...
| D2 | decided | [ADR 0003: canonical IR, dataset and backtest contract](../adr/0003-canonical-ir-dataset-backtest-contract.md) — canonical Strategy IR `IR_VERSION 4` с pinned checksum guard в `npm run check`, versioned dataset contract `dataset-v1` (sha256 fingerprint + time-ordered split с embargo) и детерминированный engine `backtest-core-v1` | R9 technical owner + architecture maintainer | принято 2026-07-18 — до schema/job API R9.1 и первого server evolutionary run, как требовалось | server GA/generator surfaces разрешены; promotion/gallery остаются запрещены до R9.2/R9.3 |
...
Production позже перешёл на принятый R4 schema 12/schema 9, затем на принятые
R5.1 schema 13, R5.2.1 schema 14, R5.3a schema 14, R5.3b-1 schema 15,
R5.3b-2 schema 16, релиз R5 chart research tools, R6 DCA paper-робот, R7
Grid paper-робот, R8 multi-leg paper intents, R9.1 server multi-market
evaluation и R9.2 server GA evolution, а теперь на принятый R9.3 versioned
strategy gallery на schema 18 и trading SQLite schema 10.
Baseline: PostgreSQL auth,
...
run `29560112312` прошёл 6/6 jobs. Production позже перешёл на принятые R5.1
schema 13, R5.2.1 schema 14, R5.3a schema 14, R5.3b-1 schema 15, R5.3b-2
schema 16, релиз R5 chart research tools (слот `r5f-schema16-2ff6101`), R6
DCA paper-робот (слот `r6a-schema16-e2411ab`), R7 Grid paper-робот (слот
`r7a-schema16-baf4217`), R8 multi-leg paper intents (слот
`r8a-schema16-69621f8`, additive миграция trading SQLite 9→10), R9.1 server
multi-market evaluation (слот `r9a-schema16-4f5bc64`, без миграции) и R9.2
server GA evolution (слот `r9b-schema17-3ed6af1`, additive миграция
PostgreSQL 16→17 `ga_evolution_lineage`), а теперь на принятый R9.3
versioned strategy gallery (слот `r9c-schema18-7afde0d`, additive миграция
PostgreSQL 17→18 `versioned_strategy_gallery`). Runtime
остаётся `public-http-paper`. Операторский runbook:
...
Статус: завершён. R5.1, R5.2.1 technical screener MVP, R5.3a screen→alert
promotion, R5.3b-1 Telegram delivery worker с chat binding, R5.3b-2 Telegram
paper-команды и chart research tools (текстовые заметки и parallel channel)
приняты и развёрнуты; полный релиз R5 был развёрнут на PostgreSQL schema 16
и неизменной trading SQLite schema 9 из защищённого слота
`r5f-schema16-2ff6101` на коммите
`2ff6101b950b42a77c378233dabecf1a5ee76ce7`; exact-SHA GitHub Actions run
`29629886774` прошёл 6/6 jobs
...
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md),
[R5 chart tools evidence](../evidence/R5_CHART_RESEARCH_TOOLS.md)).
Production позже перешёл на принятый R6 (слот `r6a-schema16-e2411ab`), затем
на принятый R7 Grid paper-робот (слот `r7a-schema16-baf4217`), принятый R8
multi-leg paper intents (слот `r8a-schema16-69621f8`, additive миграция
trading SQLite 9→10), принятый R9.1 server multi-market evaluation (слот
`r9a-schema16-4f5bc64`, schema 16 и SQLite 10 без изменений) и принятый
R9.2 server GA evolution (слот `r9b-schema17-3ed6af1`, additive миграция
PostgreSQL 16→17), а теперь на принятый R9.3 versioned strategy gallery
(слот `r9c-schema18-7afde0d`, additive миграция PostgreSQL 17→18).
Исторически R5.1 был развёрнут со слота `r5a-schema13-66394fd` на коммите
`66394fd38765d8da36174411cecd95a33fda1ea0`, R5.2.1 — со слота
`r5b-schema14-20be5b1` на коммите
`20be5b1d2fb87df38cc298953dfe7a2f414dd831`, R5.3a — со слота
`r5c-schema14-86712ba` на коммите
`86712bac3293ac8d746b638218eb66995d8e5edb`, R5.3b-1 — со слота
`r5d-schema15-cd34ec8` на коммите
`cd34ec8d11810a652bf087718f498dcece3b75fa`, а R5.3b-2 — со слота
`r5e-schema16-17e12f1` на коммите
`17e12f17933de5ffb047d63358a05fad8f0211f0`. Pending пунктов в R5 не
осталось.
...
### Технический скринер MVP — R5.2.1 (принят и развёрнут)

Принятые фильтры:

- price, volume, change, RSI, SMA/EMA cross, MACD и ATR;
- funding, OI и ML/order-book signals остаются отложенными до R10A/R10B.

Принятая архитектура:

- один canonical closed-candle/indicator engine с графиком;
- bounded universe не более 200 символов и server batches вместо socket на
  каждую пару;
- owner-scoped presets, sorting, pagination и run history; PostgreSQL schema
  14 добавляет owner-scoped таблицы presets;
...
- запуски выполняются как bounded compute jobs с квотой 5 активных на owner и
  retention 30 дней/200 jobs/256 MiB; beta limits — 40 presets на owner и 400
  presets глобально; эти лимиты не являются R11 capacity evidence;
- на момент приёмки R5.2.1 превращение сохранённого screen в серверный alert
  оставалось pending, а rule kind `screener` был зарезервированным
  placeholder; это закрыто принятым ниже R5.3a.

Acceptance record:
[R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md). Rehearsal включал
end-to-end proof на изолированной replacement-паре: создан preset, run
выполнен compute job-ом по живым закрытым свечам Binance,
30/30 evaluated / 30 matched / 0 unavailable.

Исследовательские инструменты графика (приняты и развёрнуты):
...
- parallel channel как производная двух линий с единым перемещением и
  измеряемой шириной;
- horizontal/trend/channel geometry использует один canonical contract
  `packages/contracts/chartGeometry` для canvas, store, валидации workspace и
  backend-схемы v9;
- workspace schema v9 аддитивна поверх нетронутой v8: документы v7/v8
  остаются byte-for-byte валидными, backend принимает revisions 7|8|9;
- полный каталог из 21 инструмента (было 19) доступен через общий mobile
  bottom sheet, без отдельного урезанного набора; visual baseline
  перегенерирован;
- релиз без миграции: PostgreSQL schema 16 и trading SQLite schema 9 не
  изменились; новый axe audit нашёл два WCAG AA contrast-дефекта,
  исправленных до приёмки.

Acceptance record:
[R5 chart tools evidence](../evidence/R5_CHART_RESEARCH_TOOLS.md). Recovery
chronology сохранила проверенную pre-cutover generation `7a734401` и
post-cutover generation `83c4b37e` с пройденным изолированным drill.

### R5.3a — screen→alert promotion (принят и развёрнут)

- rule kind `screener` стал runtime: правило хранит полный
  `screener-definition-v1` by value, неизменяемый в каждой revision, а
  research worker переоценивает его с cadence таймфрейма скрина
  (300–86 400 с) под lease 300 с, не более одной evaluation за sweep с
  bounded 90-секундным market-data budget;
- семантика match-set-changed: первая evaluation инициализирует состояние без
  срабатывания, unavailable символы переносятся как unknown, а не как
  departed, более 30% unavailable universe откладывает evaluation, cooldown
  откладывает без продвижения состояния; после срабатывания правило остаётся
  active (`repeat: "on-change"`, rearm отвечает
  `409 alert_rearm_unsupported`);
- completion записывает `triggered` event, outbox row и pre-delivered in-app
  delivery одной транзакцией по sha256 transition key под receipts producer
  `screener-alert-worker`;
- собственные caps — 5 enabled правил на owner и 40 active глобально — внутри
  общих alert-квот; `telegram` delivery на screener-правиле отклонялся с
  `400 unsupported_alert_delivery_channel` до принятого ниже R5.3b-1;
- Screener workspace создаёт правило кнопкой «Create alert from this screen»;
  screener-правила отображаются в панели алертов, не открывая price-quote
  подписки;
- релиз без миграции: schema 14 и trading SQLite schema 9 не изменились, а
  total-JS bundle cap перенесён 960→984 KiB по reviewed-cap паттерну с
  сохранённым обязательным 10% reserve.

Acceptance record: [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md).
Production worker journal показывает работающий screener alert lane
(`evaluationsPerSweep` 1, 0 failures); recovery chronology сохранила
проверенную pre-cutover generation `dd5c0827` и post-cutover generation
`3632bd9f` с пройденным изолированным drill.

### R5.3b-1 — Telegram delivery worker и chat binding (принят и развёрнут)

- доставку обслуживает отдельный project-owned notification worker без
  публичного HTTP listener и без trading authority; его delivery lane читает
  только PostgreSQL outbox, минимальный owner/chat scope и секрет из
  собственного защищённого environment file, добавляет retry/backoff и
  dead-letter, а ingress lane пишет только нормализованные Telegram updates в
  PostgreSQL и не открывает trading SQLite; Telegram credentials не
  передаются research-worker;
...
- для каждой revision bot identity одновременно работает ровно один consumer:
  PostgreSQL lease и монотонный fencing token не позволяют старому worker
  после потери lease продвигать cursor;
- уникальный `(botRevision, update_id)` и durable fenced forward-only cursor
  продвигаются только после durable outcome: crash до commit приводит к
  безопасному replay, а crash после commit превращает повторно полученный
  update в no-op;
- ingress имеет global, per-chat и per-owner rate limits, bounded update
  size, лимит неуспешных binding attempts и retry/backoff для Telegram
  timeout/`429`; admin role не обходит эти лимиты;
- outbox row сохраняется до отправки; revoke binding проверяется перед каждой
  попыткой; payload содержит event ID и deduplication ID; transport остаётся
  at-least-once, поэтому после crash внешний Telegram API теоретически может
  получить повторную доставку, и эта возможность документирована;
- owner-scoped привязка chat использует криптографически случайный
  high-entropy код, который хранится только как hash, имеет короткий TTL,
  один consume и ограниченное число попыток; код не попадает в URL, logs или
  metrics. Доступны просмотр привязок и немедленный revoke из веб-интерфейса;
- на production host bot token не установлен, поэтому notification worker
  намеренно idle (`notification_worker_idle` reason `token_absent`) со
  здоровым heartbeat; readiness считает worker опциональным, пока не задан
  `OPERATIONS_REQUIRE_NOTIFICATION_WORKER`, а установка token file позже
  включает доставку без нового релиза.

Acceptance record:
[R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md). Recovery
chronology сохранила pre-upgrade schema-14 generation `47645c55` с drill,
stopped rollback source `d86692ad`, post-upgrade schema-15 generation
`ba4f9d40` с drill и replacement-only rollback pair; изолированный 14→15
rehearsal включал idle boot notification worker и binding-code smoke: три
уникальных одноразовых кода, четвёртый запрос → `429`, в базе только hashed
значения.

### Remaining R5.2 alert types и принятый R5.3b-2 — Telegram-команды

Следующие типы алертов не входят в принятые
R5.1/R5.2.1/R5.3a/R5.3b-1/R5.3b-2, остаются отложенными и не входили в
завершённый R5 scope:
...
Сохраняющиеся ограничения backend:

- расширять принятую схему алертов дальше поставленных price-threshold и
  screener kinds можно только после canonical evidence contracts для
  indicator/drawing и paper-robot health/drawdown; наличие schema placeholder
  не означает runtime;
- legacy import не должен смешивать account-aware arbitrage policy state с
  generic owner price-alert state.

R5.3b-2 — Telegram-команды в paper-only режиме — принят и развёрнут:

- `/balance` возвращает только paper balance; также доступны `/daily`, `/profit`,
  `/performance`, `/trades`, `/alerts`;
- `/pause`, `/resume`, `/stop` доступны только для своих paper-ботов и идут
  через fenced executor с durable command idempotency key, сохраняемым до
  изменяющей команды. Каждое подтверждение является отдельным high-entropy
  одноразовым token, связано с owner/chat/action/portfolio/bot
  revision/authorization epoch, имеет короткий TTL и повторно проверяется при
  final consume;
- непосредственно перед consume команда повторно проверяет активную binding
  ...
  действующее подтверждение. Unbound, revoked и cross-owner команды
  fail-closed, не раскрывают tenant data и дают structured audit/counter;
  allowlist команд, bounded command size и лимит неуспешных confirmation
  attempts дополняют принятые ingress-лимиты;
- PostgreSQL schema 16 (миграция 16 `telegram_command_bridge`) добавляет
  таблицы confirmations и command bridge; без bot token notification worker
  остаётся намеренно idle, а установка token file позже включает команды без
  нового релиза;
- никакой передачи токенов, ключей или приватной биржевой телеметрии; Web
  Push отложен до HTTPS.

Acceptance record:
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md). End-to-end
доказательство команд — real-fenced-executor PostgreSQL integration suite:
полный /pause→/confirm→action→reply round trip, не более одной команды на
`update_id`, fail-closed fences. Recovery chronology сохранила pre-upgrade
schema-15 generation `3e4dc4f1` с drill, rehearsal no-op с обоими workers
ready на schema 16, stopped rollback source `0898a08d`, post-upgrade
schema-16 generation `08b6defe` с drill и replacement-only rollback pair.
...
Полный критерий R5 выполнен: R5.1, R5.2.1, R5.3a, R5.3b-1, R5.3b-2 и chart
research tools отдельно приняты, значение screener совпадает с графиком на
той же закрытой свече, а сохранённый screen производит owner-scoped alert с
закрытым браузером и не раскрывает чужие presets/rules/deliveries; принятый
R5.3b-1 допускает повтор внешней Telegram-доставки после crash только с тем
же deduplication ID.
Long-poll consumer takeover, lease loss, duplicate/replayed/out-of-order
updates, crash до/после durable cursor, Telegram timeout/`429`,
истечение/перебор/replay binding-кодов и revoke race покрыты принятыми
failure tests, а принятый R5.3b-2 закрыл cross-owner команды,
истёкшие/перебираемые/replayed confirmation tokens и доказательство, что
один command `update_id` создаёт не более одной durable paper mutation после
restart или takeover, — real-fenced-executor PostgreSQL integration suite,
зафиксированный в
[R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md). Chart research
tools (текстовые заметки и parallel channel) приняты и развёрнуты, поэтому
R5 закрыт.
...
Принятый R6 поставил этот gate: `paper-fill-model-v1` стал единственным
источником fill/fee/slippage parity для replay и paper execution; versioned
fill behaviors (`single-position-v1` по умолчанию byte-compatible плюс
explicit conflict cancel; `averaging-v1` для DCA-усреднения); контракты
`dca-params-v1`/`dca-state-v1`; серверный worst-case enforcement
зарезервированного капитала с live preview; idempotency key на каждый
переход и переиспользуемый goldenReplay harness для R7.

## 10. R6 — DCA paper-робот

Статус: завершён, принят и развёрнут без миграции (PostgreSQL schema 16 и
trading SQLite schema 9 без изменений) из защищённого слота
`r6a-schema16-e2411ab` на коммите
`e2411ab2f0b4540200089af8128304f71d3f73e0`; exact-SHA GitHub Actions run
`29633743310` прошёл 6/6 jobs. Production позже перешёл на принятый R7 Grid
paper-робот (слот `r7a-schema16-baf4217`), затем на принятый R8 multi-leg
paper intents (слот `r8a-schema16-69621f8`, additive миграция trading SQLite
9→10), принятый R9.1 server multi-market evaluation (слот
`r9a-schema16-4f5bc64`, без миграции) и принятый R9.2 server GA evolution
(слот `r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17), а теперь
на принятый R9.3 versioned strategy gallery (слот `r9c-schema18-7afde0d`,
additive миграция PostgreSQL 17→18);
runtime остаётся `public-http-paper` на порту 4180. Accepted evidence:
...
Критерий детерминизма (пройден): golden replay дважды byte-identical,
рестарт в середине цикла даёт идентичный результат, replay совпадает с
итоговым durable state, worst-case capital ни разу не превышен; один price
path даёт одинаковый replay/paper результат, лимиты и правила инструмента
невозможно превысить. Один pre-acceptance дефект (имена полей dca metadata в
read-model) исправлен до приёмки; recovery-хронология сохранила pre-cutover
generation `440523a6` и post-cutover generation `65bb4359` с пройденным
drill.
...
Статус: завершён, принят и развёрнут без миграции (PostgreSQL schema 16 и
trading SQLite schema 9 без изменений) из защищённого слота
`r7a-schema16-baf4217` на коммите
`baf42178d33043fde0965d008aee9f09462df699`; exact-SHA GitHub Actions run
`29636312303` прошёл 6/6 jobs. Production позже перешёл на принятый R8
multi-leg paper intents (слот `r8a-schema16-69621f8`, PostgreSQL schema 16
без изменений, additive миграция trading SQLite 9→10), затем на принятый
R9.1 server multi-market evaluation (слот `r9a-schema16-4f5bc64`, без
миграции) и принятый R9.2 server GA evolution (слот
`r9b-schema17-3ed6af1`, additive миграция PostgreSQL 16→17), а теперь на
принятый R9.3 versioned strategy gallery (слот `r9c-schema18-7afde0d`,
additive миграция PostgreSQL 17→18); runtime остаётся
`public-http-paper` на порту 4180 на трёх systemd units. Accepted evidence:
[R7 grid paper robot](../evidence/R7_GRID_PAPER_ROBOT.md).

Поставлено:

- контракт `grid-params-v1`: arithmetic/geometric лестница уровней (2–50),
  режимы neutral/long/short, outside-range действие pause или stop,
  опциональный stop-loss и cycle cap; цены уровней считает один общий
  детерминированный helper, используемый machine, сервером и UI preview, как
  и worst-case формула `gridLevels · orderQuote · (1 + feePct/100)`;
- pure machine `grid-state-v1`: каждый переход несёт idempotency key
  `grid:<botId>:<epochCycle>:<ordinal>` (он же order clientId), gap batches
  закрываются одним consolidated placement round, journal-deduplicated
  restart никогда не переставляет существующий clientId;
- fill behavior — переиспользованный `averaging-v1` из R6; robot kind
  `grid` аддитивен, legacy create payloads хэшируются идентично;
- worst-case bound enforced на сервере (`WORST_CASE_EXCEEDS_ALLOCATION`) с
  live preview и pre-start списком level-price preview; realized grid PnL и
  evidence-aware inventory PnL разделены; en/ru/kk.

Критерий (пройден): golden replay на реальном adapter/ledger path —
четырёхуровневый gap bar закрыт одним consolidated placement round с
непрерывным набором clientId без дубликатов, поэтому ценовой gap не создаёт
каскад; рестарт в середине цикла дал идентичный набор clientId, события и
terminal state, как непрерывный прогон, поэтому рестарт не дублирует уровни
и резервы; double drive byte-identical, replay совпадает с итоговым durable
state, worst-case capital ни разу не превышен и виден до подтверждения. Один
pre-acceptance дефект (browser read-model parser отклонял отрицательные
short-inventory количества) исправлен до приёмки; recovery-хронология
сохранила pre-cutover generation `0ee96dbe` и post-cutover generation
`cb3702ac` с пройденным drill.

## 12. R8 — торговля спредом и поиск неэффективностей (paper)

Статус: завершён, принят и развёрнут на неизменной PostgreSQL schema 16 и
trading SQLite schema 10 — первая с R4 миграция SQLite, additive
`owner_scoped_paper_multi_leg` 9→10 (SQL SHA-256
`34584a750937468d065d90b0af09a074a541da29ba1e7a38f2c5278cc6e9890d`) — из
защищённого слота `r8a-schema16-69621f8` на коммите
`69621f8107a713031f768320e9dc496010234100`; exact-SHA GitHub Actions run
`29639908389` прошёл 6/6 jobs. Production позже перешёл на принятый R9.1
server multi-market evaluation (слот `r9a-schema16-4f5bc64`, без миграции),
затем на принятый R9.2 server GA evolution (слот `r9b-schema17-3ed6af1`,
additive миграция PostgreSQL 16→17 `ga_evolution_lineage`), а теперь на
принятый R9.3 versioned strategy gallery (слот `r9c-schema18-7afde0d`,
additive миграция PostgreSQL 17→18 `versioned_strategy_gallery`); runtime
остаётся `public-http-paper` на порту 4180 на трёх systemd units.
Accepted evidence:
[R8 multi-leg paper intents](../evidence/R8_MULTI_LEG_PAPER_INTENTS.md).

Поставлено — owner-scoped multi-leg paper intents на общем capital plane:

- fenced executor получил аддитивные command kinds `paper-multi-leg.submit` и
  `paper-multi-leg.kill-switch` (legacy request hashes byte-identical); submit
  строит план через существующие fail-closed research builders
  (research-simulation only, enforced `executable === false`, обязательный
  per-leg evidence), применяет общий freshness gate (возраст источника
  ≤60 с, срок жизни плана ≤5 мин), лимиты активных intents 3 на owner и 2 на
  portfolio и резервирует детерминированный worst case
  `Σ plannedQuantity·referencePrice·(1 + 2·feeBps/10000)` (ceil до шести
  знаков) против available capital портфеля;
- pure deterministic engine доводит intent до terminal внутри fenced apply, и
  каждый переход durable-журналируется под `mleg:<intentId>:<sequence>`; leg
  risk, partial fills и reverse-order compensation (unwind) следуют
  проверенной семантике engine, restart recovery воспроизводит идентичный
  terminal state, а guarded переход running→terminal освобождает единственную
  capital reservation ровно один раз;
- combined paper PnL включает обе ноги и каждый смоделированный расход, а
  residual exposure всегда указывается явно, а не оценивается молча;
- read model портфеля отдаёт browser-shaped секцию `multiLeg` и вычитает
  running reservations из available capital; opportunity research получил
  flow «Run paper multi-leg» с live worst-case preview, portfolio center в
  Robots рисует секцию intents, а owner-level paper kill switch fail-closed;
  en/ru/kk;
- legacy изолированный arbitrage-модуль paperMultiLeg byte-identical: его
  pure engine/schema/builders переиспользованы, а не форкнуты.

Критерий (пройден; baseline нормализации/freshness R8.1 был поставлен ещё до
R8 существующими read-only research builders и переиспользован без
изменений — этот релиз закрыл R8.2): partial-fill/restart replay — частично
прогнанный run, восстановленный на свежем экземпляре сервиса, дал journal
byte-equal непрерывному pure-engine прогону с идентичным terminal state, без
дубликатов sequences и с capital reservation, освобождённой ровно один раз;
combined paper PnL включает обе ноги и все смоделированные расходы с explicit
residual exposure; ни одна возможность не называется исполнимой без
достаточной глубины и свежести за freshness gates (research-simulation only).
Paired migration rehearsal (`scripts/rehearse-trading-migration.mjs`)
мигрировал копию production `trading.db` 9→10 ровно с
`owner_scoped_paper_multi_leg`; recovery-хронология сохранила pre-cutover
generation `ddf80eba` (проверена на SQLite 9) и post-cutover generation
`7ac9a851` с пройденным изолированным drill; rollback остаётся
replacement-only, слот `r7a-schema16-baf4217` сохранён.

## 13. R9 — генератор стратегий и генетический оптимизатор

Статус: завершён — R9.1, R9.2 и R9.3 приняты и развёрнуты, R9 закрыт
целиком. Уже реализованы bounded seeded parameter GA, structural IR
generator, mutation/crossover/deduplication и pure multi-market ranker на
caller-supplied metrics; принятый R9.1 (ниже) добавил контракты D2 и
server-owned candle evaluation с shared capital-pool OOS секцией, принятый
R9.2 (ниже) — server GA evolution с lineage, Pareto/OOS promotion и
checkpoint/resume, а принятый R9.3 (ниже) — versioned strategy gallery с
provenance и безопасным импортом. Remaining delta: нет — только
сопровождение.

### R9.1 — принят: закрытие D2 и server multi-market evaluation

Статус: завершён, принят и развёрнут без миграции на неизменных PostgreSQL
schema 16 и trading SQLite schema 10 из защищённого слота
`r9a-schema16-4f5bc64` на коммите
`4f5bc64e9dfb35d379a55690755a76f7594b226d`; exact-SHA GitHub Actions run
`29643197555` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на порту
4180 на трёх systemd units. Accepted evidence:
[R9.1 server evaluation](../evidence/R9_1_SERVER_EVALUATION.md).

Поставлено:

- решение D2 закрыто
  [ADR 0003](../adr/0003-canonical-ir-dataset-backtest-contract.md)
  (Accepted 2026-07-18) до job API, как требовал roadmap: canonical Strategy
  IR — `IR_VERSION 4` с pinned checksum guard
  `scripts/check-strategy-core-ir.mjs` в `npm run check`, `parseStrategyIR`
  остаётся единственной trust boundary для входящего IR; versioned dataset
  contract `dataset-v1` — canonical serialization с sha256 fingerprint и
  time-ordered train/test split с embargo, без random splits и без
  lookahead, survivorship limitation зафиксирована; детерминированный
  `BACKTEST_ENGINE_VERSION = "backtest-core-v1"` штампуется в каждый
  результат;
- generic job-kind registry: kind-discriminated definitions с in-process и
  worker-thread lanes; существующие screener/backtest перерегистрированы
  byte-identically, unknown kinds жёстко fail;
- job kind `multi-market-eval` — server-owned candle evaluation: только
  реальные закрытые бары публичного провайдера под 90-секундным screener
  budget с explicit fail-closed причинами; 1–6 markets на одном timeframe,
  500–20 000 lookback bars, embargo split; per-market train/OOS backtests
  через существующий worker-thread protocol; OOS-секция общего capital pool
  от R4 portfolio allocator; bounded (≤256 KiB) детерминированный результат
  `multi-market-eval-v1` со штампами dataset fingerprint, engine version и
  seed;
- GeneratorPanel получил flow «Evaluate on server», питающий pure ranker:
  секция ranking переключается из unavailable в ranked с provenance (engine
  version, seed, dataset fingerprint), en/ru/kk.

Критерий релиза (пройден): один и тот же dataset, дважды прогнанный через
полный evaluation path, дал byte-identical result JSON (golden dataset
fingerprint `d076618630cf5842…`); протестированные embargo/no-lookahead
split laws доказывают отсутствие leakage; pre-acceptance fix — infinite
profitFactor, сохранённый JSONB как null, теперь парсится в NaN, и
finite-metrics gate ranker-а закрывает такое окно fail-closed.
Recovery-хронология сохранила pre-cutover generation `92026f70` и
post-cutover generation `e894eede` с пройденным изолированным drill;
rollback остаётся replacement-only, слот `r8a-schema16-69621f8` сохранён.

### R9.2 — принят: server GA evolution с lineage, Pareto и OOS promotion

Статус: завершён, принят и развёрнут на PostgreSQL schema 17 — additive
миграция 17 `ga_evolution_lineage` (SQL SHA-256
`4169ec0148c63415abe913195d34b03fa603039d0fe7defabfe76a89f7a61a73`):
owner-scoped `ga_runs` и `ga_candidates` с single-active-run partial unique
index, bounded checkpoint storage и promotion-requires-OOS CHECK — и
неизменной trading SQLite schema 10 из защищённого слота
`r9b-schema17-3ed6af1` на коммите
`3ed6af138f197ee985bd8ac998ab58cc8769b83c`; exact-SHA GitHub Actions run
`29647276230` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на порту
4180 на трёх systemd units. Accepted evidence:
[R9.2 GA evolution](../evidence/R9_2_GA_EVOLUTION.md).

Поставлено:

- pure structural generator вынесен в workspace-пакет
  `@saltanatbotv2/strategy-generator` (zero IO, guarded generated artifacts,
  frontend re-export shim): сервер выращивает кандидатов из тех же
  примитивов, что и браузер, и эволюционирует только versioned Strategy IR,
  никогда произвольный код;
- job kind `ga-evolution` поверх R9.1 registry и durable owner-fair queue:
  bounded config (1–4 markets на одном timeframe, 500–20 000 lookback bars,
  population 8–64, generations 1–16, seed 0..2³²−1); dataset скачивается
  один раз за run под real-bars дисциплиной и пиннится `dataset-v1`
  fingerprint; train/OOS evaluation дедуплицируется по fingerprint через
  worker-thread backtests; каждое поколение durable сохраняет lineage rows
  (родители, mutation log, IR, метрики, objectives), non-dominated Pareto
  ranks по out-of-sample objectives и atomic checkpoint (population + RNG
  state); cancel даёт durable resumable `checkpointed` run;
- promotion отклоняет кандидата без clean out-of-sample report на обоих
  уровнях — repository и SQL CHECK — и отклоняет flagged overfit кандидатов
  (`ga_promotion_overfit`); clean кандидат промоутится в собственную
  библиотеку стратегий владельца с полным provenance (seed, dataset
  fingerprint, версии engine и generator, lineage chain, OOS report);
  публичная галерея остаётся запрещённой до R9.3;
- owner-scoped маршруты `/api/ga` отдают runs, frontier, lineage chains
  кандидатов и promotion; секция server evolution в strategy studio
  (start/cancel/resume, frontier с explicit overfit/unstable флагами,
  lineage drawer, promote-to-library) поставлена en/ru/kk; reviewed
  total-JS budget сдвинут 1 008 → 1 032 KiB по документированному
  cap-паттерну.

Критерий релиза — seeded reproducibility (пройден): два идентичных запуска
(одинаковые seed и dataset) дали byte-identical результат и row-identical
lineage; run, отменённый после generation 1, возобновился на свежем
экземпляре до byte-identical финального состояния с равными наборами
candidate rows и нулевой повторной оценкой через checkpoint boundary;
resume, чей повторно скачанный market data больше не воспроизводит
запиненный dataset fingerprint, явно завершается `ga_dataset_drift`;
fingerprint dedup гарантирует, что genome никогда не оценивается дважды.
Paired rehearsal мигрировал восстановленную копию production-данных 16→17
ровно с `ga_evolution_lineage`; recovery-хронология сохранила проверенные
schema-16 generations `c9fbff05` (pre-migration) и `35d3b199` (pre-cutover
на green SHA) и post-cutover schema-17 generation `fb95a706` с пройденным
изолированным drill; rollback остаётся replacement-only, слот
`r9a-schema16-4f5bc64` сохранён.
...
- решение D2 закрыто до schema/job API R9.1 и первого evolutionary run, как
  требовалось: canonical Strategy IR, versioned dataset contract и
  воспроизводимый backtest engine зафиксированы в
  [ADR 0003](../adr/0003-canonical-ir-dataset-backtest-contract.md).
...
### R9.3 — принят: versioned strategy gallery с provenance и безопасным импортом

Статус: завершён, принят и развёрнут на PostgreSQL schema 18 — additive
миграция 18 `versioned_strategy_gallery` (SQL SHA-256
`421a9b93d41c7618c8f30736fae0a45cfe37a14a3e418afc6aa6492696322512`):
content-frozen таблица `gallery_artifacts`, чей BEFORE UPDATE trigger
отклоняет любое изменение контента — единственные разрешённые мутации это
revocation и visibility — и неизменной trading SQLite schema 10 из
защищённого слота `r9c-schema18-7afde0d` на коммите
`7afde0d12b350babb01e166a1888d54c225d41ec`; exact-SHA GitHub Actions run
`29652078335` прошёл 6/6 jobs, runtime остаётся `public-http-paper` на
порту 4180 на трёх systemd units. Accepted evidence:
[R9.3 strategy gallery](../evidence/R9_3_STRATEGY_GALLERY.md).

Поставлено:

- публикация — только явное действие владельца, создающее immutable
  versioned artifact `gallery-artifact-v1` из library-артефакта или
  промоутнутого GA-кандидата (требует OOS-gated promotion R9.2); повторная
  публикация того же artifact id добавляет версию N+1 и никогда не
  переписывает предыдущие; диалог публикации показывает точный sanitized
  canonical JSON, который будет сохранён, и хэширует его только после
  явного consent;
- whitelisting sanitizer гарантирует, что owner/run/workspace
  идентификаторы никогда не попадают в сохранённый artifact (adversarial
  fixtures вплоть до вложенных ключей метрик, значений и имени IR;
  обнаруженная утечка прерывает публикацию), и ни одна API-проекция не
  сериализует owner id;
- canonical-JSON SHA-256 вычисляется идентично на сервере и клиенте: сервер
  повторно проверяет сохранённый hash перед выдачей импорта, клиент
  проверяет его ещё раз; подделанный bundle отклоняется
  `gallery_hash_mismatch`, копия не создаётся;
- карточки несут полный provenance: markets/timeframes, engine/dataset
  fingerprints, seed, in-sample vs out-of-sample сводки с gap и
  overfit/unstable флагами, complexity, дата публикации и ограничения;
  рейтинг — documented composite (OOS stability 0.35, drawdown 0.25,
  reproducibility 0.20, complexity 0.10, evidence age 0.10) и никогда не
  строится только по net profit;
- импорт создаёт независимую library-копию с provenance `gallery` и
  revalidation gate: paper start (создание робота и экспериментальная
  trading-панель) заблокирован до завершения локального validation
  backtest; импорт отозванного artifact отклоняется (410), а публикация
  никогда не запускает робота;
- moderation: private/unlisted/public visibility и owner revoke-with-reason;
  revoke никогда не переписывает историю;
- шесть маршрутов `/api/gallery` (всего 176 HTTP endpoints) и Community
  gallery UI поставлены en/ru/kk; visual snapshot strategy-studio обновлён
  для новой кнопки Community.

Критерий релиза — privacy + reproducible artifact (пройден): adversarial
privacy fixtures ни разу не сериализованы в сохранённый artifact; hash
parity golden против `node:crypto`; end-to-end tamper-симуляция отклонена в
браузерном journey; SQL trigger отклонил все девять UPDATE
immutable-колонок; PG-gated integration suite `galleryPostgres` выполнена
на изолированном PostgreSQL 16 (31/31 вместе с compute-jobs и GA suites) и
подключена в CI — опубликованный artifact воспроизводится по
зафиксированным версиям, не раскрывает tenant-owned данные и не может
незаметно измениться после импорта. Paired rehearsal мигрировал
восстановленную копию production-данных
(`saltanatbotv2_restore_r9c_rehearsal`) 17→18 ровно с
`versioned_strategy_gallery`; recovery-хронология сохранила проверенную
schema-17 pre-cutover generation `pre-r9c-schema17-v10-20260718T171500Z`
(`38c116d0-2cbc-43f6-920c-3ad9842f0ad1`) и post-cutover schema-18
generation `post-r9c-schema18-v10-20260718T164500Z`
(`63c4b01f-3018-42a2-b271-3bc2037433ed`) с пройденным изолированным drill;
rollback остаётся replacement-only, слот `r9b-schema17-3ed6af1` и
schema-17 pre-cutover generation сохранены.
...
scheduler admission 4/16/8 и retention 2/30 дней, принятый R5.2.1 добавляет
caps 40 presets на owner/400 глобально, universe ≤200 и bounded compute-job
runs, принятый R5.3a — caps 5 screener-правил на owner/40 глобально, а
принятый R5.3b-1 — отдельный bounded notification worker с delivery,
ingress-rate и binding-attempt caps; они приняты в production, но не имеют
integrated 100-user evidence. R11 остаётся pending и не может быть заменён
размером текущего сервера или unit/integration тестами.
...
- измерить и настроить уже реализованный process-wide API admission, проверить
  принятые R5.1 alert, R5.2.1 screener-preset и R5.3b-1 delivery caps под
  общей нагрузкой, затем добавить недостающие global caps для WebSocket,
  robots, jobs и L2;
...
| R5 | R5.1 alerts, R5.2.1 screener MVP, R5.3a screen→alert promotion, R5.3b-1 Telegram delivery/binding, R5.3b-2 Telegram-команды и chart research tools (текстовые заметки и parallel channel) приняты | R3–R4 | завершено | at-least-once outbox drill, R5.3b-2 command drill и chart-tools geometry/recovery gates пройдены |
| R6 | common paper engine completion + DCA приняты | R4–R5 | завершено | deterministic DCA replay пройден (golden replay + mid-cycle restart + worst-case capital) |
| R7 | Grid на общем paper engine принят | R4–R6 | завершено | gap/restart/capital proof пройден (consolidated gap round + идентичный clientId set после рестарта + worst-case preview до подтверждения) |
| R8 | owner-scoped multi-leg paper intents на общем capital plane приняты | R4–R5 | завершено | partial-fill/restart replay пройден (byte-equal journal после рестарта + reservation released ровно один раз + both-legs-all-costs PnL с explicit residual exposure) |
| R9 | R9.1 (D2 ADR + job registry + server multi-market evaluation), R9.2 (server GA evolution: lineage, Pareto/OOS promotion, checkpoint/resume) и R9.3 (versioned strategy gallery: provenance, безопасный импорт, revoke) приняты | R1 + R4 portfolio metrics + canonical IR/dataset/backtest (D2 закрыт ADR 0003) | завершено | seeded OOS/Pareto promotion пройден (byte-identical double run + byte-identical resume); R9.3 privacy + reproducible artifact пройден (sanitizer + hash parity + tamper refusal + immutable trigger) |
...
строго последовательный от следующего pending инкремента: R10A →
R10B → R11 →
...
| R5.2 | R5.2.1 done: технический скринер MVP на canonical indicator engine принят и развёрнут на schema 14; screen→alert promotion закрыт принятым R5.3a | chart/screener parity пройден |
| R5.3 | done: R5.3a screen→alert promotion (rule kind `screener`) принят и развёрнут без миграции; R5.3b-1 notification worker, Telegram delivery и binding/revoke приняты и развёрнуты на schema 15; R5.3b-2 paper-only команды с one-time confirmations через fenced executor приняты и развёрнуты на schema 16 | [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md), [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md) и [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md) зафиксированы |
| R5 chart tools | done: текстовые заметки и parallel channel на canonical geometry contract `packages/contracts/chartGeometry` приняты и развёрнуты без миграции (schema 16 без изменений, workspace schema v9 аддитивна) | [R5 chart tools evidence](../evidence/R5_CHART_RESEARCH_TOOLS.md) зафиксирован |
| R6 | done: общий paper execution contract, DCA paper state machine, versioned fill behaviors и worst-case capital preview приняты и развёрнуты без миграции | deterministic golden replay пройден; [R6 evidence](../evidence/R6_DCA_PAPER_ROBOT.md) зафиксирован |
| R7 | done: Grid paper state machine `grid-state-v1` на контракте `grid-params-v1`, consolidated gap handling, journal-deduplicated restart и worst-case preview приняты и развёрнуты без миграции | capital/no-duplicate proof пройден; [R7 evidence](../evidence/R7_GRID_PAPER_ROBOT.md) зафиксирован |
| R8.1 | done: единая spread normalization, freshness, depth и economics были поставлены fail-closed read-only research builders ещё до R8 и переиспользованы принятым R8 submit pipeline без форка | rejected-reason/reference fixtures пройдены |
| R8.2 | done: owner-scoped multi-leg paper intents, leg risk и unwind приняты и развёрнуты с additive миграцией trading SQLite 9→10 | partial-fill/restart replay пройден; [R8 evidence](../evidence/R8_MULTI_LEG_PAPER_INTENTS.md) зафиксирован |
| R9.1 | done: D2 закрыт [ADR 0003](../adr/0003-canonical-ir-dataset-backtest-contract.md), generic research job registry и server multi-market evaluation `multi-market-eval` приняты и развёрнуты без миграции | byte-identical double run + embargo/no-lookahead laws пройдены; [R9.1 evidence](../evidence/R9_1_SERVER_EVALUATION.md) зафиксирован |
| R9.2 | done: `@saltanatbotv2/strategy-generator`, job kind `ga-evolution` с per-generation lineage, Pareto ranks на OOS objectives и atomic checkpoint/resume, promotion с обязательным clean OOS report и full provenance, owner-scoped `/api/ga` и server evolution секция studio приняты и развёрнуты с additive миграцией PostgreSQL 16→17 `ga_evolution_lineage` | seeded reproducibility пройден (byte-identical double run + byte-identical checkpoint/resume + explicit `ga_dataset_drift`); [R9.2 evidence](../evidence/R9_2_GA_EVOLUTION.md) зафиксирован |
| R9.3 | done: versioned strategy gallery (immutable `gallery-artifact-v1` с whitelisting sanitizer, server+client проверкой canonical-JSON SHA-256, полным provenance/composite rating, revalidation-gated import, visibility и owner revoke-with-reason) принят и развёрнут с additive миграцией PostgreSQL 17→18 `versioned_strategy_gallery` | privacy + reproducible artifact пройден; [R9.3 evidence](../evidence/R9_3_STRATEGY_GALLERY.md) зафиксирован |
...
2. Выполнено: R5.1 проверен по schema-13 upgrade, owner isolation,
   ...
   [R5.1 evidence](../evidence/R5_1_OWNER_ALERTS.md).
3. Выполнено: R5.2.1 technical screener MVP прошёл schema-14 upgrade,
   chart/screener parity, изолированный rehearsal и recovery gates, принят и
   развёрнут; exact-SHA acceptance/cutover evidence зафиксировано в
   [R5.2.1 evidence](../evidence/R5_2_1_TECHNICAL_SCREENER.md).
4. Выполнено: R5.3a screen→alert promotion прошёл exact-worktree,
   real-PostgreSQL, browser и recovery gates, принят и развёрнут без
   миграции; exact-SHA acceptance/cutover evidence зафиксировано в
   [R5.3a evidence](../evidence/R5_3A_SCREENER_ALERTS.md).
5. Выполнено: R5.3b-1 Telegram delivery worker и chat binding прошли
   schema-15 migration, idle-boot, binding-code, delivery-lane и recovery
   gates, приняты и развёрнуты; exact-SHA acceptance/cutover evidence
   зафиксировано в
   [R5.3b-1 evidence](../evidence/R5_3B1_TELEGRAM_DELIVERY.md).
6. Выполнено: R5.3b-2 Telegram read и paper-control команды с one-time
   confirmations через fenced executor прошли schema-16 migration,
   real-fenced-executor command, confirmation-token и recovery gates, приняты
   и развёрнуты; exact-SHA acceptance/cutover evidence зафиксировано в
   [R5.3b-2 evidence](../evidence/R5_3B2_TELEGRAM_COMMANDS.md).
7. Выполнено: chart research tools (текстовые заметки и parallel channel) на
   общем geometry contract прошли no-migration, browser, accessibility и
   recovery gates, приняты и развёрнуты; exact-SHA acceptance/cutover
   evidence зафиксировано в
   [R5 chart tools evidence](../evidence/R5_CHART_RESEARCH_TOOLS.md), полный
   R5 release gate закрыт.
8. Выполнено: R6 (общий paper execution contract и DCA-робот) прошёл
   golden-replay determinism, worst-case capital, restart и recovery gates,
   принят и развёрнут без миграции; exact-SHA acceptance/cutover evidence
   зафиксировано в [R6 evidence](../evidence/R6_DCA_PAPER_ROBOT.md).
9. Выполнено: R7 (Grid paper-робот на общем execution contract) прошёл
   consolidated-gap, restart-no-duplicate, worst-case capital и recovery
   gates, принят и развёрнут без миграции; exact-SHA acceptance/cutover
   evidence зафиксировано в
   [R7 evidence](../evidence/R7_GRID_PAPER_ROBOT.md).
10. Выполнено: R8 (owner-scoped multi-leg paper intents на общем capital
    plane) прошёл restart-replay, worst-case reservation, freshness и
    recovery gates, принят и развёрнут с additive миграцией trading SQLite
    9→10; exact-SHA acceptance/cutover evidence зафиксировано в
    [R8 evidence](../evidence/R8_MULTI_LEG_PAPER_INTENTS.md).
11. Выполнено: R9.1 (закрытие D2 через
    [ADR 0003](../adr/0003-canonical-ir-dataset-backtest-contract.md) до job
    API, generic research job registry и server multi-market evaluation)
    прошёл byte-identical determinism, embargo/no-lookahead и recovery
    gates, принят и развёрнут без миграции; exact-SHA acceptance/cutover
    evidence зафиксировано в
    [R9.1 evidence](../evidence/R9_1_SERVER_EVALUATION.md).
12. Выполнено: R9.2 (server GA evolution — lineage, Pareto/OOS promotion и
    checkpoint/resume поверх R9.1 registry) прошёл seeded byte-identical
    reproducibility, checkpoint/resume, dataset-drift, promotion-gate и
    recovery gates, принят и развёрнут с additive миграцией PostgreSQL
    16→17 `ga_evolution_lineage`; exact-SHA acceptance/cutover evidence
    зафиксировано в [R9.2 evidence](../evidence/R9_2_GA_EVOLUTION.md).
13. Выполнено: R9.3 (versioned strategy gallery с provenance, safe import и
    revoke) прошёл privacy/sanitizer, hash-parity/tamper-refusal,
    immutable-trigger и recovery gates, принят и развёрнут с additive
    миграцией PostgreSQL 17→18 `versioned_strategy_gallery`; exact-SHA
    acceptance/cutover evidence зафиксировано в
    [R9.3 evidence](../evidence/R9_3_STRATEGY_GALLERY.md); R9 закрыт
    целиком.
14. Следующий pending инкремент — R10A (funding/OI/MTF контракты и bounded
    L2 collector/storage/quality корпус); R10A ещё не начат; не допускать
    код более поздних релизов в `main` или production до его полной
    приёмки.
...
| --- | --- |
| Алерты умирают вместе с вкладкой; мало типов условий | Generic `price-threshold` закрыт принятым и развёрнутым R5.1, screener alerts — принятым и развёрнутым R5.3a, notification worker и Telegram delivery — принятым и развёрнутым R5.3b-1, Telegram-команды — принятым и развёрнутым R5.3b-2. Indicator/drawing alerts остаются отложенным R5.2 scope |
| Нет DCA и Grid | DCA закрыт принятым и развёрнутым R6, Grid — принятым и развёрнутым R7, оба поверх общего R4 paper ledger |
...
| Нет обычного технического скринера | Закрыто принятым и развёрнутым R5.2.1 technical screener MVP |
| Нет текста/заметок и parallel channel | Закрыто принятым и развёрнутым релизом R5 chart research tools на общем geometry contract |
| Шаринг стратегий односторонний | Закрыт принятым и развёрнутым R9.3: versioned gallery и безопасный import |
