...
Текущий production — принятый релиз R5 chart research tools (текстовые
заметки и parallel channel) на PostgreSQL schema 16 из защищённого слота
`r5f-schema16-2ff6101` на коммите
`2ff6101b950b42a77c378233dabecf1a5ee76ce7`. Описанные ниже R5.1, R5.2.1,
R5.3a, R5.3b-1, R5.3b-2 и chart research tools приняты и развёрнуты; R5 в
целом завершён, следующий pending инкремент — R6.
...
R5.1 schema 13, R5.2.1 schema 14, R5.3a schema 14, R5.3b-1 schema 15 и
R5.3b-2 schema 16, а теперь на принятый релиз R5 chart research tools на
неизменной schema 16. Baseline: PostgreSQL auth,
...
schema 13, R5.2.1 schema 14, R5.3a schema 14, R5.3b-1 schema 15 и R5.3b-2
schema 16, а теперь на принятый релиз R5 chart research tools (слот
`r5f-schema16-2ff6101`, schema 16 без изменений). Runtime остаётся
...
приняты и развёрнуты; production работает на PostgreSQL schema 16 и
неизменной trading SQLite schema 9 из защищённого слота
`r5f-schema16-2ff6101` на коммите
...
строго последовательный от следующего pending инкремента: R6 → R7 → R8 →
R9 → R10A → R10B → R11 →
...
8. Следующий pending инкремент — R6 (общий paper execution contract и
   DCA-робот); не допускать код более поздних релизов в `main` или
   production до полной приёмки R6.
