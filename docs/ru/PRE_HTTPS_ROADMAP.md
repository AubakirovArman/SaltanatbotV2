...
Текущий production — принятый релиз R9.1 (ADR решения D2, generic research
job registry и server multi-market evaluation) на неизменных PostgreSQL
schema 16 и trading SQLite schema 10 из защищённого слота
`r9a-schema16-4f5bc64` на коммите
`4f5bc64e9dfb35d379a55690755a76f7594b226d` без миграции; accepted evidence —
...
R6, R7, R8 и R9.1 приняты и развёрнуты; R5, R6, R7 и R8 завершены, R9
продолжается, следующий pending инкремент — R9.2.
...
Grid paper-робот и R8 multi-leg paper intents, а теперь на принятый R9.1
server multi-market evaluation на schema 16 и trading SQLite schema 10.
...
`r7a-schema16-baf4217`) и R8 multi-leg paper intents (слот
`r8a-schema16-69621f8`, additive миграция trading SQLite 9→10), а теперь на
принятый R9.1 server multi-market evaluation (слот `r9a-schema16-4f5bc64`,
schema 16 и SQLite 10 без изменений). Runtime
...
на принятый R7 Grid paper-робот (слот `r7a-schema16-baf4217`) и принятый R8
multi-leg paper intents (слот `r8a-schema16-69621f8`, additive миграция
trading SQLite 9→10), а теперь на принятый R9.1 server multi-market
evaluation (слот `r9a-schema16-4f5bc64`, schema 16 и SQLite 10 без
изменений).
...
9→10), а теперь на принятый R9.1 server multi-market evaluation (слот
`r9a-schema16-4f5bc64`, без миграции); runtime
остаётся `public-http-paper` на порту 4180. Accepted evidence:
...
без изменений, additive миграция trading SQLite 9→10), а затем на принятый
R9.1 server multi-market evaluation (слот `r9a-schema16-4f5bc64`, без
миграции); runtime остаётся
...
runtime остаётся `public-http-paper` на порту 4180 на трёх systemd units.
Accepted evidence:
...
Статус: active — R9.1 принят и развёрнут, R9 в целом продолжается. Уже
реализованы bounded seeded parameter GA, structural IR generator,
mutation/crossover/deduplication и pure multi-market ranker на
caller-supplied metrics, а принятый R9.1 (ниже) добавил контракты D2 и
server-owned candle evaluation с shared capital-pool OOS секцией. Remaining
delta: GA/generator lineage storage, Pareto/OOS promotion и checkpoint/resume
(R9.2), затем strategy gallery (R9.3).
...
| R9 | R9.1 (D2 ADR + job registry + server multi-market evaluation) принят; остаются R9.2 lineage/Pareto/OOS/checkpoints и R9.3 gallery | R1 + R4 portfolio metrics + canonical IR/dataset/backtest (D2 закрыт ADR 0003) | 5–8 | seeded OOS/Pareto promotion |
...
строго последовательный от следующего pending инкремента: R9.2 →
R9.3 → R10A → R10B → R11 →
...
12. Следующий pending инкремент — R9.2 (GA/generator lineage, Pareto/OOS
    promotion и checkpoint/resume внутри ещё не закрытого R9); не допускать
    код более поздних релизов в `main` или production до полной приёмки
    R9.2.
