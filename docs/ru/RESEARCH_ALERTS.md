...
Production теперь работает на принятом и развёрнутом релизе R5.1 на
PostgreSQL schema 13, защищённый слот r5a-schema13-66394fd
([evidence](../evidence/R5_1_OWNER_ALERTS.md)). Контур без producer-ов из
этого документа по-прежнему не считается принятым или развёрнутым.
...
семантику publish-before-checkpoint. Release gates — browser-closed
restart/dedup, same-owner multi-tab convergence, local-storage failure и
desktop/mobile accessibility/visual — пройдены перед production cutover. R5.2
technical screener, R5.3 notification worker/Telegram и integrated 100-user
proof R11 остаются pending и не доказаны.
...
статус принятого generic релиза R5.1.
