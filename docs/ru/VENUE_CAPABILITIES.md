...
| Приватно, экспериментально | Аутентифицированный код есть, но по умолчанию разоружён и не готов к mainnet |
...
| Приватная торговля | Paper и экспериментальные пути Binance/Bybit; все общие публичные адаптеры объявляют private execution равным false |
...
| --- | --- | --- | --- | --- |
| Binance | Нативный реестр, свечи графика, Spot и derivative top-book/depth/funding | Есть специализированные потоки сканера, но Binance не входит в общий continuous-public-feed | Текущий Binance↔Bybit и внутрибиржевой basis, выбранное треугольное исследование по top book | USDⓈ-M execution экспериментален. Live Spot выключен до появления аутентифицированного учёта Spot-исполнений. Inverse execution не поддерживается |
| Bybit | Нативный реестр, свечи, Spot/linear/inverse public data и нативный spread book | Есть специализированные snapshot/delta книги, но Bybit не входит в общий continuous-public-feed | Текущие basis, triangular и native-spread исследования | Spot и USDT-linear экспериментальны. UTA collateral/borrow требуют явного opt-in. Заявления о mainnet-readiness нет |
...
| Binance USDⓈ-M | Экспериментально | Signed REST, private order updates и reconciliation есть; по умолчанию выключены |
| Binance inverse | Не поддерживается | Order path отсутствует |
| Bybit Spot | Экспериментально | Нужны `ENABLE_LIVE_SPOT`, attributed inventory и private v5 accounting |
| Bybit USDT linear | Экспериментально | Signed v5 lifecycle/reconciliation есть; по умолчанию выключены |
| Bybit UTA cross collateral/manual debt | Явный opt-in | Borrow, repay и collateral mutations — защищённые действия оператора, не автоматизация сканера |
