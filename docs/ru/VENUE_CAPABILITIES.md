...
| --- | --- |
| Графики | Публичные свечи Binance/Bybit и last-trade perpetual-свечи first DEX Hyperliquid; ограничения market type и price source указаны явно |
...
| Hyperliquid | Зарегистрированы public HyperCore `/info`, L2/funding, история perpetual-свечей first DEX и public candle/L2/trade streams | Каждый `l2Book` — атомарный block snapshot без protocol sequence/checksum. Это только continuous research, книга исключена из route-ready | Общий continuous research, ordinary first-DEX perpetual selector графика и опциональный market-data source paper-робота | Нет wallet, address, signing, `/exchange`, HyperEVM и order-кода |
