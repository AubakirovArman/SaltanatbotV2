...
const instruments = [instrument("BTCUSDT", "Bitcoin / Tether", 100, 2), instrument("ETHUSDT", "Ethereum / Tether", 80, 2), instrument("SOLUSDT", "Solana / Tether", 60, 2), instrument("BNBUSDT", "BNB / Tether", 40, 2), instrument("EURUSD", "Euro / US Dollar", 1.08, 5, "forex")];
    ...
    route.fulfill({
      ...
      body: JSON.stringify({
        ...
        series: Object.fromEntries(
          instruments.map(({ symbol, basePrice }, index) => [
            symbol,
            {
...
async function capture(page: Page, name: string) {
  ...
  await expect(page).toHaveScreenshot(name, {
    ...
    maxDiffPixelRatio: 0.002,
    maskColor: "#314151",
    mask: [page.locator(".status-pill"), page.locator(".pane-feed"), page.locator(".current-price-pill span"), page.locator(".artifact-version-panel > summary"), page.locator(".ir-note")]
  });
}
...
async function waitForCanvasPaint(page: Page, expectedCount: number) {
  ...
  await expect
    .poll(
      () =>
        canvases.evaluateAll((items: HTMLCanvasElement[]) =>
          items.every((canvas) => {
