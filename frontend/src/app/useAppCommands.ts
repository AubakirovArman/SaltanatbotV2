...
export function useAppCommands(options: UseAppCommandsOptions) {
  ...
  const [shortcuts, setShortcuts] = useState<ShortcutMap>(() => loadShortcuts(options.storageOwnerId));
  useEffect(() => saveShortcuts(shortcuts, options.storageOwnerId), [options.storageOwnerId, shortcuts]);
  const commands = useMemo<Command[]>(() => {
    const list: Command[] = [];
    for (const item of options.catalog?.instruments ?? [])
      list.push({
        id: `sym-${item.symbol}`,
        ...
        hint: item.exchange,
        run: () => {
          options.setSymbol(item.symbol);
          options.setMode("chart");
        }
      });
    (options.catalog?.timeframes ?? []).forEach((timeframe, index) =>
      list.push({
        id: `tf-${timeframe}`,
        group: shellText(locale, "timeframe"),
        label: timeframe,
        hint: `${shellText(locale, "key")} ${index + 1}`,
        run: () => options.setTimeframe(timeframe)
      })
    );
    for (const chartType of options.catalog?.chartTypes ?? [])
      list.push({
        id: `ct-${chartType}`,
        group: shellText(locale, "chartType"),
        label: chartTypeLabel(locale, chartType),
        run: () => {
    ...
    list.push({
      id: "view-strategy",
      group: shellText(locale, "view"),
      label: shellText(locale, "openStrategy"),
      run: () => {
        warmStrategyLab();
        options.setMode("strategy");
      }
    });
    list.push({
      id: "view-trade",
      group: shellText(locale, "view"),
      label: shellText(locale, "openTrading"),
      run: () => {
        warmTradingView();
        options.setMode("trade");
      }
    });
    list.push({
      id: "view-screener",
      group: shellText(locale, "view"),
      label: shellText(locale, "openScreener"),
      run: () => {
        warmArbitrageScreener();
        options.setMode("screener");
      }
    });
  ...
  return {
    ...
    openShortcutSettings: () => setShortcutSettingsOpen(true),
    closeShortcutSettings: () => setShortcutSettingsOpen(false),
    shortcuts,
    setShortcuts
  };
}
