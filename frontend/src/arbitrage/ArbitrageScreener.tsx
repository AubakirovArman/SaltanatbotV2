...
import { loadFeeProfile, netEdgeBps, routeCostBps, routeCostBreakdown, storeFeeProfile } from "./fees";
...
export function ArbitrageScreener({ locale, onOpenChart }: Props) {
  ...
  const opportunities = useMemo(() => {
    ...
    return (scan?.opportunities ?? []).filter((row) => (!query || row.symbol.includes(query)) && netEdgeBps(row, profile, notionalUsd) >= minEdge && row.topBookCapacityUsd >= minCapacity).sort((left, right) => netEdgeBps(right, profile, notionalUsd) - netEdgeBps(left, profile, notionalUsd));
  }, [minCapacity, minEdge, notionalUsd, profile, scan?.opportunities, search]);
  ...
  useEffect(() => {
    ...
    const current = new Set(scan.opportunities.filter((row) => netEdgeBps(row, profile, notionalUsd) >= alertConfig.thresholdBps && row.topBookCapacityUsd >= minCapacity).map((row) => row.id));
    ...
    for (const row of fired.slice(0, 3)) {
      const edge = netEdgeBps(row, profile, notionalUsd);
  ...
  const analyzeDepth = useCallback(
    async (row: ArbitrageOpportunity) => {
      ...
      try {
        ...
        setDepth({ routeId: row.id, loading: false, error: cause instanceof Error ? cause.message : "Order-book depth unavailable" });
      }
    },
    [notionalUsd]
  );
  ...
  const best = opportunities[0] ? netEdgeBps(opportunities[0], profile, notionalUsd) : undefined;
    ...
    <section className="arb-screener" aria-labelledby="arb-title">
      <header className="arb-hero">
        ...
        <div className="arb-hero-actions">
          ...
          <button type="button" className="arb-refresh" onClick={() => void refresh()}>
      ...
      {scan && (
        <div className="arb-source-row" aria-label={arbitrageText(locale, "sourceHealth")}>
          {scan.sources.map((source) => (
            <span key={`${source.exchange}-${source.market}`} className={source.ok ? "ok" : "error"} title={source.message}>
      ...
      <ArbitrageTable
        locale={locale}
        rows={opportunities}
        costs={(row) => routeCostBps(row, profile, notionalUsd)}
        breakdown={(row) => routeCostBreakdown(row, profile, notionalUsd)}
        net={(row) => netEdgeBps(row, profile, notionalUsd)}
        depth={depth}
