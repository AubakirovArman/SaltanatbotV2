...
export interface ArbitragePaperPosition {
  id: string; routeId: string; symbol: string; spotExchange: "binance" | "bybit"; futuresExchange: "binance" | "bybit";
  notionalUsd: number; spotQuantity: number; futuresQuantity: number; spotEntry: number; futuresEntry: number; openedAt: number;
  estimatedRoundTripCostUsd: number; closedAt?: number; realizedPnlUsd?: number;
}
...
export function openPaperPosition(row: ArbitrageOpportunity, depth: ArbitrageDepthResponse, profile: ArbitrageFeeProfile, now = Date.now()): ArbitragePaperPosition {
  ...
  return {
    id: `arb-paper-${now.toString(36)}-${Math.random().toString(36).slice(2, 7)}`, routeId: row.id, symbol: row.symbol,
    spotExchange: row.spotExchange, futuresExchange: row.futuresExchange, notionalUsd: depth.requestedNotionalUsd,
    spotQuantity: depth.spot.quantity, futuresQuantity: depth.perpetual.quantity,
    spotEntry: depth.spot.averagePrice, futuresEntry: depth.perpetual.averagePrice, openedAt: now,
...
export function paperPnl(position: ArbitragePaperPosition, quote: ArbitrageOpportunity | undefined): number | undefined {
  ...
  return position.spotQuantity * (quote.spotBid - position.spotEntry)
    + position.futuresQuantity * (position.futuresEntry - quote.futuresAsk)
    - position.estimatedRoundTripCostUsd;
}
...
export function loadPaperPositions(): ArbitragePaperPosition[] {
  try {
    ...
    return value.slice(0, 100).filter((row) => row && typeof row.id === "string" && Number.isFinite(row.notionalUsd)).map((row) => ({
      ...row,
...
export function storePaperPositions(positions: ArbitragePaperPosition[]) { localStorage.setItem(KEY, JSON.stringify(positions.slice(0, 100))); }
