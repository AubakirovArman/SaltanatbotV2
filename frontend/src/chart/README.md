...
- `priceRepresentationSettings.ts` and `components/chartCanvas/PriceRepresentationControl.tsx`: clamped/bounded pane+symbol persistence, exact-scope tab synchronization, primary-only legacy migration and a native accessible construction control for Renko/Kagi/P&F percentages, Line Break depth and P&F reversal boxes.
...
- Price-representation construction settings share across timeframes only within the same stable chart id + symbol. Sibling panes never consume each other's same-tab events, even when their chart types are linked.
...
- `canvasDensity.ts` owns the HiDPI contract: each backing store is at least CSS size × DPR, every 2D context is transformed back to logical CSS pixels, and render plans, pointer events, DOM HUDs, depth and footprint must never mix physical pixels into viewport geometry.
