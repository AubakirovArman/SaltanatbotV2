...
export function DrawingObjectsPanel({ locale, drawings, selectedId, canUndo, canRedo, onSelect, onToggleHidden, onToggleLocked, onDelete, onApplyTemplate, onUndo, onRedo, onClose }: DrawingObjectsPanelProps) {
  const t = (key: Parameters<typeof shellText>[1]) => shellText(locale, key);
  const [templates, setTemplates] = useState(loadDrawingTemplates);
  ...
  const saveTemplate = () => {
    ...
    if (!name) return;
    setTemplates(saveDrawingTemplate({ id: `drawing-template-${Date.now()}`, name, tool: selected.tool, style: { ...selected.style }, createdAt: Date.now() }));
  };
  return (
    <aside className="drawing-objects-panel" aria-label={t("drawingObjects")}>
      <header>
        ...
        <div>
          <button type="button" disabled={!canUndo} onClick={onUndo} aria-label={t("undoDrawing")} title={t("undoDrawing")}><Undo2 size={13} aria-hidden="true" /></button>
          <button type="button" disabled={!canRedo} onClick={onRedo} aria-label={t("redoDrawing")} title={t("redoDrawing")}><Redo2 size={13} aria-hidden="true" /></button>
          <button type="button" disabled={!selected} onClick={saveTemplate} aria-label={t("saveDrawingTemplate")} title={t("saveDrawingTemplate")}><Save size={13} aria-hidden="true" /></button>
          <button type="button" onClick={onClose} aria-label={t("closeDrawingObjects")}><X size={13} aria-hidden="true" /></button>
        </div>
      </header>
      <ul className="drawing-object-list">
        {drawings.map((drawing, index) => (
          <li key={drawing.id} className={drawing.id === selectedId ? "active" : ""}>
            ...
            <button type="button" onClick={() => onDelete(drawing.id)} aria-label={t("deleteDrawing")}><Trash2 size={12} aria-hidden="true" /></button>
          </li>
        ))}
      ...
      {templates.length > 0 && (
        <section className="drawing-template-list" aria-label={t("drawingTemplates")}>
          ...
          {templates.map((template) => (
            <div key={template.id}>
              <button type="button" disabled={!selected || selected.tool !== template.tool} onClick={() => onApplyTemplate(template)}>{template.name} · {template.tool}</button>
              <button type="button" aria-label={`${t("remove")} ${template.name}`} onClick={() => setTemplates(removeDrawingTemplate(template.id))}><X size={11} aria-hidden="true" /></button>
            </div>
          ))}
        </section>
      )}
