...
- `useChartNavigation.ts` contains wheel/trackpad normalization and data-anchored two-finger touch pinch/pan; native pointer containment is scoped to the interaction Canvas and hands a remaining finger back to ordinary pan. Pinch state updaters must capture an immutable per-frame gesture before scheduling React state because coalesced mobile `pointerup` can clear the live gesture first. Keep gesture math pure so browser injection and unit invariants cover the same model.
