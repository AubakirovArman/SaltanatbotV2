...
const en = {
  ...
  objective: "Objective", trainPercent: "Train %", walkForwardShort: "Walk-fwd", folds: "Folds", runOptimizer: "Run optimizer", optimizing: "Optimizing…",
  ...
  loadPineFiles: "Load .pine file(s)", pastedScript: "Pasted script", convert: "Convert", artifact: "artifact", artifacts: "artifacts", add: "Add", converted: "converted", rejected: "rejected", approximations: "Approximations — review before trusting with money:", someFilesUnreadable: "Some files could not be read.", skippedTooLarge: "skipped (too large)", couldNotRead: "could not be read", skippedMaxFiles: "skipped (file limit)", remove: "Remove"
} as const;
...
const ru: Record<StrategyMessageKey, string> = {
  ...
  importPine: "Импорт Pine Script", pineHint: "Вставьте TradingView Pine Script (v4–v6) или загрузите файлы .pine. Каждый indicator() станет индикатором, а strategy() — редактируемой стратегией. Неподдерживаемые конструкции отклоняются с понятной причиной.", loadPineFiles: "Загрузить файл(ы) .pine", pastedScript: "Вставленный скрипт", convert: "Преобразовать", artifact: "артефакт", artifacts: "артефакта(ов)", add: "Добавить", converted: "преобразовано", rejected: "отклонено", approximations: "Приближения — проверьте перед использованием с реальными средствами:", someFilesUnreadable: "Некоторые файлы не удалось прочитать.", skippedTooLarge: "пропущено (слишком большой размер)", couldNotRead: "не удалось прочитать", skippedMaxFiles: "пропущено (лимит файлов)", remove: "Удалить"
};
