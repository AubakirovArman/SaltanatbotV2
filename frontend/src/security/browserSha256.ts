/**
 * SHA-256 that remains available on public HTTP origins where Web Crypto is
 * intentionally unavailable. Web Crypto is preferred; the fallback is a small
 * deterministic implementation for integrity checks, not password hashing.
 */
...
function sha256Bytes(input: Uint8Array): Uint8Array {
  ...
  view.setUint32(message.length - 8, Math.floor(bitLength / 0x1_0000_0000), false);
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + sigma1 + choose + CONSTANTS[index] + schedule[index]) >>> 0;
