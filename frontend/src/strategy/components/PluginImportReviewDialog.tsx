    ...
    <dialog ref={dialogRef} className="plugin-dialog" aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); onClose(); }} onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      ...
      <div className="plugin-dialog-body">
        ...
        <section className={`plugin-update-status ${riskyTransition ? "risk" : "safe"}`} aria-label={t("pluginUpdateAssessment")}>
          ...
          {analysis.reference && <p>{t("installedReferenceVersion")}: v{analysis.reference.version} → v{manifest.version} · {analysis.relatedInstallations} {t("localInstallations")}</p>}
      ...
      <footer><button type="button" onClick={onClose}>{t("cancel")}</button><button type="button" className="primary" disabled={!confirmationAllowed} onClick={() => onConfirm(plugin, trustSigner)}>{t("importReviewedPlugin")}</button></footer>
    </dialog>
  );
}
