 ...
 */
export async function generateStrategyCandidates(spec: StrategyGeneratorSpec = {}, runtime: GeneratorRuntimeOptions = {}): Promise<StrategyGenerationResult> {
  const config = resolveGeneratorConfig(spec);
  const random = createGeneratorRandom(config.seed);
  const candidates: GeneratedStrategyCandidate[] = [];
  ...
  let population: GeneratedStrategyCandidate[] = [];
  ...
  for (let generation = 1; generation <= config.generations && candidates.length < config.targetCandidates; generation += 1) {
    ...
    if (nextPopulation.length < target) exhausted = true;
    if (!nextPopulation.length) break;
...
export function resolveGeneratorConfig(spec: StrategyGeneratorSpec = {}): ResolvedGeneratorConfig {
  ...
  return {
    seed: Number.isFinite(spec.seed) ? (spec.seed as number) >>> 0 : DEFAULT_SEED,
    populationSize,
    generations,
    families,
    directions,
...
function breedCandidate(population: readonly GeneratedStrategyCandidate[], generation: number, config: ResolvedGeneratorConfig, random: GeneratorRandom): GeneratedStrategyCandidate {
  ...
  let mutationLog: GenerationProvenance["mutationLog"] = [];
  if (mutate) {
...
function buildCandidate(genome: StrategyGenome, provenance: GenerationProvenance): GeneratedStrategyCandidate {
  const ir = compileStrategyGenome(genome);
  const validation = validateGeneratedStrategy(ir);
  if (!validation.valid) throw new Error(`Generator produced invalid StrategyIR: ${validation.issues.join(", ")}`);
  return {
...
function registerCandidate(candidate: GeneratedStrategyCandidate, registry: Map<string, string>): boolean {
  const canonical = canonicalStrategyJson(candidate.ir);
  const registered = registry.get(candidate.fingerprint);
  if (registered === canonical) return false;
  if (registered !== undefined) throw new Error(`Canonical strategy fingerprint collision: ${candidate.fingerprint}`);
  registry.set(candidate.fingerprint, canonical);
  return true;
}

function progress(phase: GeneratorProgress["phase"], generation: number, config: ResolvedGeneratorConfig, accepted: number, attempts: number, duplicates: number): GeneratorProgress {
...
function report(runtime: GeneratorRuntimeOptions, update: GeneratorProgress): void {
...
function allowedFamilies(requested: readonly StrategyFamily[] | undefined): StrategyFamily[] {
...
function allowedDirections(requested: readonly TradeDirection[] | undefined): TradeDirection[] {
...
function assertNotAborted(signal: AbortSignal | undefined): void {
  if (signal?.aborted) throw new StrategyGenerationAbortedError();
}

async function yieldWhenDue(deadline: number, signal: AbortSignal | undefined): Promise<number> {
