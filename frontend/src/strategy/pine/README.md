...
`convert.ts` remains a temporary lowering monolith, while pure semantic analysis, drawing lowering and numeric built-in call lowering have moved to dedicated modules. Continue extracting identifier/operator, boolean and statement lowering plus Blockly serialization in the order defined by `docs/MODULAR_ARCHITECTURE.md`. Keep `convertPine()` as the stable facade during the migration.
