...
import { PaperRobotGridSection } from "./PaperRobotGridSection";
            ...
            <tr key={row.robot.botId}>
              <th scope="row">
                <button type="button" className="paper-robot-open" onClick={(event) => onOpen(row, event.currentTarget)}>
                  <strong>{row.name}{row.metadata?.dca && <span className="ex-badge dca">{dcaText(locale, "typeBadge")}</span>}{row.metadata?.grid && <span className="ex-badge grid">{gridText(locale, "typeBadge")}</span>}</strong><small>{row.symbol ?? paperPortfolioText(locale, "unavailable")}</small>
                </button>
              </th>
          ...
          <li key={row.robot.botId}>
            <article className="paper-robot-card">
              <button type="button" className="paper-robot-card-open" onClick={(event) => onOpen(row, event.currentTarget)}>
                <span><strong>{row.name}{row.metadata?.dca && <span className="ex-badge dca">{dcaText(locale, "typeBadge")}</span>}{row.metadata?.grid && <span className="ex-badge grid">{gridText(locale, "typeBadge")}</span>}</strong><small>{row.strategy ?? row.symbol ?? paperPortfolioText(locale, "unavailable")}</small></span>
    ...
    <div className="paper-detail-backdrop" onMouseDown={(event) => {
      ...
      <aside ref={drawerRef} className="paper-detail-drawer" role="dialog" aria-modal="true" aria-labelledby={headingId}>
        ...
        <div className="paper-detail-scroll">
          ...
          {row.metadata?.grid && <PaperRobotGridSection grid={row.metadata.grid} inventoryPnl={row.robot.metrics.unrealizedPnl} locale={locale} />}
