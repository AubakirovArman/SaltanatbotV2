# Workspace domain

This folder owns portable chart-workspace state. It does not render React UI.

`workspaces.ts` defines schema v6 for:

- one, vertical-split, horizontal-split and four-chart layouts;
- independently linked symbol, timeframe, chart type, indicators, comparisons, crosshair and absolute visible-time-range settings;
- side-panel visibility, size and dock order;
- canonical enabled indicators/comparisons, bounded per-pane overrides, exchange and theme;
- bounded immutable autosave revisions and rollback;
- SHA-256 verified `.saltanat-workspace.json` export/import.

Legacy snapshots keep independent secondary chart types when `linkChartType` is absent; new schema-v6 panes store the choice explicitly. Keep migrations in
`normalizeWorkspace`; UI components must not parse persisted JSON directly.

Tests live in `frontend/tests/workspaces.test.ts`.
