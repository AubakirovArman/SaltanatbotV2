...
function normalizeChart(value: unknown, missingLinkChartType: boolean, primary: boolean, fallbackTimeZone: ChartTimeZone): WorkspaceChart | undefined {
  ...
  return {
    ...
    timeframe: item.timeframe,
    chartType: item.chartType,
    exchange: item.exchange === "binance" || item.exchange === "bybit" || item.exchange === "hyperliquid" ? item.exchange : undefined,
    marketType: item.exchange === "hyperliquid" ? "linear" : item.marketType === "linear" || item.marketType === "inverse" ? item.marketType : "spot",
    priceType: item.exchange === "hyperliquid" ? "last" : item.priceType === "mark" || item.priceType === "index" ? item.priceType : "last",
