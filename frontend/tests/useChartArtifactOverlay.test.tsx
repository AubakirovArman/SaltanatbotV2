...
import {
  useChartArtifactOverlay,
  type ArtifactOverlayBuilder,
  type BuiltArtifactOverlay
} from "../src/chart/useChartArtifactOverlay";
...
function built(symbol: string): BuiltArtifactOverlay {
  return {
    overlay: {
      id: artifact.id,
      name: artifact.name,
...
describe("useChartArtifactOverlay", () => {
  it("publishes a completed overlay and forwards explicit input overrides", async () => {
    const builder = vi.fn<ArtifactOverlayBuilder>(async (request) => built(request.symbol));
  ...
  it("ignores an async result after the market changes", async () => {
    ...
    await act(async () => { void controller?.addArtifact(artifact.id); });
