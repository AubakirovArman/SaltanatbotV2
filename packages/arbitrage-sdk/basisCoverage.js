...
export function parseBasisIdentityCoverage(value) {
    if (value === undefined)
        return undefined;
    ...
    if (new Set(failedSources).size !== failedSources.length)
        throw new Error("identityCoverage.failedSources must be unique");
    if (complete !== (failedSources.length === 0) || (complete && stale))
        throw new Error("identityCoverage status is inconsistent with failed sources");
