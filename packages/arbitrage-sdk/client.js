...
import { parseContinuousFeedHealthResponse } from "./continuousFeedHealth.js";
...
export class SaltanatArbitrageClient {
    ...
    async instruments(options = {}, signal = undefined) {
    ...
    async request(url, init, signal) {
        if (signal?.aborted)
            throw new ArbitrageSdkError("request cancelled", undefined, "cancelled");
        ...
        try {
            ...
            if (!response.ok) {
                const error = value && typeof value === "object" && !Array.isArray(value) && typeof value.error === "string" ? value.error : `HTTP ${response.status}`;
            ...
            if (signal?.aborted)
                throw new ArbitrageSdkError("request cancelled", undefined, "cancelled");
            if (timedOut)
                throw new ArbitrageSdkError(`request exceeded ${this.timeoutMs}ms`, undefined, "timeout");
...
export function parseBasisScan(value) {
    ...
    const sources = array(row.sources, "sources", 100).map((value, index) => {
        ...
        return {
            exchange: exact(source.exchange, ["binance", "bybit"], `sources[${index}].exchange`),
            market: exact(source.market, ["spot", "perpetual"], `sources[${index}].market`),
            ok: bool(source.ok, `sources[${index}].ok`),
            ...(message ? { message } : {})
        };
    });
    assertUnique(sources.map((source) => `${source.exchange}:${source.market}`), "basis source keys");
    if (sources.some((source) => !source.ok) && !stale)
        throw new Error("basis stale flag must cover unhealthy sources");
    const opportunities = rawOpportunities.map((opportunity) => parseBasisOpportunity(opportunity, stale, sources));
    assertUnique(opportunities.map((opportunity) => opportunity.id), "basis opportunity IDs");
    ...
    if (totalOpportunities < opportunities.length)
        throw new Error("basis totalOpportunities cannot be smaller than returned rows");
    if (truncated !== totalOpportunities > opportunities.length)
        throw new Error("basis truncated must match totalOpportunities and returned rows");
    ...
    for (const opportunity of opportunities) {
        ...
        if (opportunity.capturedAt < updatedAt)
            throw new Error("basis opportunity capturedAt cannot precede scan updatedAt");
    }
    return {
        updatedAt,
        stale,
...
function parseBasisOpportunity(value, scanStale, sources) {
    ...
    const identityScope = exact(row.identityScope, ["venue-native", "cross-venue-reviewed"], "identityScope");
    const spotExchange = exact(row.spotExchange, ["binance", "bybit"], "spotExchange");
    const futuresExchange = exact(row.futuresExchange, ["binance", "bybit"], "futuresExchange");
    ...
    if (spotBid >= spotAsk)
        throw new Error("spot top book must have bid below ask");
    if (futuresBid >= futuresAsk)
        throw new Error("futures top book must have bid below ask");
    ...
    if (spotReceivedAt > capturedAt || futuresReceivedAt > capturedAt)
        throw new Error("basis receive timestamp cannot be in the future");
    ...
    const dataQuality = exact(row.dataQuality, ["fresh", "stale", "skewed", "unverified"], "dataQuality");
    ...
    if (!dependencyHealthy && dataQuality !== "stale")
        throw new Error("basis route depends on an unhealthy source but is not marked stale");
    const expectedId = `${symbol}:${spotExchange}:${futuresExchange}`;
    const routeId = text(row.id, "id");
    if (routeId !== expectedId)
        throw new Error("basis opportunity id does not match its ordered route");
    return {
        id: routeId,
        strategyKind: exact(row.strategyKind, ["cash-and-carry"], "strategyKind"),
        edgeKind: exact(row.edgeKind, ["projected"], "edgeKind"),
        identityScope,
        symbol,
        assetId,
        spotInstrumentId,
...
export function parseTriangularScan(value) {
    const row = record(value, "triangular scan");
    if (row.snapshotSource !== "rest-snapshot")
        throw new Error("triangular snapshotSource must be rest-snapshot");
    if (row.executionStatus !== "non-executable-candidate")
        throw new Error("triangular REST scan must be non-executable");
    if (row.sequenceVerified !== false)
        throw new Error("triangular REST scan cannot be sequence verified");
    return {
        updatedAt: positive(row.updatedAt, "updatedAt"),
        venue: exact(row.venue, ["binance", "bybit"], "venue"),
        ...
        marketDataMode: exact(row.marketDataMode, ["rest-top-book"], "marketDataMode"),
...
function parseTriangularOpportunity(value) {
    const row = record(value, "triangular opportunity");
    const rawLegs = array(row.legs, "legs", 3);
    if (rawLegs.length !== 3)
        throw new Error("triangular opportunity requires three legs");
    ...
    for (const required of ["top-book-only", "rest-snapshot", "unsequenced", "non-executable-candidate"])
        if (!riskFlags.includes(required))
            throw new Error(`triangular opportunity is missing ${required} provenance`);
    return {
        id: text(row.id, "id"),
        ...
        legs: rawLegs.map((leg, index) => parseTriangularLeg(leg, index)),
...
function parseTriangularLeg(value, index) {
    const row = record(value, "triangular leg");
    const wireIndex = integer(row.index, `leg[${index}].index`);
    if (wireIndex > 2)
        throw new Error(`leg[${index}].index is unsupported`);
    if (wireIndex !== index)
        throw new Error(`leg[${index}].index must preserve the ordered 0,1,2 route`);
    return {
        index: wireIndex,
        symbol: text(row.symbol, "symbol"),
        side: exact(row.side, ["buy", "sell"], "side"),
...
function assertBasisIdentity(identityScope, assetId, spotExchange, futuresExchange) {
...
function basisInstrumentId(value, exchange, market, symbol, label) {
    const parsed = text(value, label);
    if (parsed !== `${exchange}:${market}:${symbol}`)
        throw new Error(`${label} does not match the route identity`);
    return parsed;
}
function verifiedTimestamp(timestamp, verified, leg, evaluatedAt, clockCorrected = false) {
    if (timestamp === undefined) {
        if (verified)
            throw new Error(`${leg}ExchangeTs is required when exchange timestamp is verified`);
        return undefined;
    }
    const value = positiveSafeTimestamp(timestamp, `${leg}ExchangeTs`);
    if (!verified)
        throw new Error(`${leg}ExchangeTs must be omitted when exchange timestamp is unverified`);
    if (!clockCorrected && value > evaluatedAt + MAX_BASIS_FUTURE_CLOCK_SKEW_MS)
        throw new Error(`${leg}ExchangeTs exceeds the future-clock safety boundary`);
    return value;
}
function basisSourceHealthy(sources, exchange, market) {
    const source = sources.find((candidate) => candidate.exchange === exchange && candidate.market === market);
    if (!source)
        throw new Error(`basis route is missing source status for ${exchange}:${market}`);
    return source.ok;
}
function assertApproximately(actual, expected, label) {
    const tolerance = 1e-8 * Math.max(1, Math.abs(expected));
    if (Math.abs(actual - expected) > tolerance)
        throw new Error(`${label} is inconsistent with executable prices and sizes`);
}
...
function validBaseUrl(value) {
    ...
    if (url.username || url.password || url.search || url.hash)
        throw new Error("baseUrl cannot contain credentials, query or fragment");
    return url;
}
...
function venuePath(value) {
    const normalized = value.trim().toLowerCase();
    if (!/^[a-z0-9_-]{2,30}$/.test(normalized))
        throw new Error("venue is unsupported");
    return normalized;
}
