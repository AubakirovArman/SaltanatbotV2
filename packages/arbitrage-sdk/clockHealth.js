import { array, bool, exact, integer, nonNegative, optionalFinite, optionalText, record, text } from "./validation.js";
export function parseVenueClockHealth(value) {
    const row = record(value, "venue clock health");
    if (row.schemaVersion !== 1)
        throw new Error("venue clock health schemaVersion must be 1");
    ...
    if (sources.length < 1)
        throw new Error("venue clock health requires at least one source");
    if (new Set(sources.map(({ sourceId }) => sourceId)).size !== sources.length)
        throw new Error("venue clock sourceId values must be unique");
    const measuredStale = sources.some(({ ok, status }) => !ok || status !== "calibrated");
    if (stale !== measuredStale)
        throw new Error("venue clock stale flag is inconsistent with source health");
    return { schemaVersion: 1, updatedAt, stale, sources };
}
function parseVenueClockSource(value, updatedAt, label) {
    ...
    const status = exact(row.status, ["calibrated", "degraded", "expired", "unavailable"], `${label}.status`);
    const evaluatedAt = positiveSafeTimestamp(row.evaluatedAt, `${label}.evaluatedAt`);
    if (evaluatedAt !== updatedAt)
        throw new Error(`${label}.evaluatedAt must match updatedAt`);
    ...
    if (consistentSampleCount > sampleCount)
        throw new Error(`${label}.consistentSampleCount exceeds sampleCount`);
    ...
    const reason = row.reason === undefined ? undefined : exact(row.reason, ["no-samples", "sample-expired", "insufficient-consistent-samples", "uncertainty-too-high"], `${label}.reason`);
    ...
    if (status === "unavailable") {
        if (sampleCount !== 0 || consistentSampleCount !== 0 || reason !== "no-samples" || timing.some((item) => item !== undefined))
            throw new Error(`${label} unavailable state contains calibrated timing fields`);
    }
    else {
        if (timing.some((item) => item === undefined) || sampledAt > updatedAt || expiresAt < sampledAt || offsetLowerMs > offsetUpperMs || minimumObservedRoundTripMs > roundTripMs)
            throw new Error(`${label} timing interval is incomplete or inconsistent`);
        ...
        if (status === "calibrated" && reason !== undefined)
            throw new Error(`${label} calibrated state cannot include a degradation reason`);
        if (status === "expired" && reason !== "sample-expired")
            throw new Error(`${label} expired state requires sample-expired reason`);
        if (status === "degraded" && reason !== "insufficient-consistent-samples" && reason !== "uncertainty-too-high")
            throw new Error(`${label} degraded state requires a calibration reason`);
    }
    return {
        sourceId,
        status,
...
function optionalSafeTimestamp(value, label) {
...
function assertApproximately(actual, expected, label) {
    ...
    if (!Number.isFinite(actual) || Math.abs(actual - expected) > tolerance)
        throw new Error(`${label} is inconsistent with the clock interval`);
}
