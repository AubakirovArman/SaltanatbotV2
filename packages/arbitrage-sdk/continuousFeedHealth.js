...
const VENUES = ["okx", "gate", "hyperliquid", "deribit", "kraken", "coinbase", "dydx", "kucoin", "mexc"];
...
const HEALTH_STATES = ["healthy", "degraded", "unhealthy"];
const MAX_SOURCES = 128;
/** Strict parser for the public, credential-free operator diagnostics endpoint. */
export function parseContinuousFeedHealthResponse(value) {
    ...
    if (maxReceiveAgeMs < 100 || maxReceiveAgeMs > 300_000)
        throw new Error("maxReceiveAgeMs is outside the supported boundary");
    const sources = array(row.sources, "sources", MAX_SOURCES).map((source, index) => parseSource(source, index, capturedAt, maxReceiveAgeMs));
    unique(sources.map(({ instrumentId }) => instrumentId), "source instrument IDs");
    if (sources.some((source, index) => index > 0 && sources[index - 1].instrumentId.localeCompare(source.instrumentId) > 0))
        throw new Error("continuous feed health sources must be sorted by instrumentId");
    ...
    if (JSON.stringify(counts) !== JSON.stringify(expectedCounts))
        throw new Error("continuous feed health counts are inconsistent with sources");
    const state = exact(row.state, ["idle", "healthy", "degraded", "unhealthy"], "state");
    const expectedState = aggregateState(sources);
    if (state !== expectedState)
        throw new Error("continuous feed health aggregate state is inconsistent with sources");
    return {
        schemaVersion: 1,
...
function parseSource(value, index, capturedAt, maxReceiveAgeMs) {
    ...
    if (reconnect.scheduled !== (state === "reconnecting") || reconnect.observedConnectionRestarts !== Math.max(0, generation - 1))
        throw new Error(`${label}.reconnect is inconsistent with feed generation/state`);
    ...
    if (hasBook && !continuity)
        throw new Error(`${label} current book lacks continuity evidence`);
    if ((hasBook || hasTopBook || hasFunding) && !lastReceive)
        throw new Error(`${label} evidence lacks last-receive provenance`);
    if (continuity && (!lastReceive || continuity.receivedAt > lastReceive.at))
        throw new Error(`${label} continuity lacks ordered last-receive provenance`);
    ...
    if (bookContinuityReady !== expectedBookContinuityReady)
        throw new Error(`${label}.bookContinuityReady is inconsistent with current book freshness/continuity`);
    const health = exact(row.health, HEALTH_STATES, `${label}.health`);
    if (health !== sourceHealth(state, lastReceive?.fresh === true))
        throw new Error(`${label}.health is inconsistent with feed state/freshness`);
    return {
        venue,
        instrumentId,
        marketType,
...
function parseLastReceive(value, label, capturedAt, maxReceiveAgeMs, generation) {
    ...
    if (at > capturedAt)
        throw new Error(`${label}.at cannot follow capturedAt`);
    const ageMs = integer(row.ageMs, `${label}.ageMs`);
    if (ageMs !== capturedAt - at)
        throw new Error(`${label}.ageMs is inconsistent with capturedAt`);
    ...
    if (currentGeneration !== (connectionGeneration === generation))
        throw new Error(`${label}.currentGeneration is inconsistent`);
    const fresh = bool(row.fresh, `${label}.fresh`);
    if (fresh !== (currentGeneration && ageMs <= maxReceiveAgeMs))
        throw new Error(`${label}.fresh is inconsistent with age/generation`);
    return { at, ageMs, kind: exact(row.kind, ["book", "top-book", "funding"], `${label}.kind`), connectionGeneration, currentGeneration, fresh };
}
function parseContinuity(value, label, generation, capturedAt, maxReceiveAgeMs) {
    const kind = exact(record(value, label).kind, ["sequence-verified", "checksum-verified", "sequence-observed", "atomic-snapshot"], `${label}.kind`);
    ...
    if (generationMatches !== (connectionGeneration === generation))
        throw new Error(`${label}.generationMatches is inconsistent`);
    const receivedAt = positiveInteger(row.receivedAt, `${label}.receivedAt`);
    if (receivedAt > capturedAt)
        throw new Error(`${label}.receivedAt cannot follow capturedAt`);
    const ageMs = integer(row.ageMs, `${label}.ageMs`);
    if (ageMs !== capturedAt - receivedAt)
        throw new Error(`${label}.ageMs is inconsistent with capturedAt`);
    const fresh = bool(row.fresh, `${label}.fresh`);
    if (fresh !== (generationMatches && ageMs <= maxReceiveAgeMs))
        throw new Error(`${label}.fresh is inconsistent with age/generation`);
    const timing = { receivedAt, ageMs, fresh, connectionGeneration, generationMatches };
    if (kind === "sequence-verified") {
        if (row.verified !== true)
            throw new Error(`${label}.verified is invalid`);
        return {
            kind,
    ...
    if (row.verified !== false || row.protocol !== "hyperliquid-block-snapshot")
        throw new Error(`${label} atomic snapshot proof is invalid`);
...
function boundedCount(value, label) {
    const result = integer(value, label);
    if (result > MAX_SOURCES)
        throw new Error(`${label} exceeds the source boundary`);
    return result;
}
function sourceHealth(state, fresh) {
    if (state === "live" && fresh)
        return "healthy";
...
function aggregateState(sources) {
    if (sources.length === 0)
        return "idle";
    if (sources.every((source) => source.health === "healthy"))
        return "healthy";
    if (sources.every((source) => source.health === "unhealthy"))
        return "unhealthy";
    return "degraded";
}
