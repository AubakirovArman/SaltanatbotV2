...
import {
  assertContinuousSameBook,
  parseContinuousMarketBooks,
  parseContinuousMarketInstruments,
  parseContinuousMarketSources,
  validateContinuousMarketCandidateContext,
  type ContinuousMarketBookContext,
  type ContinuousMarketInstrumentContext,
  type ContinuousMarketSourceContext
} from "./continuousMarketEconomicsMetadata.js";
import { expectedContinuousPairQuantities, type ContinuousExpectedLegQuantity } from "./continuousMarketEconomicsQuantity.js";
import { expectedContinuousStrategyReasons } from "./continuousMarketEconomicsStrategy.js";
import {
  CONTINUOUS_MARKET_BLOCK_CODES,
  ...
  type ContinuousMarketCalibratedClockLeg,
  type ContinuousMarketEconomicIdentityEvidence,
  ...
  type ContinuousMarketOnlyEvaluation
} from "./continuousMarketEconomicsTypes.js";
...
const MARKET_BLOCK_CODES = new Set<ContinuousMarketBlockCode>(CONTINUOUS_MARKET_BLOCK_CODES.slice(0, CONTINUOUS_MARKET_BLOCK_CODES.indexOf("account-capital-missing")));
const BPS = 10_000;
...
export function parseContinuousMarketEconomics(summaryValue: unknown, evaluationsValue: unknown, context: ContinuousMarketEconomicsParseContext): { marketEconomics: ContinuousMarketEconomicsSummary; marketEvaluations: ContinuousMarketEvaluation[] } {
  ...
  if (context.totalCompatibleCandidates < context.candidates.length) throw new Error("continuous discovery candidate total is inconsistent");
  if (context.discoveryTruncated !== context.totalCompatibleCandidates > context.candidates.length) throw new Error("continuous discovery truncation is inconsistent");
  if (summary.evaluatedAt !== context.capturedAt || summary.totalCandidates !== context.totalCompatibleCandidates) throw new Error("continuous market economics discovery totals are inconsistent");
  if (summary.publishedEvaluations !== rawEvaluations.length || rawEvaluations.length !== context.candidates.length) throw new Error("continuous market economics published count is inconsistent");
  ...
  if (!context.discoveryTruncated && summary.evaluatedCandidates !== context.totalCompatibleCandidates) throw new Error("complete continuous market economics evaluated count is inconsistent");
  if (summary.truncated !== context.discoveryTruncated) throw new Error("continuous market economics truncation is inconsistent");
  const marketEvaluations = rawEvaluations.map((value, index) => {
    const candidate = context.candidates[index];
    if (!candidate) throw new Error("continuous market evaluation has no matching candidate");
    return parseEvaluation(value, index, candidate, context.capturedAt, instruments, books, sources);
  });
  unique(
    marketEvaluations.map(({ routeId }) => routeId),
    "continuous market evaluation route IDs"
  );
  const publishedMarketOnlyCandidates = marketEvaluations.filter(({ status }) => status === "market-only").length;
  if (
    summary.marketOnlyCandidates + summary.blockedCandidates !== summary.evaluatedCandidates ||
...
function parseSummary(value: unknown): ContinuousMarketEconomicsSummary {
  const row = record(value, "discovery.marketEconomics");
  onlyKeys(
    row,
    ["engine", "readOnly", "researchOnly", "executable", "outcomeClass", "evaluatedAt", "totalCandidates", "evaluatedCandidates", "marketOnlyCandidates", "blockedCandidates", "publishedEvaluations", "publishedMarketOnlyCandidates", "publishedBlockedCandidates", "truncated", "feePolicy"],
    "discovery.marketEconomics"
  );
  if (row.engine !== CONTINUOUS_MARKET_ECONOMICS_ENGINE || row.readOnly !== true || row.researchOnly !== true || row.executable !== false || row.outcomeClass !== "projected") throw new Error("continuous market economics safety envelope is invalid");
...
function parseEvaluation(
  value: unknown,
  index: number,
  candidate: ContinuousRouteCandidate,
  capturedAt: number,
  instruments: ReadonlyMap<string, ContinuousMarketInstrumentContext>,
  books: ReadonlyMap<string, ContinuousMarketBookContext>,
  sources: ReadonlyMap<string, ContinuousMarketSourceContext>
): ContinuousMarketEvaluation {
  const label = `discovery.marketEvaluations[${index}]`;
  const row = record(value, label);
  const status = exact(row.status, ["market-only", "blocked"] as const, `${label}.status`);
  ...
  if (baseAsset !== expectedBaseAsset || quoteAsset !== expectedQuoteAsset) throw new Error(`${label} canonical assets are inconsistent`);
  ...
  if (status === "blocked") {
    if (marketReasons.length === 0) throw new Error(`${label} blocked status lacks a market-data blocker`);
    return { ...base, status: "blocked" };
  }
  if (!longInstrument || !shortInstrument || baseAsset === null || quoteAsset === null) throw new Error(`${label} market-only metadata is incomplete`);
  if (marketReasons.length > 0) throw new Error(`${label} market-only status contains a market-data blocker`);
  if ([longInstrument, shortInstrument].some((instrument) => capturedAt < instrument.economicIdentityAsOf || capturedAt > instrument.economicIdentityValidUntil)) throw new Error(`${label} market-only economic identity is not valid at evaluatedAt`);
  const rawLegs = array(row.legs, `${label}.legs`, 2);
  if (rawLegs.length !== 2) throw new Error(`${label}.legs must contain exactly two rows`);
  const longBook = books.get(longInstrumentId);
  const shortBook = books.get(shortInstrumentId);
  if (!longBook || !shortBook) throw new Error(`${label} market-only books are missing`);
...
function parseExecutionBoundary(value: unknown, label: string): ContinuousMarketEvaluationBase["executionBoundary"] {
  const row = record(value, `${label}.executionBoundary`);
  onlyKeys(row, ["permission", "orders", "reason"], `${label}.executionBoundary`);
  if (row.permission !== false || row.orders !== "not-supported" || row.reason !== "market-data-and-public-entry-fees-only") throw new Error(`${label} execution boundary is invalid`);
  return { permission: false, orders: "not-supported", reason: "market-data-and-public-entry-fees-only" };
}

function parseBlockedReasons(value: unknown, label: string, candidate: ContinuousRouteCandidate): ContinuousMarketBlockReason[] {
  const reasons = array(value, `${label}.blockedReasons`, 32).map((value, index) => {
    ...
    const stage = exact(row.stage, ["market-data", "strategy-evidence"] as const, `${reasonLabel}.stage`);
    if ((stage === "market-data") !== MARKET_BLOCK_CODES.has(code)) throw new Error(`${reasonLabel} stage is inconsistent with its code`);
  ...
  if (reasons.length === 0) throw new Error(`${label}.blockedReasons must not be empty`);
  unique(
    reasons.map((reason) => `${reason.stage}\u0000${reason.code}\u0000${reason.subject ?? ""}\u0000${reason.message}`),
  ...
  const expectedStrategy = expectedContinuousStrategyReasons(candidate)
    .map(({ code, subject }) => `${code}\u0000${subject}`)
    .sort();
  if (JSON.stringify(actualStrategy) !== JSON.stringify(expectedStrategy)) throw new Error(`${label} required strategy blockers are inconsistent`);
  return reasons;
}

function parseLeg(value: unknown, label: string, expectedRole: "long" | "short", instrument: ContinuousMarketInstrumentContext, book: ContinuousMarketBookContext, source: ContinuousMarketSourceContext | undefined): ContinuousMarketLeg {
  const row = record(value, label);
  ...
  const role = exact(row.role, ["long", "short"] as const, `${label}.role`);
  const side = exact(row.side, ["buy", "sell"] as const, `${label}.side`);
  if (role !== expectedRole || side !== (expectedRole === "long" ? "buy" : "sell")) throw new Error(`${label} must preserve long-buy/short-sell order`);
  ...
  if (instrumentId !== instrument.instrumentId || venue !== instrument.venue || symbol !== instrument.symbol || marketType !== instrument.marketType || quantityUnit !== instrument.quantityModel.unit) throw new Error(`${label} normalized instrument identity is inconsistent`);
  if (book.instrumentId !== instrumentId || book.venue !== venue || book.marketType !== marketType || book.quantityUnit !== quantityUnit) throw new Error(`${label} top-book identity is inconsistent`);
  ...
  if (takerFeeBps >= BPS || takerFeeBps !== instrument.takerFeeBps) throw new Error(`${label} public taker fee is inconsistent`);
...
function parseBookEvidence(value: unknown, label: string, book: ContinuousMarketBookContext): ContinuousMarketLeg["bookEvidence"] {
  ...
  if (book.continuity.kind !== "sequence-verified" && book.continuity.kind !== "checksum-verified") throw new Error(`${evidenceLabel} cannot upgrade unverified continuity`);
  const sourceId = identifier(row.sourceId, `${evidenceLabel}.sourceId`);
  const quality = exact(row.quality, ["sequence-verified", "checksum-verified"] as const, `${evidenceLabel}.quality`);
  ...
  if (book.continuity.kind === "checksum-verified") {
    ...
    if (checksum !== book.continuity.checksum) throw new Error(`${evidenceLabel} checksum is inconsistent`);
    return { sourceId, quality, protocol, sequence, checksum, connectionGeneration, exchangeTs, receivedAt };
  }
  if (row.checksum !== undefined) throw new Error(`${evidenceLabel} sequence proof cannot claim a checksum`);
  return { sourceId, quality, protocol, sequence, connectionGeneration, exchangeTs, receivedAt };
}

function parseCapacity(value: unknown, label: string): ContinuousMarketOnlyEvaluation["capacity"] {
  ...
  if (row.scope !== "maximum-visible-top-book") throw new Error(`${label} capacity scope is invalid`);
  return {
...
function parseEdges(value: unknown, label: string): ContinuousMarketOnlyEvaluation["edges"] {
  ...
  if (row.coverage !== "top-book-entry-and-public-taker-fees-only") throw new Error(`${label} edge coverage is invalid`);
  return {
...
function parseFreshness(value: unknown, label: string, capturedAt: number, longBook: ContinuousMarketBookContext, shortBook: ContinuousMarketBookContext): ContinuousMarketOnlyEvaluation["freshness"] {
  const row = record(value, `${label}.freshness`);
  if (row.status !== "fresh") throw new Error(`${label} freshness status is invalid`);
  ...
  if (oldestReceivedAt !== expectedOldest || newestReceivedAt !== expectedNewest) throw new Error(`${label} receipt-time provenance is inconsistent`);
  const common = { status: "fresh" as const, quoteAgeMs, legSkewMs, maxBookAgeMs, maxLegSkewMs, oldestReceivedAt, newestReceivedAt };
  if (row.clockBasis === "local-receipt-fallback") {
    ...
    return {
      ...
      fallbackReason: exact(row.fallbackReason, ["same-venue-clock-unavailable", "same-venue-clock-not-calibrated", "clock-provider-unavailable"] as const, `${label}.freshness.fallbackReason`)
    };
  }
  if (row.clockBasis !== "calibrated-venue-interval") throw new Error(`${label} freshness basis is invalid`);
  ...
  if (rawLegs.length !== 2) throw new Error(`${label}.freshness.clockLegs must contain two rows`);
  const clockLegs = [parseClockLeg(rawLegs[0], `${label}.freshness.clockLegs[0]`, capturedAt, longBook), parseClockLeg(rawLegs[1], `${label}.freshness.clockLegs[1]`, capturedAt, shortBook)] as const;
  ...
  if (
    !closeNumber(quoteAgeLowerMs, expectedAgeLower) ||
...
function parseClockLeg(value: unknown, label: string, capturedAt: number, book: ContinuousMarketBookContext): ContinuousMarketCalibratedClockLeg {
  const row = record(value, label);
...
function intervalDistance(left: Pick<ContinuousMarketCalibratedClockLeg, "localEventEarliestAt" | "localEventLatestAt">, right: Pick<ContinuousMarketCalibratedClockLeg, "localEventEarliestAt" | "localEventLatestAt">) {
  if (left.localEventLatestAt < right.localEventEarliestAt) return right.localEventEarliestAt - left.localEventLatestAt;
  if (right.localEventLatestAt < left.localEventEarliestAt) return left.localEventEarliestAt - right.localEventLatestAt;
  return 0;
}

function closeNumber(left: number, right: number) {
  const scale = Math.max(1, Math.abs(left), Math.abs(right));
  return Math.abs(left - right) <= scale * Number.EPSILON * 16;
}

function parseEvidence(value: unknown, label: string, long: ContinuousMarketLeg, short: ContinuousMarketLeg, longInstrument: ContinuousMarketInstrumentContext, shortInstrument: ContinuousMarketInstrumentContext): ContinuousMarketOnlyEvaluation["evidence"] {
  ...
  if (row.marketDataComplete !== true || row.continuityVerified !== true || row.requiredStrategyEvidenceComplete !== false) throw new Error(`${label} evidence boundary is invalid`);
  ...
  if (identityValues.length !== 2) throw new Error(`${label}.evidence.economicIdentities must contain exactly two rows`);
  const economicIdentities = [parseEconomicIdentityEvidence(identityValues[0], `${label}.evidence.economicIdentities[0]`, longInstrument), parseEconomicIdentityEvidence(identityValues[1], `${label}.evidence.economicIdentities[1]`, shortInstrument)] as const;
...
function parseEconomicIdentityEvidence(value: unknown, label: string, expected: ContinuousMarketInstrumentContext): ContinuousMarketEconomicIdentityEvidence {
  const row = record(value, label);
  onlyKeys(row, ["instrumentId", "economicAssetId", "status", "source", "version", "asOf", "validUntil"], label);
  const result: ContinuousMarketEconomicIdentityEvidence = {
  ...
  if (
    result.instrumentId !== expected.instrumentId ||
...
function validateLegQuantities(actual: ContinuousMarketLeg, expected: ContinuousExpectedLegQuantity, label: string) {
  for (const key of ["topNativeQuantity", "alignedNativeCapacity", "usedNativeQuantity", "baseQuantity"] as const) assertApproximately(actual[key], expected[key], `${label} ${key}`);
}
...
function tuple2<T>(value: unknown, label: string, parser: (value: unknown, label: string) => T): readonly [T, T] {
  const values = array(value, label, 2);
  if (values.length !== 2) throw new Error(`${label} must contain exactly two rows`);
...
function boundedInteger(value: unknown, label: string, maximum: number) {
  ...
  if (result > maximum) throw new Error(`${label} exceeds ${maximum}`);
  return result;
}

function boundedPositiveInteger(value: unknown, label: string, maximum: number) {
...
function onlyKeys(row: Record<string, unknown>, allowed: readonly string[], label: string) {
