...
export function expectedContinuousPairQuantities(long, short, longBook, shortBook) {
    ...
    if (commonUnits <= 0n)
        throw new Error("continuous market common quantity is invalid");
    ...
    if (longExpected.usedNativeQuantity + tolerance(longExpected.usedNativeQuantity) < long.minimumQuantity ||
...
function quantityValues(instrument, price, topNativeQuantity) {
    const nativeStep = rational(instrument.quantityStep);
    const quantityModel = instrument.quantityModel;
    const basePerNative = quantityModel.unit === "base"
        ? rational(1)
    ...
    if (alignedNativeUnits <= 0n)
        throw new Error("continuous market top book has no step-aligned capacity");
...
function expectedLegQuantity(values, commonBase) {
    ...
    if (divideRational(usedNative, values.nativeStep).denominator !== 1n)
        throw new Error("continuous market common quantity is not native-step aligned");
    return {
...
function commonRationalStep(left, right) {
...
function rational(value) {
    if (!Number.isFinite(value) || value <= 0)
        throw new Error("continuous market quantity input must be finite and positive");
    ...
    if (!Number.isSafeInteger(exponent) || Math.abs(exponent) > MAX_RATIONAL_EXPONENT)
        throw new Error("continuous market quantity exponent is unsupported");
    const [whole = "", fraction = ""] = coefficient.split(".");
    const digits = `${whole}${fraction}`.replace(/^0+/, "") || "0";
    if (digits.length > MAX_RATIONAL_EXPONENT)
        throw new Error("continuous market quantity precision is unsupported");
...
function normalized(value) {
    if (value.numerator <= 0n || value.denominator <= 0n)
        throw new Error("continuous market rational quantity must be positive");
    ...
    if (result.numerator.toString().length + result.denominator.toString().length > MAX_RATIONAL_DIGITS)
        throw new Error("continuous market rational quantity is too large");
    return result;
}
