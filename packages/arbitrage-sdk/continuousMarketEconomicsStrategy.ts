...
export function expectedContinuousStrategyReasons(candidate: ContinuousRouteCandidate): Array<{ code: ContinuousMarketBlockCode; subject: string }> {
  const result: Array<{ code: ContinuousMarketBlockCode; subject: string }> = [];
  const add = (code: ContinuousMarketBlockCode, subject: string) => result.push({ code, subject });
  if (candidate.family === "cross-venue-spot-spot") {
    ...
    for (const instrumentId of [candidate.longInstrumentId, candidate.shortInstrumentId]) add("derivative-margin-missing", instrumentId);
    add("convergence-evidence-missing", candidate.routeId);
