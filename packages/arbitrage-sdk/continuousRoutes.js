...
const FEED_STATES = ["connecting", "syncing", "live", "gap", "reconnecting", "stopped", "overloaded", "error"];
/**
 ...
 */
export function parseContinuousRouteLiveResponse(value) {
    ...
    const state = exact(row.state, ["disabled", "starting", "live", "degraded", "error"], "state");
    const evaluatedAt = timestamp(row.evaluatedAt, "evaluatedAt");
    const refreshedAt = optionalTimestamp(row.refreshedAt, "refreshedAt");
    if (refreshedAt !== undefined && refreshedAt > evaluatedAt)
        throw new Error("refreshedAt cannot follow evaluatedAt");
    ...
    if (activeInstrumentIds.some((id) => !configured.has(id)))
        throw new Error("active instrument is absent from the operator allowlist");
    const unavailable = array(row.unavailable, "unavailable", 48).map((item, index) => {
        ...
        if (!configured.has(instrumentId))
            throw new Error("unavailable instrument is absent from the operator allowlist");
    ...
    if (state === "disabled" && (configuredInstrumentIds.length > 0 || activeInstrumentIds.length > 0 || discovery.sources.length > 0))
        throw new Error("disabled continuous route state contains active configuration");
    if (state === "live" && unavailable.length > 0)
        throw new Error("live continuous route state cannot contain unavailable instruments");
    return {
        schemaVersion: 1,
...
function parseRuntimeCoverage(value, state, refreshedAt) {
    ...
    if (Object.keys(row).some((key) => !keys.includes(key)))
        throw new Error("continuous route coverage contains unsupported fields");
    const coverage = {
        ...
        reason: exact(row.reason, ["complete", "configuration-disabled", "configuration-invalid", "refresh-pending", "refresh-failed", "partial-instruments"], "coverage.reason")
    };
    ...
    if (JSON.stringify(coverage) !== JSON.stringify(expected) || (state === "error" && coverage.reason === "configuration-invalid" && refreshedAt !== undefined))
        throw new Error("continuous route coverage is inconsistent with runtime state");
    if (coverage.complete && !coverage.current)
        throw new Error("complete continuous route coverage must be current");
    if (coverage.retainedPriorDiscovery && (coverage.current || refreshedAt === undefined))
        throw new Error("retained continuous route discovery lacks stale-refresh provenance");
    return coverage;
}
function parseDiscovery(value, activeIds) {
    const row = record(value, "discovery");
    if (row.engine !== "continuous-route-discovery-v1" || row.executionStatus !== "research-only" || row.executable !== false)
        throw new Error("continuous discovery safety envelope is invalid");
    const candidates = array(row.candidates, "discovery.candidates", 500).map(parseCandidate);
    unique(candidates.map(({ routeKey }) => routeKey), "candidate route keys");
    unique(candidates.map(({ routeId }) => routeId), "candidate route IDs");
    ...
    for (const candidate of candidates) {
        if (!instrumentIds.has(candidate.longInstrumentId) || !instrumentIds.has(candidate.shortInstrumentId))
            throw new Error("candidate references an absent discovery instrument");
    }
    const readyBooks = array(row.routeReadyBooks, "discovery.routeReadyBooks", 64);
    for (const [index, item] of readyBooks.entries())
        parseRouteReadyBook(item, `discovery.routeReadyBooks[${index}]`, activeIds);
    ...
    unique(sources.map(({ instrumentId }) => instrumentId), "continuous source instrument IDs");
    ...
    if (hasMarketEconomics !== hasMarketEvaluations)
        throw new Error("continuous market economics siblings must be present together");
...
function parseCandidate(value, index) {
    ...
    if (row.edgeKind !== "research-candidate" || row.executable !== false)
        throw new Error("continuous candidate safety envelope is invalid");
    ...
    if (!Array.isArray(key) || key.length !== 3 || key[0] !== family || key[1] !== longInstrumentId || key[2] !== shortInstrumentId || JSON.stringify(key) !== routeKey)
        throw new Error("candidate.routeKey does not match its ordered route");
    if (!new RegExp(`^rf:${family}:[a-f0-9]{24}$`).test(routeId))
        throw new Error("candidate.routeId is inconsistent with its family");
    return {
        routeKey,
        routeId,
        family,
...
function parseTopBook(value, label, activeIds) {
    ...
    if (bid >= ask)
        throw new Error(`${label} must not be crossed`);
    return {
...
function parseFunding(value, label, activeIds) {
    const row = record(value, label);
    ...
    if ((exchangeTs !== undefined) !== exchangeTimestampVerified)
        throw new Error(`${label} exchange timestamp proof is inconsistent`);
    return {
        venue: identifier(row.venue, `${label}.venue`),
...
function parseSource(value, label, activeIds) {
    const row = record(value, label);
    ...
    if (status.instrumentId !== instrumentId || status.venue !== instrument.venue)
        throw new Error(`${label} status identity is inconsistent`);
    if (row.book !== undefined)
        validateRawBook(row.book, `${label}.book`, instrumentId);
    if (row.topBook !== undefined && record(row.topBook, `${label}.topBook`).instrumentId !== instrumentId)
        throw new Error(`${label}.topBook identity is inconsistent`);
    if (row.funding !== undefined && record(row.funding, `${label}.funding`).instrumentId !== instrumentId)
        throw new Error(`${label}.funding identity is inconsistent`);
    return {
...
function parseRouteReadyBook(value, label, activeIds) {
    const row = record(value, label);
    activeIdentifier(row.instrumentId, `${label}.instrumentId`, activeIds);
    if (row.complete !== true || row.source !== "websocket")
        throw new Error(`${label} lacks sequence-ready WebSocket provenance`);
...
function validateRawBook(value, label, instrumentId) {
    const row = record(value, label);
    if (row.instrumentId !== instrumentId || row.complete !== true || row.source !== "public-websocket")
        throw new Error(`${label} provenance is inconsistent`);
    validateLevels(row.bids, `${label}.bids`);
    validateLevels(row.asks, `${label}.asks`);
    continuity(row.continuity, `${label}.continuity`);
}
function validateLevels(value, label) {
    for (const [index, item] of array(value, label, 500).entries()) {
        if (!Array.isArray(item) || item.length !== 2)
            throw new Error(`${label}[${index}] must be a price/quantity pair`);
        positive(item[0], `${label}[${index}].price`);
        positive(item[1], `${label}[${index}].quantity`);
    }
}
function continuity(value, label) {
    const row = record(value, label);
    ...
    if (kind === "sequence-verified")
        ...
        if (checksum < 0 || checksum > 0xffffffff || row.protocol !== "kraken-spot-crc32")
            throw new Error(`${label} checksum proof is invalid`);
    }
    ...
    else if (row.sequenceVerified !== false)
        throw new Error(`${label} atomic snapshot cannot claim sequence proof`);
    return kind;
}
function parseProblems(value, label, maximum, withCode) {
...
function activeIdentifier(value, label, activeIds) {
    const result = identifier(value, label);
    if (!activeIds.has(result))
        throw new Error(`${label} is absent from active instruments`);
    return result;
}
function identifier(value, label) {
    const result = text(value, label);
    if (result.length > 300 || [...result].some((character) => character.charCodeAt(0) < 32))
