...
const RATE_UNIT = "decimal-per-settlement";
const STRESS_UNIT = "basis-points-additive-per-settlement";
const BPS = 10_000;
const MAX_RATE = 10;
...
export function parseFundingCurveUniverseResponse(value) {
    ...
    if (stale !== sourceErrors.length > 0)
        throw new Error("funding universe stale flag is inconsistent");
    return {
        engine: exact(row.engine, ["funding-curve-universe-v1"], "engine"),
...
function parseEconomicIdentityCatalog(value, updatedAt) {
    ...
    if (row.schemaVersion !== 1)
        throw new Error("economicIdentityCatalog.schemaVersion is unsupported");
    return {
        schemaVersion: 1,
...
function parseUniverseContract(value) {
    const row = strictRecord(value, "contract", ["owner", "adapterRegistry", "instruments", "execution"]);
    return {
        owner: exact(row.owner, ["server"], "contract.owner"),
        adapterRegistry: exact(row.adapterRegistry, ["publicVenueAdapters"], "contract.adapterRegistry"),
        instruments: exact(row.instruments, ["fresh-verified-trading-perpetuals"], "contract.instruments"),
        execution: exact(row.execution, ["none"], "contract.execution")
    };
}
function parseFundingUniverseInstrument(value, index, supportedVenues) {
...
export function parseFundingCurveResponse(value) {
    ...
    if (horizonEnd <= evaluatedAt)
        throw new Error("horizonEnd must follow evaluatedAt");
    ...
    return {
        engine: exact(row.engine, ["funding-curve-v1"], "engine"),
        readOnly: trueValue(row.readOnly, "readOnly"),
        researchOnly: trueValue(row.researchOnly, "researchOnly"),
        executable: falseValue(row.executable, "executable"),
        evaluatedAt,
...
function parseContract(value) {
    ...
    return {
        source: exact(row.source, ["credential-free-public-venue-adapters"], "contract.source"),
        rateUnit: exact(row.rateUnit, [RATE_UNIT], "contract.rateUnit"),
        stressUnit: exact(row.stressUnit, [STRESS_UNIT], "contract.stressUnit"),
        scheduleRequirement: exact(row.scheduleRequirement, ["adapter-verified-discrete-settlements"], "contract.scheduleRequirement"),
        projection: exact(row.projection, ["point-in-time-estimate-persistence"], "contract.projection"),
        pnl: exact(row.pnl, ["not-computed-without-explicit-notional-and-price-path"], "contract.pnl"),
        execution: exact(row.execution, ["none"], "contract.execution")
    };
}
function parseCrossVenueClock(value, curves) {
    ...
    if (comparedVenueCount < 2) {
        ...
        if (row.status !== "not-applicable" || row.eligible !== false || row.reason !== "fewer-than-two-successful-venues")
            throw new Error(`${label} not-applicable state is inconsistent`);
    ...
    if (calibratedVenueCount !== comparedVenueCount) {
        strictRecord(value, label, ["status", "eligible", "reason", "comparedVenueCount", "calibratedVenueCount", "maxSkewMs"]);
        if (row.status !== "blocked" || row.eligible !== false || row.reason !== "clock-not-calibrated")
            throw new Error(`${label} uncalibrated state is inconsistent`);
        return { status: "blocked", eligible: false, reason: "clock-not-calibrated", comparedVenueCount, calibratedVenueCount, maxSkewMs };
    }
    const calibrated = curves.filter((curve) => curve.freshness.clockBasis === "calibrated-venue-interval");
    ...
    if (!closeTiming(maximumPossibleSkewMs, expectedMaximumSkew))
        throw new Error(`${label} worst-case skew is inconsistent`);
    if (expectedMaximumSkew > maxSkewMs) {
        ...
        if (row.status !== "blocked" || row.eligible !== false || row.reason !== "skew-exceeded")
            throw new Error(`${label} skew-blocked state is inconsistent`);
    ...
    if (row.status !== "eligible" || row.eligible !== true || row.clockBasis !== "calibrated-venue-interval")
        throw new Error(`${label} eligible state is inconsistent`);
...
function parseCurve(value, index, evaluatedAt, horizonEnd) {
    ...
    assertStrictlyIncreasing(history.map(({ settlementAt }) => settlementAt), `${label}.history`);
    if (history.some(({ settlementAt }) => settlementAt > evaluatedAt + 60_000))
        throw new Error(`${label}.history contains a future point`);
    ...
    return {
        venue,
        instrumentId,
        marketType: exact(row.marketType, ["perpetual"], `${label}.marketType`),
        rateUnit: exact(row.rateUnit, [RATE_UNIT], `${label}.rateUnit`),
        rateSignConvention: exact(row.rateSignConvention, ["positive-longs-pay-shorts"], `${label}.rateSignConvention`),
        projectionSemantics: exact(row.projectionSemantics, ["rate-sum-only-no-notional-or-pnl"], `${label}.projectionSemantics`),
        freshness,
        schedule,
        current,
        history,
...
function parseFreshness(value, parent, evaluatedAt) {
    ...
    const common = { status: exact(row.status, ["fresh"], `${label}.status`), observedAt, ageMs, maxAgeMs };
    if (row.clockBasis === "local-receipt-fallback") {
        ...
        return {
            ...
            fallbackReason: exact(row.fallbackReason, ["clock-provider-unavailable", "clock-unavailable", "clock-not-calibrated", "source-declared-local-receipt"], `${label}.fallbackReason`)
        };
    }
    if (row.clockBasis !== "calibrated-venue-interval")
        throw new Error(`${label}.clockBasis is unsupported`);
...
function parseFundingClockLeg(value, label, evaluatedAt) {
...
function parseSchedule(value, parent) {
    ...
    if (fundingTime + interval * 60_000 !== nextFundingTime)
        throw new Error(`${label} timestamps are inconsistent`);
    return {
        verified: trueValue(row.verified, `${label}.verified`),
        interval,
        unit: exact(row.unit, ["minutes"], `${label}.unit`),
        fundingTime,
        nextFundingTime
    };
}
function parseCurrent(value, parent, fundingTime) {
    ...
    if (settlementAt !== fundingTime || !close(estimateRateBps, estimateRate * BPS))
        throw new Error(`${label} estimate provenance is inconsistent`);
...
function parseHistory(value, label) {
    ...
    const rateKind = exact(row.rateKind, ["estimate", "realized"], `${label}.rateKind`);
...
function parseSettlement(value, label) {
    ...
    if (!close(baseRateBps, baseRate * BPS))
        throw new Error(`${label} basis-point conversion is inconsistent`);
    return {
        settlementAt: positiveInteger(row.settlementAt, `${label}.settlementAt`),
        baseRate,
        baseRateBps,
        rateUnit: exact(row.rateUnit, [RATE_UNIT], `${label}.rateUnit`),
        rateSource: exact(row.rateSource, ["current-estimate", "next-estimate", "latest-estimate-persistence"], `${label}.rateSource`)
    };
}
function validateSettlements(points, current, schedule, evaluatedAt, horizonEnd, label) {
    assertStrictlyIncreasing(points.map(({ settlementAt }) => settlementAt), `${label}.settlements`);
    ...
    if (expectedCount > MAX_SETTLEMENTS || points.length !== expectedCount)
        throw new Error(`${label}.settlements do not cover the bounded horizon`);
    for (const [index, point] of points.entries()) {
        ...
        const expected = point.settlementAt === schedule.fundingTime
        ...
        if (!close(point.baseRate, expected.rate) || point.rateSource !== expected.source)
            throw new Error(`${label}.settlements[${index}] estimate provenance is inconsistent`);
        if (index > 0 && point.settlementAt - points[index - 1].settlementAt !== intervalMs)
            throw new Error(`${label}.settlements are not contiguous`);
    }
}
function parseScenario(value, label, settlements, minimumRate, maximumRate) {
    ...
    return {
        id,
        bumpBps,
        unit: exact(row.unit, [STRESS_UNIT], `${label}.unit`),
        settlementCount,
        cumulativeRate,
...
function parseSource(value, parent, venue, freshness, evaluatedAt) {
    const label = `${parent}.source`;
    const row = strictRecord(value, label, ["adapter", "operation", "public", "credentialed", "exchangeTs", "receivedAt", "formulaType", "method", "network", "currentEstimateSource", "timestampSource", "historyComplete", "sourceErrors", "sourceErrorsTruncated"], ["formulaType", "method", "network", "currentEstimateSource", "timestampSource"]);
    ...
    if (receivedAt > evaluatedAt + 60_000)
        throw new Error(`${label} receive timestamp is too far in the future`);
    if (freshness.clockBasis === "local-receipt-fallback") {
        if (receivedAt !== freshness.observedAt)
            throw new Error(`${label} receipt fallback provenance is inconsistent`);
    }
    ...
    const formulaType = optionalBoundedText(row.formulaType, `${label}.formulaType`);
    const method = optionalBoundedText(row.method, `${label}.method`);
    const network = row.network === undefined ? undefined : exact(row.network, ["mainnet", "testnet"], `${label}.network`);
    const currentEstimateSource = optionalBoundedText(row.currentEstimateSource, `${label}.currentEstimateSource`);
    const timestampSource = row.timestampSource === undefined ? undefined : exact(row.timestampSource, ["exchange", "local-receive"], `${label}.timestampSource`);
    ...
    if (historyComplete !== (sourceErrors.length === 0) || (sourceErrorsTruncated && sourceErrors.length !== 32))
        throw new Error(`${label} history completeness is inconsistent`);
    return {
        adapter: exact(row.adapter, ["publicVenueAdapters"], `${label}.adapter`),
        operation: exact(row.operation, ["funding"], `${label}.operation`),
...
function parseRejection(value, index) {
...
function strictRecord(value, label, keys, optional = []) {
    const row = record(value, label);
    ...
    for (const key of keys)
        if (!optionalSet.has(key) && row[key] === undefined)
...
function boundedNonBlankText(value, label, maximum) {
    const result = boundedText(value, label, maximum);
    if (!result.trim())
        throw new Error(`${label} must be non-empty`);
    return result;
}
function optionalBoundedText(value, label) {
    return value === undefined ? undefined : boundedText(value, label, 1_000);
}
function venueText(value, label) {
    const result = boundedText(value, label, 30);
    if (!/^[a-z0-9_-]{2,30}$/.test(result))
        throw new Error(`${label} is invalid`);
    return result;
}
function identifier(value, label) {
    const result = boundedText(value, label, 200);
    if (!/^(?:@[0-9]{1,6}|[A-Za-z0-9][A-Za-z0-9:._/@-]*)$/.test(result))
        throw new Error(`${label} is invalid`);
    return result;
}
function scenarioIdentifier(value, label) {
    const result = boundedText(value, label, 40);
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(result))
        throw new Error(`${label} is invalid`);
    return result;
}
function identity(venue, instrumentId) {
...
function closeTiming(left, right) {
