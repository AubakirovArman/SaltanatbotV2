...
const KINDS = ["basis", "triangular", "native-spread", "pairwise"];
const STATUSES = ["first-seen", "confirmed", "decaying", "expired"];
const QUALITIES = ["unverified", "degraded", "fresh", "verified"];
const REASONS = [
    "candidate-observed",
...
export function parseLifecycleResponse(value) {
    const row = record(value, "lifecycle response");
    if (row.schemaVersion !== 1 || row.readOnly !== true || row.executionPermission !== false)
        throw new Error("lifecycle response safety envelope is invalid");
    ...
    unique(universes.map(({ universeId }) => universeId), "universe IDs");
    unique(routes.map(({ key }) => key), "route keys");
    unique(events.map(({ id }) => id), "event IDs");
    unique(events.map(({ sequence }) => String(sequence)), "event sequences");
    if (events.some((event, index) => index > 0 && event.sequence >= events[index - 1].sequence))
        throw new Error("lifecycle events must be newest first");
    if (summary.universeCount !== universes.length || summary.returnedRoutes !== routes.length || summary.returnedEvents !== events.length)
        throw new Error("lifecycle summary does not match returned rows");
    if (summary.retainedRoutes < summary.matchedRoutes || summary.matchedRoutes < summary.returnedRoutes || summary.retainedEvents < summary.matchedEvents || summary.matchedEvents < summary.returnedEvents)
        throw new Error("lifecycle summary count hierarchy is invalid");
    if (summary.eventsTruncated !== summary.returnedEvents < summary.matchedEvents)
        throw new Error("lifecycle event truncation flag is inconsistent");
...
function parseRuntime(value) {
    ...
    if (lastError && lastError.length > 240)
        throw new Error("runtime.lastError is too long");
...
function parseUniverse(value, label) {
...
function parseRoute(value, label) {
    ...
    if (actionable && (status !== "confirmed" || !evidenceComplete || (effectiveEvidenceQuality !== "fresh" && effectiveEvidenceQuality !== "verified")))
        throw new Error(`${label}.actionable is inconsistent with lifecycle evidence`);
    ...
    if (firstSeenAt > lastSeenAt)
        throw new Error(`${label} timestamp order is invalid`);
    ...
    if (key !== `${universeId}\u001f${kind}\u001f${routeId}`)
        throw new Error(`${label}.key does not match its universe and route`);
    return {
        key,
        universeId,
...
function parseEvent(value, label) {
    ...
    return {
        ...
        type: exact(row.type, ["universe", "transition", "evidence-rejected"], `${label}.type`),
        universeId: identifier(row.universeId, `${label}.universeId`),
        policyId: identifier(row.policyId, `${label}.policyId`),
        ...(kind ? { kind } : {}),
        ...(routeId ? { routeId } : {}),
...
function optionalRouteTimes(row, label) {
    const values = {
...
function identifier(value, label) {
    ...
    if (parsed.length > 256 || !/^[A-Za-z0-9][A-Za-z0-9:._/@#|+=>-]*$/.test(parsed))
