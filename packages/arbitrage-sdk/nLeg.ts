import type { NLegAssetUnit, NLegGraphWork, NLegLegSimulation, NLegMetadataRejection, NLegOpportunity, NLegRejection, NLegResearchResponse, NLegResidual } from "./nLegTypes.js";
import { array, bool, exact, finite, integer, nonNegative, positive, record, text } from "./validation.js";

const REJECTION_CODES = ["missing-market", "missing-book", "identity-mismatch", "invalid-book", "incomplete-book", "unsequenced-book", "stale-book", "skewed-books", "fee-conservation", "minimum-quantity", "minimum-notional", "insufficient-depth", "work-limit", "non-profitable"] as const;

/** Strict parser for the bounded, credential-free N-leg research response. */
export function parseNLegResearchResponse(value: unknown): NLegResearchResponse {
  const row = record(value, "N-leg response");
  rejectExecutionFields(row, "N-leg response");
  const envelope = {
    engine: exact(row.engine, ["n-leg-v1"] as const, "N-leg engine"),
    ...
    execution: exact(row.execution, ["none"] as const, "N-leg execution")
  };
  ...
  if (new Set(cycleIds).size !== cycleIds.length) throw new Error("N-leg response contains duplicate cycle outcomes");
  if (graph.truncationReason === "cycle-limit" && totalCycles !== graph.maxCycles) throw new Error("N-leg cycle-limit proof is inconsistent");
  if (opportunities.some((item, index) => index > 0 && compareOpportunity(opportunities[index - 1]!, item) > 0)) throw new Error("N-leg opportunities are not deterministically ranked");
...
function parseGraph(value: unknown): NLegGraphWork {
  ...
  const truncationReason = row.truncationReason === undefined ? undefined : exact(row.truncationReason, ["cycle-limit", "traversal-work-limit"] as const, "N-leg graph.truncationReason");
  if (marketCount > maxMarkets || traversalSteps > maxTraversalSteps) throw new Error("N-leg graph work exceeds its declared bound");
  if (truncated !== Boolean(truncationReason)) throw new Error("N-leg graph truncation proof is inconsistent");
  if (truncationReason === "traversal-work-limit" && traversalSteps !== maxTraversalSteps) throw new Error("N-leg traversal-limit proof is inconsistent");
  return { marketCount, maxMarkets, traversalSteps, maxTraversalSteps, maxCycles, truncated, ...(truncationReason ? { truncationReason } : {}) };
}

function parseMetadataRejection(value: unknown): NLegMetadataRejection {
  const row = record(value, "N-leg metadata rejection");
  return {
    instrumentId: text(row.instrumentId, "N-leg metadata rejection.instrumentId"),
    code: exact(row.code, ["invalid-metadata", "duplicate-instrument", "fee-conservation"] as const, "N-leg metadata rejection.code"),
    message: text(row.message, "N-leg metadata rejection.message")
  };
}

function parseRejection(value: unknown): NLegRejection {
  ...
  if ((legIndex === undefined) !== (instrumentId === undefined)) throw new Error("N-leg rejection leg provenance is incomplete");
  return {
...
function parseOpportunity(value: unknown, opportunityIndex: number, evaluatedAt: number, requestedStartQuantity: number, expectedStart: NLegAssetUnit): NLegOpportunity {
  ...
  if (id !== `n-leg-opportunity:${cycleId}`) throw new Error(`${label}.id is inconsistent with cycleId`);
  ...
  if (startKey !== assetKey(start) || assetKey(start) !== assetKey(expectedStart)) throw new Error(`${label}.start identity is inconsistent`);
  const opportunityRequested = positive(row.requestedStartQuantity, `${label}.requestedStartQuantity`);
  if (!close(opportunityRequested, requestedStartQuantity)) throw new Error(`${label}.requestedStartQuantity differs from the response envelope`);
  ...
  if (depthLimited !== (limitingLegIndex !== undefined && limitingInstrumentId !== undefined)) throw new Error(`${label}.depth limit provenance is inconsistent`);
  const legs = array(row.legs, `${label}.legs`, 8).map((item, index) => parseLeg(item, index, label));
  if (legs.length !== legCount) throw new Error(`${label}.legCount is inconsistent`);
  ...
  if (capacityUtilizationPct > 100 + 1e-7 || legs.some((leg) => leg.venue !== venue)) throw new Error(`${label} venue or capacity provenance is inconsistent`);
  return {
    id,
    strategyKind: exact(row.strategyKind, ["n-leg-cycle"] as const, `${label}.strategyKind`),
    ...
    executionModel: exact(row.executionModel, ["sequential-visible-depth"] as const, `${label}.executionModel`),
    cycleId,
    venue,
    legCount,
    start,
...
function parseLeg(value: unknown, index: number, parent: string): NLegLegSimulation {
  ...
  if (fromKey !== assetKey(from) || toKey !== assetKey(to) || feeAssetKey !== assetKey(feeAsset)) throw new Error(`${label} asset keys are inconsistent`);
  ...
  const feeDebit = exact(row.feeDebit, ["input", "output"] as const, `${label}.feeDebit`);
  ...
  if (feeBps >= 10_000 || !close(inputQuantity, totalInputDebitedQuantity + inputDustQuantity)) throw new Error(`${label} input conservation is inconsistent`);
  if (!close(feeQuantity, (feeDebit === "input" ? tradeInputQuantity : grossOutputQuantity) * (feeBps / 10_000))) throw new Error(`${label} fee arithmetic is inconsistent`);
  if (!close(outputQuantity, grossOutputQuantity - (feeDebit === "output" ? feeQuantity : 0))) throw new Error(`${label} output conservation is inconsistent`);
  return {
    ...
    side: exact(row.side, ["buy", "sell"] as const, `${label}.side`),
    from,
    to,
    fromKey,
    toKey,
...
function parseResidual(value: unknown, index: number, legs: NLegLegSimulation[], parent: string): NLegResidual {
  ...
  return { legIndex, asset, assetKey: assetKeyValue, quantity, reason: exact(row.reason, ["lot-rounding"] as const, `${label}.reason`) };
}

function parseTimestamps(value: unknown, legs: NLegLegSimulation[], evaluatedAt: number, label: string): NLegOpportunity["timestamps"] {
...
function parseProvenance(value: unknown, legs: NLegLegSimulation[], cycleId: string, label: string): NLegOpportunity["provenance"] {
  ...
  if (cycleId !== `n-leg:${canonicalSignature}`) throw new Error(`${label}.cycleId is inconsistent with canonical signature`);
  ...
  if (
    !sameArray(
      instrumentIds,
      legs.map(({ instrumentId }) => instrumentId)
    ) ||
    !sameArray(
      feeScheduleIds,
      legs.map(({ feeScheduleId }) => feeScheduleId)
    )
  ) {
    throw new Error(`${label}.provenance order is inconsistent with legs`);
  }
  return { engine: exact(row.engine, ["n-leg-v1"] as const, `${label}.provenance.engine`), canonicalSignature, instrumentIds, feeScheduleIds, bookSourceIds };
}

function assertLegChain(legs: NLegLegSimulation[], startKey: string, startQuantity: number, endQuantity: number, label: string) {
...
function assertAggregates(legs: NLegLegSimulation[], residuals: NLegResidual[], dust: Record<string, number>, fees: Record<string, number>, label: string) {
  ...
  if (!sameQuantityMap(dust, expectedDust) || !sameQuantityMap(fees, expectedFees)) throw new Error(`${label} dust or fee aggregates are inconsistent`);
}

function parseAsset(value: unknown, label: string): NLegAssetUnit {
  const row = record(value, label);
  const asset = { venue: text(row.venue, `${label}.venue`), assetId: text(row.assetId, `${label}.assetId`), unitId: text(row.unitId, `${label}.unitId`) };
  if (asset.venue !== asset.venue.toLowerCase() || asset.assetId !== asset.assetId.toUpperCase() || asset.unitId !== asset.unitId.toUpperCase()) throw new Error(`${label} is not normalized`);
  return asset;
}

function parseQuantityMap(value: unknown, label: string): Record<string, number> {
  const row = record(value, label);
  if (Object.keys(row).length > 16) throw new Error(`${label} exceeds its bounded key count`);
  return Object.fromEntries(Object.entries(row).map(([key, quantity]) => [key, nonNegative(quantity, `${label}.${key}`)]));
}

function textArray(value: unknown, label: string, length: number) {
  const result = array(value, label, 8).map((item, index) => text(item, `${label}[${index}]`));
  if (result.length !== length) throw new Error(`${label} length is inconsistent`);
  return result;
}

function assetKey(value: NLegAssetUnit) {
...
function exactInteger(value: unknown, expected: number, label: string) {
  const result = integer(value, label);
  if (result !== expected) throw new Error(`${label} is inconsistent`);
  return result;
}
...
function sumByKey(rows: Array<{ key: string; quantity: number }>) {
  const result: Record<string, number> = {};
  for (const { key, quantity } of rows) result[key] = (result[key] ?? 0) + quantity;
  return result;
}

function sameQuantityMap(left: Record<string, number>, right: Record<string, number>) {
...
function compareOpportunity(left: NLegOpportunity, right: NLegOpportunity) {
  return right.netReturnBps - left.netReturnBps || right.startQuantity - left.startQuantity || left.id.localeCompare(right.id);
}

function rejectExecutionFields(row: Record<string, unknown>, label: string) {
  for (const key of ["order", "orders", "apiKey", "apiSecret", "secret", "credentials", "submit", "placeOrder"]) {
    if (key in row) throw new Error(`${label} contains forbidden execution material`);
  }
}
