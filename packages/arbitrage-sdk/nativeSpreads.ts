import type { NativeSpreadOpportunity, NativeSpreadScan } from "./types.js";
import { array, bool, exact, finite, integer, optionalFinite, positive, record, text } from "./validation.js";

const REQUIRED_RISK_FLAGS = ["read-only", "top-book-only", "venue-native-combination", "revalidate-before-order"] as const;
const MAX_BOOK_AGE_MS = 10_000;
const MAX_FUTURE_SKEW_MS = 2_000;

export function parseNativeSpreadScan(value: unknown): NativeSpreadScan {
  ...
  if (new Set(opportunities.map(({ id }) => id)).size !== opportunities.length) throw new Error("native opportunity ids must be unique");
  if (new Set(opportunities.map(({ symbol }) => symbol)).size !== opportunities.length) throw new Error("native opportunity symbols must be unique");

  return {
    venue: exact(row.venue, ["bybit"] as const, "venue"),
    marketDataMode: exact(row.marketDataMode, ["venue-native-spread-orderbook"] as const, "marketDataMode"),
    executionModel: exact(row.executionModel, ["venue-matched-multi-leg"] as const, "executionModel"),
    readOnly: true,
    updatedAt,
    totalInstruments,
    eligibleInstruments,
...
function parseNativeOpportunity(value: unknown, index: number, updatedAt: number): NativeSpreadOpportunity {
  ...
  if (rawLegs.length !== 2) throw new Error("native spread requires exactly two legs");
  const legs = rawLegs.map((leg, legIndex) => parseLeg(leg, `${label}.legs[${legIndex}]`)) as NativeSpreadOpportunity["legs"];
  if (legs[0].symbol === legs[1].symbol && legs[0].contractType === legs[1].contractType) throw new Error(`${label}.legs must be distinct`);
  ...
  if (quoteAgeMs !== actualQuoteAgeMs) throw new Error(`${label}.quoteAgeMs must equal updatedAt - exchangeTs`);
  if (actualQuoteAgeMs > MAX_BOOK_AGE_MS) throw new Error(`${label} exchange timestamp exceeds the native-spread freshness gate`);
...
function parseLeg(value: unknown, label: string): NativeSpreadOpportunity["legs"][number] {
  const row = record(value, label);
  return {
...
function parseRiskFlags(value: unknown, label: string): NativeSpreadOpportunity["riskFlags"] {
