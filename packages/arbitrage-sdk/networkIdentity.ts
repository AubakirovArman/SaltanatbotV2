import type {
  NetworkAssetIdentity,
  NetworkAssetRepresentation,
  NetworkIdentityAsset,
  NetworkIdentityEvidence,
  NetworkIdentityNetwork,
  NetworkIdentityRegistryDocument,
  NetworkIdentityRegistryResponse,
  NetworkTransferCompatibilityResult,
  NetworkTransferFailureCode,
  VenueNetworkMapping,
  VenueTransferCapability
} from "./networkIdentityTypes.js";
...
] as const satisfies readonly NetworkTransferFailureCode[];

/** Strict bounded parser for the server-owned identity registry envelope. */
export function parseNetworkIdentityRegistryResponse(value: unknown): NetworkIdentityRegistryResponse {
  ...
  if (row.schemaVersion !== 1) throw new Error("network identity response schemaVersion is unsupported");
  ...
  return {
    schemaVersion: 1,
    modelVersion: exact(row.modelVersion, ["network-identity-registry-v1"] as const, "modelVersion"),
...
function registryEvidence(registry: NetworkIdentityRegistryDocument): NetworkIdentityEvidence[] {
  return [
    registry.evidence,
...
function parseValidity(value: unknown, evaluatedAt: number): NetworkIdentityRegistryResponse["validity"] {
  ...
  const reason = exact(row.reason, ["current", "not-yet-valid", "expired"] as const, "validity.reason");
  const status = exact(row.status, ["current", "stale"] as const, "validity.status");
  const expectedReason = evaluatedAt < asOf ? "not-yet-valid" : evaluatedAt > validUntil ? "expired" : "current";
  if (reason !== expectedReason || status !== (reason === "current" ? "current" : "stale")) throw new Error("validity status is inconsistent with evaluatedAt");
  const remainingMs = boundedInteger(row.remainingMs, "validity.remainingMs", 0, Number.MAX_SAFE_INTEGER);
  if (remainingMs !== Math.max(0, validUntil - evaluatedAt)) throw new Error("validity.remainingMs is inconsistent");
  return { status, reason, asOf, validUntil, remainingMs };
}

/** Strict bounded parser for the non-executable transfer preflight result. */
export function parseNetworkTransferCompatibilityResult(value: unknown): NetworkTransferCompatibilityResult {
  const optional = ["assetId", "networkId", "networkAssetId", "sourceMappingId", "destinationMappingId", "grossAmount", "feeAmount", "minimumArrivalAmount", "estimatedArrivalMs", "requiredConfirmations", "safeConfirmations"] as const;
  ...
  if (row.schemaVersion !== 1) throw new Error("network transfer compatibility schemaVersion is unsupported");
  ...
  if (!evidenceIds.every((item, index) => index === 0 || evidenceIds[index - 1]!.localeCompare(item) <= 0)) throw new Error("evidenceIds must be sorted");
  ...
  if (compatible !== (failures.length === 0)) throw new Error("compatible must match the absence of failures");
  const result: NetworkTransferCompatibilityResult = {
    schemaVersion: 1,
    modelVersion: exact(row.modelVersion, ["network-transfer-compatibility-v1"] as const, "modelVersion"),
  ...
  for (const field of ["assetId", "networkId", "networkAssetId", "sourceMappingId", "destinationMappingId"] as const) {
    if (row[field] !== undefined) result[field] = identifier(row[field], field);
  }
  for (const field of ["grossAmount", "feeAmount", "minimumArrivalAmount"] as const) {
    if (row[field] !== undefined) result[field] = decimal(row[field], field);
  }
  for (const field of ["estimatedArrivalMs", "requiredConfirmations", "safeConfirmations"] as const) {
    if (row[field] !== undefined) result[field] = boundedInteger(row[field], field, 0, Number.MAX_SAFE_INTEGER);
  }
  return result;
}

function parseRegistry(value: unknown): NetworkIdentityRegistryDocument {
  ...
  if (row.schemaVersion !== 1) throw new Error("registry.schemaVersion is unsupported");
  ...
  unique(
    networks.map(({ networkId }) => networkId),
    "network IDs"
  );
  unique(
    networkAssets.map(({ networkAssetId }) => networkAssetId),
    "network asset IDs"
  );
  unique(
    venueMappings.map(({ mappingId }) => mappingId),
    "mapping IDs"
  );
  unique(
    transferCapabilities.map(({ mappingId }) => mappingId),
    "capability mapping IDs"
  );
  ...
  for (const item of networkAssets) {
    if (!assetIds.has(item.assetId) || !networkIds.has(item.networkId)) throw new Error("network asset contains an unknown identity reference");
  }
  for (const item of venueMappings) {
    const networkAsset = networkAssetById.get(item.networkAssetId);
    if (!assetIds.has(item.assetId) || !networkAsset || networkAsset.assetId !== item.assetId) throw new Error("venue mapping contains an unknown or mismatched identity reference");
  }
  for (const item of transferCapabilities) if (!mappingIds.has(item.mappingId) || !assetIds.has(item.fee.feeAssetId)) throw new Error("transfer capability contains an unknown identity reference");
  return {
    schemaVersion: 1,
...
function parseEvidence(value: unknown, label: string): NetworkIdentityEvidence {
  ...
  if (validUntil <= asOf) throw new Error(`${label}.validUntil must follow asOf`);
  return {
...
function parseAsset(value: unknown, index: number): NetworkIdentityAsset {
  const label = `registry.assets[${index}]`;
  const row = strictRecord(value, label, ["assetId", "symbol", "kind", "evidence"], ["underlyingAssetId"]);
  const kind = exact(row.kind, ["native", "wrapped"] as const, `${label}.kind`);
  ...
  if ((kind === "wrapped") !== (underlyingAssetId !== undefined)) throw new Error(`${label} wrapped identity is inconsistent`);
  return {
...
function parseNetwork(value: unknown, index: number): NetworkIdentityNetwork {
  ...
  return {
    ...
    finalityModel: exact(row.finalityModel, ["deterministic", "probabilistic", "external"] as const, `${label}.finalityModel`),
    reorgSensitive: bool(row.reorgSensitive, `${label}.reorgSensitive`),
    evidence: parseEvidence(row.evidence, `${label}.evidence`)
  };
}

function parseNetworkAsset(value: unknown, index: number): NetworkAssetIdentity {
...
function parseRepresentation(value: unknown, label: string): NetworkAssetRepresentation {
  const base = record(value, label);
  const kind = exact(base.kind, ["native", "token-contract", "wrapped"] as const, `${label}.kind`);
  if (kind === "native") {
...
function parseTokenContract(value: unknown, label: string) {
...
function parseMapping(value: unknown, index: number): VenueNetworkMapping {
  ...
  const requirement = exact(memo.requirement, ["none", "optional", "required"] as const, `${label}.memo.requirement`);
  const memoType = memo.memoType === undefined ? undefined : boundedText(memo.memoType, `${label}.memo.memoType`, 240);
  if (requirement === "required" && memoType === undefined) throw new Error(`${label}.memoType is required`);
  if (requirement === "none" && memoType !== undefined) throw new Error(`${label}.memoType is forbidden`);
  return {
...
function parseCapability(value: unknown, index: number): VenueTransferCapability {
  ...
  if (safe < required) throw new Error(`${label}.confirmations.safe must cover required`);
  return {
    mappingId: identifier(row.mappingId, `${label}.mappingId`),
    status: {
      deposit: exact(status.deposit, ["enabled", "disabled", "maintenance", "unknown"] as const, `${label}.status.deposit`),
      withdrawal: exact(status.withdrawal, ["enabled", "disabled", "maintenance", "unknown"] as const, `${label}.status.withdrawal`),
...
function strictRecord(value: unknown, label: string, required: readonly string[], optional: readonly string[] = []): Record<string, unknown> {
  ...
  for (const key of Object.keys(row)) if (!allowed.has(key)) throw new Error(`${label}.${key} is not allowed`);
  for (const key of required) if (!Object.hasOwn(row, key)) throw new Error(`${label}.${key} is required`);
  return row;
}

function identifier(value: unknown, label: string): string {
  const result = boundedText(value, label, 200);
  if (!/^[A-Za-z0-9][A-Za-z0-9:._/@-]*$/.test(result)) throw new Error(`${label} is not a valid identifier`);
  return result;
}

function decimal(value: unknown, label: string): string {
  const result = boundedText(value, label, 80);
  if (!/^(0|[1-9][0-9]*)(?:\.[0-9]+)?$/.test(result)) throw new Error(`${label} is not a decimal`);
  return result;
}

function boundedText(value: unknown, label: string, maximum: number): string {
  const result = text(value, label).trim();
  if (result.length < 1 || result.length > maximum) throw new Error(`${label} has an invalid length`);
  return result;
}

function boundedInteger(value: unknown, label: string, minimum: number, maximum: number): number {
  const result = integer(value, label);
  if (result < minimum || result > maximum) throw new Error(`${label} is outside its bounds`);
  return result;
}
