...
const STRATEGIES = ["put-call-parity", "conversion", "reversal", "box", "synthetic-forward"];
const DIRECTIONS = ["call-rich", "put-rich", "long-box", "short-box", "long-synthetic", "short-synthetic"];
...
export function parseOptionsParityEvaluation(value) {
    const row = strictRecord(value, "options-parity evaluation", ["engine", "readOnly", "researchOnly", "executable", "evaluatedAt", "edgeKind", "assumptionContract", "candidates", "rejections"]);
    const evaluatedAt = positiveSafeInteger(row.evaluatedAt, "evaluatedAt");
    const candidates = array(row.candidates, "candidates", 16).map((candidate, index) => parseCandidate(candidate, evaluatedAt, `candidates[${index}]`));
    const rejections = array(row.rejections, "rejections", 64).map((rejection, index) => parseRejection(rejection, `rejections[${index}]`));
    assertUnique(candidates.map((candidate) => candidate.id), "candidate IDs");
    assertCandidateOrder(candidates);
    return {
        engine: exact(row.engine, ["options-parity-v1"], "engine"),
        ...
        evaluatedAt,
        edgeKind: exact(row.edgeKind, ["research-simulation"], "edgeKind"),
...
function parseContract(value) {
    ...
    for (const [key, expected] of Object.entries(CONTRACT)) {
        if (row[key] !== expected)
            throw new Error(`assumptionContract.${key} is unsupported`);
    }
    return { ...CONTRACT };
}
function parseCandidate(value, evaluatedAt, label) {
    ...
    if (legs.length < 2)
        throw new Error(`${label}.legs must contain at least two legs`);
    ...
    if (strikes.length < 1 || !strictlyIncreasing(strikes))
        throw new Error(`${label}.strikes must be non-empty, unique and increasing`);
    const assumptionSources = array(row.assumptionSources, `${label}.assumptionSources`, 32).map((source, index) => boundedText(source, `${label}.assumptionSources[${index}]`));
    ...
    const candidate = {
        ...
        simulationBasis: exact(row.simulationBasis, ["visible-depth-taker"], `${label}.simulationBasis`),
        outcomeLabel: exact(row.outcomeLabel, [
...
function parseLeg(value, label) {
    ...
    const bookSide = exact(row.bookSide, ["asks", "bids"], `${label}.bookSide`);
    ...
    if (levelsUsed < 1 || levelsUsed > 400)
        throw new Error(`${label}.levelsUsed is outside the bounded book depth`);
    return {
        role: exact(row.role, ["call", "put", "underlying"], `${label}.role`),
...
function parseTimestamps(value, label) {
    const row = strictRecord(value, label, [
        "evaluatedAt",
        "oldestExchangeTs",
        "newestExchangeTs",
...
function parseRejection(value, label) {
...
function validateCandidate(candidate, evaluatedAt, label) {
    if (candidate.timestamps.evaluatedAt !== evaluatedAt)
        throw new Error(`${label}.timestamps.evaluatedAt does not match the envelope`);
    if (candidate.expiryTime <= evaluatedAt)
        throw new Error(`${label}.expiryTime is not in the future`);
    if (candidate.settlementAsset !== candidate.valuationAsset)
        throw new Error(`${label} requires an unmodelled settlement FX conversion`);
    ...
    for (const leg of candidate.legs)
        assertApproximately(leg.baseQuantity, candidate.baseQuantity, `${label} leg baseQuantity`);
    validateStrategyShape(candidate, label);
    validateTimestamps(candidate, evaluatedAt, label);
}
function validateStrategyShape(candidate, label) {
    const specs = {
    ...
    if (fixed !== spec.fixed || reference !== spec.reference)
        throw new Error(`${label} payoff/reference shape is inconsistent`);
    const expectedOutcome = spec.fixed
        ? "fixed-valuation-payoff-at-expiry-under-stated-assumptions"
        : "parity-deviation-research-only-no-fixed-profit-without-hedge";
    if (candidate.outcomeLabel !== expectedOutcome)
        throw new Error(`${label}.outcomeLabel is inconsistent with strategy`);
    const forwardFields = candidate.theoreticalForwardPrice !== undefined && candidate.impliedForwardPrice !== undefined;
    if (forwardFields !== spec.reference)
        throw new Error(`${label} forward-price fields are inconsistent with strategy`);
    if (candidate.referenceUnderlying && candidate.referenceUnderlying.role !== "underlying")
        throw new Error(`${label}.referenceUnderlying has an invalid role`);
    if (candidate.strategyKind !== "reversal" && candidate.borrowCostValue !== 0)
        throw new Error(`${label}.borrowCostValue is only valid for reversal`);
    ...
    if (roles.join("|") !== expectedRoles.join("|"))
        throw new Error(`${label} leg roles are inconsistent with strategy`);
}
function validateTimestamps(candidate, evaluatedAt, label) {
    ...
    if (timestamps.oldestExchangeTs !== Math.min(...exchange) ||
    ...
    if (timestamps.quoteAgeMs !== expectedAge || timestamps.legSkewMs !== expectedSkew)
        throw new Error(`${label} quote age/skew is inconsistent`);
...
function boundedText(value, label, maximum = 200) {
    ...
    if (parsed.length > maximum || parsed.trim() !== parsed)
        throw new Error(`${label} must be trimmed and at most ${maximum} characters`);
    return parsed;
}
...
function strictlyLexical(values) {
...
function assertCandidateOrder(candidates) {
    for (let index = 1; index < candidates.length; index += 1) {
        const previous = candidates[index - 1];
        const current = candidates[index];
        if (current.netEdgeValue > previous.netEdgeValue ||
