...
const PAIRWISE_STRATEGIES = ["spot-spot", "perpetual-perpetual", "reverse-cash-and-carry", "spot-dated-future", "perpetual-future", "calendar-spread", "dated-futures-spread"];
...
export function assertPairwiseRequestEconomicIdentity(value) {
    ...
    if (instruments.length !== 2)
        throw new Error("pairwise request requires exactly two instruments");
...
export function parsePairwiseEvaluation(value) {
    const row = record(value, "pairwise evaluation");
    const envelope = {
        engine: exact(row.engine, ["pairwise-v1"], "engine"),
    ...
    if (hasOpportunity === hasRejection)
        throw new Error("pairwise evaluation must contain exactly one opportunity or rejection");
    if (hasOpportunity) {
        const opportunity = parsePairwiseOpportunity(row.opportunity, envelope.evaluatedAt);
        if (opportunity.timestamps.evaluatedAt !== envelope.evaluatedAt)
            throw new Error("pairwise evaluatedAt must match opportunity timestamps");
        return { ...envelope, opportunity };
    }
    return { ...envelope, rejection: parsePairwiseRejection(row.rejection) };
}
function parsePairwiseOpportunity(value, evaluatedAt) {
    const row = record(value, "pairwise opportunity");
    const rawLegs = array(row.legs, "pairwise opportunity.legs", 2);
    if (rawLegs.length !== 2)
        throw new Error("pairwise opportunity.legs must contain exactly two legs");
    const legs = [parsePairwiseLeg(rawLegs[0], "long"), parsePairwiseLeg(rawLegs[1], "short")];
    const routeId = text(row.routeId, "routeId");
    const provenance = parsePairwiseProvenance(row.provenance, evaluatedAt);
    if (provenance.routeId !== routeId)
        throw new Error("pairwise provenance.routeId must match opportunity.routeId");
    ...
    if (id !== `pairwise:${routeId}`)
        throw new Error("pairwise opportunity id must match routeId");
...
function parsePairwiseLeg(value, expectedRole) {
    const row = record(value, `${expectedRole} pairwise leg`);
    const role = exact(row.role, ["long", "short"], "role");
    if (role !== expectedRole)
        throw new Error("pairwise legs must preserve long/short order");
    const side = exact(row.side, ["buy", "sell"], "side");
    const bookSide = exact(row.bookSide, ["asks", "bids"], "bookSide");
    ...
    return {
        ...
        marketType: exact(row.marketType, ["spot", "perpetual", "future"], "marketType"),
        side,
        bookSide,
        nativeQuantity: nonNegative(row.nativeQuantity, "nativeQuantity"),
        quantityUnit: exact(row.quantityUnit, ["base", "quote", "contract"], "quantityUnit"),
...
function parsePairwiseCosts(value) {
...
function parsePairwiseTimestamps(value) {
...
function validatePairwiseTimestampSemantics(legs, provenance, timestamps, evaluatedAt) {
    ...
    if (timestamps.evaluatedAt !== evaluatedAt ||
    ...
    if (provenance.assumptions.length === 0)
        throw new Error("pairwise provenance requires timestamped assumptions");
...
function assertApproximately(actual, expected, label) {
    ...
    if (Math.abs(actual - expected) > tolerance)
        throw new Error(`pairwise ${label} is inconsistent`);
}
function parsePairwiseProvenance(value, evaluatedAt) {
    ...
    if (rawMetadataIds.length !== 2 || rawBooks.length !== 2)
        throw new Error("pairwise provenance requires exactly two metadata and book records");
    return {
        engine: exact(row.engine, ["pairwise-v1"], "provenance.engine"),
...
function parseEconomicIdentityProvenance(value, evaluatedAt) {
    const row = record(value, "pairwise economic identity provenance");
    const rawLegs = array(row.legs, "economicIdentity.legs", 2);
    if (rawLegs.length !== 2)
        throw new Error("pairwise economic identity provenance requires exactly two legs");
    ...
    const parsedLegs = rawLegs.map((value, index) => {
        ...
        if (effectiveValidUntil !== expectedBoundary)
            throw new Error("pairwise economic identity effective validity boundary is inconsistent");
        if (review.asOf > evaluatedAt + maxFutureClockSkewMs)
            throw new Error("pairwise economic identity asOf exceeds its future-clock boundary");
        if (evaluatedAt > effectiveValidUntil)
            throw new Error("pairwise economic identity provenance is stale or expired");
    ...
    return {
        ...
        matchPolicy: exact(row.matchPolicy, ["exact"], "economicIdentity.matchPolicy"),
        authority: exact(row.authority, ["caller-supplied"], "economicIdentity.authority"),
        maxAgeMs,
        maxFutureClockSkewMs,
        legs: [parsedLegs[0], parsedLegs[1]]
    };
}
function parseEconomicIdentityReview(value, label) {
    ...
    if (validUntil < asOf)
        throw new Error(`${label}.validUntil must be at or after asOf`);
    return {
...
function parseEconomicAssetId(value, label) {
    const parsed = nonBlankText(value, label);
    if (!ECONOMIC_ASSET_ID.test(parsed))
        throw new Error(`${label} must use the lowercase namespace:value format`);
    return parsed;
}
...
function parsePairwiseBookProvenance(value) {
    ...
    return {
        instrumentId: text(row.instrumentId, "instrumentId"),
        source: exact(row.source, ["websocket", "rest", "fixture"], "source"),
...
function parsePairwiseRejection(value) {
