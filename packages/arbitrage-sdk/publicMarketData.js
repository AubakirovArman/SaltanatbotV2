...
export function parsePublicVenueTopBook(value, requireReadOnly = true) {
    const row = record(value, "public venue top book");
    if (requireReadOnly && row.readOnly !== true)
        throw new Error("public venue top book must be read-only");
    const bid = positive(row.bid, "bid");
    const ask = positive(row.ask, "ask");
    if (bid >= ask)
        throw new Error("public venue top book must have bid below ask");
...
export function parsePublicVenueInstruments(value) {
...
export function parsePublicVenueTickers(value) {
...
export function parsePublicVenueDepth(value) {
    const row = record(value, "public venue depth");
    if (row.readOnly !== true)
        throw new Error("public venue depth must be read-only");
    if (row.complete !== true)
        throw new Error("public venue depth must be complete");
    const bids = levels(row.bids, "bid", true);
    const asks = levels(row.asks, "ask", false);
    if (bids[0][0] >= asks[0][0])
        throw new Error("public venue depth must not be crossed or locked");
    const sequence = integer(row.sequence, "sequence");
    const sequenceVerified = optionalBoolean(row.sequenceVerified, "sequenceVerified");
    if (sequenceVerified === true && sequence === 0)
        throw new Error("verified sequence must be positive");
    const source = optionalText(row.source, "source");
    return {
        readOnly: true,
        venue: text(row.venue, "venue"),
...
export function parsePublicVenueFunding(value) {
    ...
    const network = row.network === undefined ? undefined : exact(row.network, ["mainnet", "testnet"], "network");
    const currentEstimateSource = optionalText(row.currentEstimateSource, "currentEstimateSource");
    const timestampSource = row.timestampSource === undefined ? undefined : exact(row.timestampSource, ["exchange", "local-receive"], "timestampSource");
    return {
        readOnly: true,
        venue: text(row.venue, "venue"),
        marketType: exact(row.marketType, ["perpetual"], "marketType"),
        instrumentId: text(row.instrumentId, "instrumentId"),
...
function levels(value, label, descending) {
    const parsed = array(value, `${label}s`, 400).map((level) => parseLevel(level, label));
    if (parsed.length === 0)
        throw new Error(`public venue depth requires at least one ${label}`);
    for (let index = 1; index < parsed.length; index += 1) {
        const ordered = descending ? parsed[index - 1][0] > parsed[index][0] : parsed[index - 1][0] < parsed[index][0];
        if (!ordered)
            throw new Error(`public venue ${label}s must be strictly price sorted`);
    }
    return parsed;
}
function parseLevel(value, label) {
    ...
    if (level.length < 2)
        throw new Error(`${label} level requires price and quantity`);
    const orderCount = level[2] === undefined ? undefined : integer(level[2], `${label} orderCount`);
    const result = [positive(level[0], `${label} price`), positive(level[1], `${label} quantity`)];
    return orderCount === undefined ? result : [result[0], result[1], orderCount];
}
function parseFundingPoint(value) {
...
function parseValidationIssue(value) {
...
function readOnlyRecord(value, label) {
...
function marketType(value) {
    return exact(value, ["spot", "margin", "perpetual", "future", "option", "native-spread"], "marketType");
}
function quantityUnit(value) {
    return exact(value, ["base", "quote", "contract"], "quantityUnit");
}
