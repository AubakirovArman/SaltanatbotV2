...
import type { InstrumentRegistryResponse, InstrumentRegistrySourceState, VenueCapabilitiesResponse } from "./types.js";
...
export function parseInstrumentRegistry(value: unknown): InstrumentRegistryResponse {
  ...
  if (total < instruments.length) throw new Error("instrument registry total cannot be smaller than returned instruments");
  if (truncated !== total > instruments.length) throw new Error("instrument registry truncated must match total and returned count");
  assertUnique(
    instruments.map((instrument) => instrument.id),
    "instrument registry IDs"
  );
  return {
    ...envelope,
...
export function parseVenueCapabilities(value: unknown): VenueCapabilitiesResponse {
...
export function parseRegistryInstrument(value: unknown): RegistryInstrument {
  const row = record(value, "instrument");
  const result: RegistryInstrument = {
    ...
    marketType: exact(row.marketType, ["spot", "margin", "perpetual", "future", "option", "native-spread"] as const, "marketType"),
    ...
    status: exact(row.status, ["trading", "prelaunch", "settling", "closed"] as const, "status")
  };
  const priceRules = parsePriceRules(row.priceRules);
  if (result.tickSize === 0 && !priceRules) throw new Error("tickSize must be positive unless dynamic priceRules are supplied");
  ...
  const contractDirection = row.contractDirection === undefined ? undefined : exact(row.contractDirection, ["linear", "inverse", "quanto"] as const, "contractDirection");
  const quantityUnit = row.quantityUnit === undefined ? undefined : exact(row.quantityUnit, ["base", "quote", "contract"] as const, "quantityUnit");
  const optionType = row.optionType === undefined ? undefined : exact(row.optionType, ["call", "put"] as const, "optionType");
  return {
    ...result,
...
function parseRegistryEnvelope(row: Record<string, unknown>, label: string) {
  ...
  if (updatedAt !== checkedAt) throw new Error(`${label} updatedAt and checkedAt must match`);
  ...
  assertUnique(
    sourceStates.map((state) => state.source),
    `${label} source names`
  );
  const expectedStale = sourceErrors.length > 0 || sourceStates.some((state) => state.status !== "fresh");
  if (stale !== expectedStale) throw new Error(`${label} stale flag is inconsistent with source errors/states`);
  return { updatedAt, checkedAt, stale, sourceErrors, sourceStates };
}

function parseSourceState(value: unknown, envelopeCheckedAt: number, index: number): InstrumentRegistrySourceState {
  const label = `sourceStates[${index}]`;
  const row = record(value, label);
  const source = nonBlankText(row.source, `${label}.source`);
  const status = exact(row.status, ["fresh", "stale-cache", "quarantined"] as const, `${label}.status`);
  const checkedAt = positiveSafeTimestamp(row.checkedAt, `${label}.checkedAt`);
  if (checkedAt !== envelopeCheckedAt) throw new Error(`${label}.checkedAt must match envelope checkedAt`);
  ...
  if (receivedAt !== undefined && receivedAt > checkedAt) throw new Error(`${label}.receivedAt cannot exceed checkedAt`);
...
function parseVenueCapability(value: unknown): VenueCapabilityManifest {
  ...
  const scopes = row.scopes === undefined
    ...
    : array(row.scopes, "venue capability scopes", 100).map((value, index) => {
        ...
        return {
          product: exact(scope.product, ["spot", "margin", "perpetual", "future", "option", "native-spread", "account"] as const, `venue capability scopes[${index}].product`),
          operation: exact(scope.operation, ["public-data", "private-execution", "borrow", "deposit-withdrawal"] as const, `venue capability scopes[${index}].operation`),
          status: exact(scope.status, ["implemented", "experimental", "manual-only"] as const, `venue capability scopes[${index}].status`)
        };
      });
  if (scopes) {
    assertUnique(scopes.map((scope) => `${scope.product}:${scope.operation}`), "venue capability product/operation scopes");
  }
  const result: VenueCapabilityManifest = {
    venue: nonBlankText(row.venue, "venue"),
...
function parsePriceRules(value: unknown) {
  if (value === undefined) return undefined;
  const row = record(value, "priceRules");
  if (row.staticTickSize !== false) throw new Error("priceRules.staticTickSize must be false");
  return {
