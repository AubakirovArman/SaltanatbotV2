import type { TriangularDepthEvidence, TriangularDepthRejection, TriangularDepthVerificationResponse, VerifiedTriangularLeg, VerifiedTriangularOpportunity } from "./triangularDepthTypes.js";
import { array, bool, exact, finite, integer, nonNegative, positive, record, text } from "./validation.js";

const REJECTION_CODES = ["unknown-market", "invalid-book", "incomplete-book", "missing-book", "stale-book", "skewed-books", "minimum-quantity", "minimum-notional", "insufficient-depth", "non-profitable"] as const;

/** Strict parser for the selected three-book, credential-free verification response. */
export function parseTriangularDepthVerification(value: unknown): TriangularDepthVerificationResponse {
  const row = record(value, "triangular depth verification");
  rejectExecutionShape(row);
  const symbols = tuple3(row.symbols, "symbols").map((value, index) => safeSymbol(value, `symbols[${index}]`)) as [string, string, string];
  if (new Set(symbols).size !== 3) throw new Error("symbols must contain three distinct markets");
  const venue = exact(row.venue, ["binance", "bybit"] as const, "venue");
  ...
  const books = tuple3(row.books, "books").map((value, index) => parseBook(value, symbols[index] as string)) as [TriangularDepthEvidence, TriangularDepthEvidence, TriangularDepthEvidence];
  ...
  if (totalOpportunities !== opportunities.length) throw new Error("totalOpportunities is inconsistent with opportunities");
  if (opportunities.length > 0 && rejections.length > 0) throw new Error("successful depth verification cannot retain stale rejections");
  return {
    ...
    execution: exact(row.execution, ["none"] as const, "execution"),
    verificationStatus: exact(row.verificationStatus, ["sequence-verified-paper-candidate"] as const, "verificationStatus"),
    marketDataMode: exact(row.marketDataMode, ["sequence-verified-depth"] as const, "marketDataMode"),
    venue,
    startAsset,
...
function parseBook(value: unknown, expectedSymbol: string): TriangularDepthEvidence {
  const row = record(value, `book ${expectedSymbol}`);
  const symbol = safeSymbol(row.symbol, "book.symbol");
  if (symbol !== expectedSymbol) throw new Error("book order must match symbols");
  return {
    symbol,
    ...
    source: exact(row.source, ["websocket-reconstructed"] as const, "book.source"),
    sequenceVerified: exactBoolean(row.sequenceVerified, true, "book.sequenceVerified")
  };
}

function parseOpportunity(value: unknown, envelope: { venue: "binance" | "bybit"; symbols: readonly string[]; startAsset: string; requestedStartQuantity: number; evaluatedAt: number }): VerifiedTriangularOpportunity {
  const row = record(value, "verified triangular opportunity");
  const venue = exact(row.venue, ["binance", "bybit"] as const, "opportunity.venue");
  if (venue !== envelope.venue) throw new Error("opportunity venue differs from verification envelope");
  const startAsset = asset(row.startAsset, "opportunity.startAsset");
  const endAsset = asset(row.endAsset, "opportunity.endAsset");
  if (startAsset !== envelope.startAsset || endAsset !== startAsset) throw new Error("opportunity must preserve the requested cycle asset");
  const requestedStartQuantity = positive(row.requestedStartQuantity, "opportunity.requestedStartQuantity");
  if (!close(requestedStartQuantity, envelope.requestedStartQuantity)) throw new Error("opportunity requested quantity differs from verification envelope");
  const legs = tuple3(row.legs, "opportunity.legs").map((leg, index) => parseLeg(leg, index, venue, envelope.symbols)) as [VerifiedTriangularLeg, VerifiedTriangularLeg, VerifiedTriangularLeg];
  ...
  for (const required of ["sequential-leg-risk", "output-fee-assumption"]) if (!riskFlags.includes(required)) throw new Error(`verified opportunity is missing ${required}`);
  for (const forbidden of ["top-book-only", "rest-snapshot", "unsequenced", "non-executable-candidate"]) if (riskFlags.includes(forbidden)) throw new Error(`verified opportunity contains forbidden ${forbidden}`);
  const dustRow = record(row.dustByAsset, "dustByAsset");
  const dustByAsset: Record<string, number> = {};
  for (const [key, quantity] of Object.entries(dustRow)) dustByAsset[asset(key, "dust asset")] = nonNegative(quantity, `dustByAsset.${key}`);
  ...
  if ((limitingLegIndex === undefined) !== (limitingMarketId === undefined)) throw new Error("limiting leg and market provenance must be supplied together");
  return {
    id: text(row.id, "opportunity.id"),
    strategyKind: exact(row.strategyKind, ["triangular"] as const, "strategyKind"),
    edgeKind: exact(row.edgeKind, ["executable-sequential"] as const, "edgeKind"),
    executionStatus: exact(row.executionStatus, ["executable"] as const, "executionStatus"),
    marketDataMode: exact(row.marketDataMode, ["sequence-verified-depth"] as const, "marketDataMode"),
    ...
    grossReturnBps,
    netReturnBps,
    limitingCapacity: { requestedStartQuantity, executableStartQuantity, utilizationPct, ...(limitingLegIndex === undefined ? {} : { limitingLegIndex, limitingMarketId: limitingMarketId as string }) },
    legs,
    dustByAsset,
    timestamps,
    riskFlags
...
function parseLeg(value: unknown, expectedIndex: number, venue: string, symbols: readonly string[]): VerifiedTriangularLeg {
  const row = record(value, `leg[${expectedIndex}]`);
  const index = optionalLegIndex(row.index, `leg[${expectedIndex}].index`);
  if (index !== expectedIndex) throw new Error("verified legs must preserve ordered indices 0,1,2");
  const symbol = safeSymbol(row.symbol, `leg[${expectedIndex}].symbol`);
  if (!symbols.includes(symbol)) throw new Error("verified leg symbol is outside the selected route");
  const marketId = text(row.marketId, `leg[${expectedIndex}].marketId`);
  if (marketId !== `${venue}:spot:${symbol}`) throw new Error("verified leg market identity is inconsistent");
  ...
  if (!close(inputQuantity, inputConsumedQuantity + inputDustQuantity)) throw new Error("leg input conservation is inconsistent");
  ...
  if (!close(outputQuantity, grossOutputQuantity - feeQuantity) || !close(feeQuantity, (grossOutputQuantity * feeBps) / 10_000)) throw new Error("leg fee arithmetic is inconsistent");
  return {
    index: index as 0 | 1 | 2,
    marketId,
    symbol,
    side: exact(row.side, ["buy", "sell"] as const, "leg.side"),
...
function parseTimestamps(value: unknown, evaluatedAt: number) {
  const row = record(value, "opportunity.timestamps");
  const wireEvaluatedAt = positive(row.evaluatedAt, "timestamps.evaluatedAt");
  if (wireEvaluatedAt !== evaluatedAt) throw new Error("opportunity timestamp differs from verification envelope");
  ...
  if (oldestExchangeTs > newestExchangeTs || oldestReceivedAt > newestReceivedAt) throw new Error("triangular timestamp order is invalid");
  ...
  if (!close(quoteAgeMs, derivedAge) || !close(legSkewMs, derivedSkew)) throw new Error("triangular timestamp arithmetic is inconsistent");
...
function parseRejection(value: unknown): TriangularDepthRejection {
...
function tuple3(value: unknown, label: string): [unknown, unknown, unknown] {
  const rows = array(value, label, 3);
  if (rows.length !== 3) throw new Error(`${label} must contain exactly three rows`);
  return rows as [unknown, unknown, unknown];
}

function uniqueTextArray(value: unknown, label: string, maximum: number) {
...
function safeSymbol(value: unknown, label: string) {
  const result = text(value, label);
  if (!/^[A-Z0-9-]{2,32}$/.test(result)) throw new Error(`${label} is invalid`);
  return result;
}

function asset(value: unknown, label: string) {
  const result = text(value, label);
  if (!/^[A-Z0-9_-]{2,20}$/.test(result)) throw new Error(`${label} is invalid`);
  return result;
}

function optionalLegIndex(value: unknown, label: string): 0 | 1 | 2 | undefined {
  if (value === undefined) return undefined;
  const result = integer(value, label);
  if (result > 2) throw new Error(`${label} is unsupported`);
...
function exactBoolean<const T extends boolean>(value: unknown, expected: T, label: string): T {
  const result = bool(value, label);
  if (result !== expected) throw new Error(`${label} must be ${expected}`);
  return expected;
}

function exactNumber<const T extends number>(value: unknown, expected: T, label: string): T {
  const result = finite(value, label);
  if (result !== expected) throw new Error(`${label} must be ${expected}`);
  return expected;
}

function rejectExecutionShape(row: Record<string, unknown>) {
  for (const key of ["order", "orders", "apiKey", "apiSecret", "credential", "credentials"]) if (key in row) throw new Error(`triangular depth response contains forbidden ${key}`);
}
