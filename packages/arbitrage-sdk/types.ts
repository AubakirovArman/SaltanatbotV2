...
export interface BasisScan {
  ...
  sources: Array<{ exchange: BasisVenue; market: "spot" | "perpetual"; ok: boolean; message?: string }>;
...
export type PairwiseDeliveryAssumption = { mode: "close-before-expiry"; exitAt: number; deliveryFeeBps: number; source: string; asOf: number } | { mode: "settle-near-roll-far"; exitAt: number; nearInstrumentId: string; deliveryFeeBps: number; settlementPriceSource: string; source: string; asOf: number };
...
export type PairwiseEvaluationResponse = PairwiseEvaluationEnvelope & ({ opportunity: PairwiseOpportunity; rejection?: never } | { opportunity?: never; rejection: PairwiseRejection });
