 ...
 */
export function canonicalDatasetSerialization(source: string, timeframe: string, barsBySymbol: DatasetBarsBySymbol): string {
  const entries = normalizeDatasetEntries(source, timeframe, barsBySymbol);
  const parts: string[] = [`${DATASET_SCHEMA_VERSION}\n${source}\n${timeframe}\n`];
  for (const entry of entries) {
    parts.push(`${entry.symbol}\n`);
    for (const bar of entry.bars) {
      parts.push(
        `${canonicalNumber(bar.time)},${canonicalNumber(bar.open)},${canonicalNumber(bar.high)},${canonicalNumber(bar.low)},${canonicalNumber(bar.close)},${canonicalNumber(bar.volume)}\n`
      );
    }
  }
  return parts.join("");
...
export function computeDatasetFingerprint(source: string, timeframe: string, barsBySymbol: DatasetBarsBySymbol): string {
...
export function buildDatasetDescriptor(input: DatasetDescriptorInputV1): DatasetDescriptorV1 {
 ...
 */
export function splitDatasetBars<TBar extends { time: number }>(
  bars: readonly TBar[],
  split: DatasetSplitConfigV1
): { train: TBar[]; test: TBar[] } {
...
export function normalizeDatasetSplit(split: DatasetSplitConfigV1): DatasetSplitV1 {
  if (
    !Number.isFinite(split.trainFraction) ||
    ...
    throw new DatasetContractError(
      `Dataset trainFraction must be between ${DATASET_TRAIN_FRACTION_MINIMUM} and ${DATASET_TRAIN_FRACTION_MAXIMUM}.`
    );
  }
...
function normalizeDatasetEntries(source: string, timeframe: string, barsBySymbol: DatasetBarsBySymbol): DatasetEntry[] {
  requireLabel("source", source);
  requireLabel("timeframe", timeframe);
  const pairs: [string, readonly Candle[]][] =
    barsBySymbol instanceof Map ? [...barsBySymbol.entries()] : Object.entries(barsBySymbol);
  if (pairs.length === 0) throw new DatasetContractError("Dataset requires at least one symbol.");
  const entries = pairs
    .map(([symbol, bars]) => ({ symbol, bars }))
    .sort((a, b) => (a.symbol < b.symbol ? -1 : a.symbol > b.symbol ? 1 : 0));
  for (const entry of entries) {
    requireLabel("symbol", entry.symbol);
    if (entry.bars.length === 0) throw new DatasetContractError(`Dataset symbol ${entry.symbol} has no bars.`);
