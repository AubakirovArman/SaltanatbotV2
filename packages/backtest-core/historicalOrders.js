 ...
 */
export function stepHistoricalOrder(input, candle, costs) {
    ...
    if (rawPrice === undefined)
        return { order, status: order.filledQty > 0 ? "partially_filled" : "resting" };
...
export function runHistoricalOrder(order, candles, costs) {
    const steps = [];
    let current = order;
...
function eligiblePrice(order, candle) {
    ...
    if (order.type === "limit") {
        if (order.side === "buy" && candle.low <= order.price)
            return Math.min(candle.open, order.price);
        if (order.side === "sell" && candle.high >= order.price)
            return Math.max(candle.open, order.price);
        return undefined;
    }
