 ...
 */
export function simulatePortfolioBacktest(legs, config = {}) {
    ...
    for (const time of times) {
        for (const reader of prices.values())
            reader.advance(time);
        ...
        for (let index = active.length - 1; index >= 0; index -= 1) {
            ...
            if (open.candidate.trade.exitTime !== time)
                continue;
        ...
        while (candidates[candidateIndex]?.trade.entryTime === time) {
            ...
            let reason;
            if (!(candidate.originalNotional > 0) || !(requestedNotional > 0))
                reason = "invalid_candidate";
            else if (active.length >= cfg.maxConcurrentPositions)
                reason = "max_concurrent";
            else if (!(capacity > 0))
                reason = "gross_exposure";
            const allocatedNotional = Math.min(requestedNotional, capacity);
            if (!reason && allocatedNotional < requestedNotional * cfg.minAllocationPct / 100)
...
export function sanitizePortfolioBacktestConfig(input) {
    return {
...
function fundingForTrade(events, trade) {
...
function candidateReportCommission(legs, symbol) {
    return legs.find((leg) => leg.symbol === symbol)?.report.metadata.config.commissionPct ?? 0;
}
function createPriceReaders(legs) {
    return new Map(legs.map((leg) => {
        let index = -1;
        let close = leg.candles[0]?.close ?? 0;
        return [leg.symbol, {
                advance(time) {
                    while (index + 1 < leg.candles.length && leg.candles[index + 1].time <= time)
                        close = leg.candles[++index].close;
                },
                price: () => close
            }];
    }));
}
function portfolioEquity(cash, active, prices) {
...
function portfolioMetrics(config, curve, trades, rejected, totalCandidates, excludedCandidates, maxConcurrentPositions, fundingPaid) {
...
function symbolContributions(symbols, candidates, trades, rejected, netProfit) {
...
function correlationMatrix(legs, fromTime, toTime) {
...
function candleReturns(candles, fromTime, toTime) {
    const result = new Map();
    for (let index = 1; index < candles.length; index += 1) {
        const candle = candles[index];
        const previous = candles[index - 1];
        if (candle.time < fromTime || candle.time > toTime || !(previous.close > 0))
            continue;
...
function pearson(left, right) {
    const pairs = [...left.entries()].flatMap(([time, value]) => right.has(time) ? [[value, right.get(time)]] : []);
    if (pairs.length < 2)
        return null;
...
function standardDeviation(values, average) { return values.length < 2 ? 0 : Math.sqrt(mean(values.map((value) => (value - average) ** 2))); }
