...
function numberWarmup(expr, params) {
    switch (expr.k) {
        ...
        case "macd": {
            ...
            return merge(numberWarmup(expr.source, params), numberWarmup(expr.fast, params), { bars: slow.bars + signal.bars, dynamic: slow.dynamic || signal.dynamic });
        }
        case "stoch": {
        ...
        case "sar":
            return dynamic(numberWarmup(expr.start, params), numberWarmup(expr.inc, params), numberWarmup(expr.max, params));
        case "kc":
...
function conditionWarmup(expr, params) {
    switch (expr.k) {
        case "bool":
        case "session":
        case "dayofweek":
        ...
        case "trend":
            return merge(period(expr.period, params), numberWarmup(expr.source, params));
        case "between":
            return merge(numberWarmup(expr.value, params), numberWarmup(expr.low, params), numberWarmup(expr.high, params));
        case "isna":
            return numberWarmup(expr.a, params);
    }
}
function statementsWarmup(statements, params) {
    const estimates = [];
    for (const statement of statements) {
        switch (statement.k) {
            ...
            case "setvarb":
                ...
                break;
            case "box":
                estimates.push(numberWarmup(statement.top, params), numberWarmup(statement.bottom, params), conditionWarmup(statement.when, params));
                break;
            case "projection":
                estimates.push(numberWarmup(statement.left, params), numberWarmup(statement.right, params), numberWarmup(statement.top, params), numberWarmup(statement.bottom, params), conditionWarmup(statement.when, params));
                break;
            case "metric":
            ...
            case "if":
                ...
                if (statement.else)
                    estimates.push(statementsWarmup(statement.else, params));
                break;
            case "repeat":
            ...
            case "while":
                ...
                break;
            case "for":
                estimates.push(numberWarmup(statement.from, params), numberWarmup(statement.to, params), numberWarmup(statement.step, params), statementsWarmup(statement.body, params));
                break;
        }
    }
...
export function estimateWarmupBars(ir) {
