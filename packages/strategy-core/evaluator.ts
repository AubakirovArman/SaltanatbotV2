...
export function beginStrategyBar(rt: StrategyRuntime, ctx: Record<string, number> = {}): void {
  rt.ops = 0;
  rt.budgetHit = false;
  rt.ctx = ctx;
...
export function evaluateStrategyBar(
  ir: StrategyIR,
  index: number,
  rt: StrategyRuntime,
  ctx: Record<string, number> = rt.ctx
): BarIntents {
...
function renderAlert(message: string, args: Record<string, NumExpr> | undefined, i: number, rt: StrategyRuntime): string {
...
export function evaluateNumber(expr: NumExpr, i: number, rt: StrategyRuntime): number {
  switch (expr.k) {
    case "var":
...
export function evaluateCondition(expr: BoolExpr, i: number, rt: StrategyRuntime): boolean {
  switch (expr.k) {
    case "bool":
      return expr.v;
    case "compare": {
...
function getSeries(expr: NumExpr, rt: StrategyRuntime): number[] {
...
function computeSeries(expr: NumExpr, rt: StrategyRuntime): number[] {
  const n = rt.n;
  switch (expr.k) {
    case "num":
    ...
    case "cond": {
      ...
      for (let idx = 0; idx < n; idx += 1) out[idx] = evaluateCondition(expr.cond, idx, rt) ? ca[idx] : cb[idx];
      return out;
    }
    case "nz": {
    ...
    case "barssince": {
      ...
      let last = -1;
      for (let idx = 0; idx < n; idx += 1) {
        if (evaluateCondition(expr.cond, idx, rt)) last = idx;
    ...
    case "valuewhen": {
      ...
      for (let idx = 0; idx < n; idx += 1) cond[idx] = evaluateCondition(expr.cond, idx, rt);
...
function constNum(expr: NumExpr, rt: StrategyRuntime): number {
  switch (expr.k) {
