...
export function priceAt(candle, field) {
    switch (field) {
...
export function sourceSeries(candles, field) {
...
export function wma(src, period) {
    ...
    for (let i = period - 1; i < src.length; i += 1) {
        ...
        for (let j = 0; j < period; j += 1)
            acc += src[i - j] * (period - j);
        out[i] = acc / denom;
    }
    return out;
}
export function vwma(src, volume, period) {
    const out = NaNArray(src.length);
    for (let i = period - 1; i < src.length; i += 1) {
        let pv = 0;
        let vol = 0;
...
export function stdev(src, period) {
    ...
    for (let i = period - 1; i < src.length; i += 1) {
        let acc = 0;
        for (let j = 0; j < period; j += 1)
            acc += (src[i - j] - means[i]) ** 2;
        out[i] = Math.sqrt(acc / period);
    }
    return out;
}
export function correlationSeries(a, b, period) {
...
export function lowest(src, period) {
    ...
    for (let i = period - 1; i < src.length; i += 1) {
        let min = Infinity;
        for (let j = 0; j < period; j += 1)
            min = Math.min(min, src[i - j]);
        out[i] = min;
    }
    return out;
}
export function change(src, period) {
    const out = NaNArray(src.length);
    for (let i = period; i < src.length; i += 1)
        out[i] = src[i] - src[i - period];
    return out;
}
export function bollingerBand(src, period, dev, band) {
...
export function cci(candles, period) {
    ...
    return tp.map((value, i) => {
        if (Number.isNaN(mean[i]))
            return NaN;
        let dev = 0;
        for (let j = 0; j < period; j += 1)
            dev += Math.abs(tp[i - j] - mean[i]);
        dev /= period;
...
export function roc(src, period) {
    ...
    for (let i = period; i < src.length; i += 1) {
        if (src[i - period] !== 0)
            out[i] = (src[i] / src[i - period] - 1) * 100;
    }
    return out;
}
export function macdLine(src, fast, slow, signal, line) {
    const fastEma = ema(src, fast);
    const slowEma = ema(src, slow);
    const macd = src.map((_, i) => Number.isNaN(fastEma[i]) || Number.isNaN(slowEma[i]) ? NaN : fastEma[i] - slowEma[i]);
    if (line === "macd")
        return macd;
...
/* ================= wave 3 indicator primitives =================
   ...
   All output NaN until the indicator is warm (Pine `na`). This package source
   is shared by frontend preview/backtest and backend live evaluation. */
...
export function valueWhen(cond, src, occurrence) {
...
export function extremeBars(kind, src, period) {
    const out = NaNArray(src.length);
    for (let i = period - 1; i < src.length; i += 1) {
...
export function linregSeries(src, period, offset) {
    const out = NaNArray(src.length);
...
export function supertrendSeries(candles, factor, atrPeriod) {
    const n = candles.length;
 ...
 *  DX over adxLen. */
export function dmiSeries(candles, diLen, adxLen) {
    const n = candles.length;
    ...
    const dx = NaNArray(n);
    for (let i = 0; i < n; i += 1) {
        if (Number.isNaN(trur[i]) || Number.isNaN(plusSm[i]) || Number.isNaN(minusSm[i]))
            continue;
...
export function mfiSeries(candles, period) {
    const n = candles.length;
    const out = NaNArray(n);
    const tp = sourceSeries(candles, "hlc3");
...
export function cmoSeries(src, period) {
    const out = NaNArray(src.length);
    for (let i = period; i < src.length; i += 1) {
        let up = 0;
        let down = 0;
...
export function tsiSeries(src, shortLen, longLen) {
    const n = src.length;
    const pc = NaNArray(n);
    ...
    return num.map((v, i) => Number.isNaN(v) || Number.isNaN(den[i]) || den[i] === 0 ? NaN : (100 * v) / den[i]);
}
/** ta.alma: Arnaud Legoux MA — gaussian window weights with
 *  m = offset·(period−1), s = period/sigma (j = 0 oldest … period−1 newest). */
export function almaSeries(src, period, offset, sigma) {
    const out = NaNArray(src.length);
    if (sigma <= 0)
        return out;
...
export function cogSeries(src, period) {
    const out = NaNArray(src.length);
    for (let i = period - 1; i < src.length; i += 1) {
        let num = 0;
        let den = 0;
...
export function percentRankSeries(src, period) {
    const out = NaNArray(src.length);
    for (let i = period; i < src.length; i += 1) {
        const cur = src[i];
        if (Number.isNaN(cur))
...
export function sarSeries(candles, start, inc, max) {
    const n = candles.length;
    const out = NaNArray(n);
    if (n < 2)
        return out;
    ...
    for (let i = 1; i < n; i += 1) {
        ...
        if (isBelow) {
            ...
            if (i > 1)
                result = Math.min(result, candles[i - 2].low);
        }
        else {
            result = Math.max(result, candles[i - 1].high);
            if (i > 1)
                result = Math.max(result, candles[i - 2].high);
        }
        out[i] = result;
    }
    return out;
...
export function kcSeries(candles, period, mult) {
