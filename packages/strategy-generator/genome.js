...
export function randomStrategyGenome(random, families = ALL_FAMILIES, directions = ALL_DIRECTIONS) {
    return {
...
export function randomSignal(random, families = ALL_FAMILIES) {
    ...
    if (family === "mean-reversion") {
        if (random() < 0.5)
            return { family, variant: "rsi-reentry", period: randomInt(random, 5, 50), trigger: randomInt(random, 20, 40), exitLevel: randomInt(random, 50, 75) };
        return { family, variant: "bollinger-fade", period: randomInt(random, 10, 100), deviation: randomDecimal(random, 1, 4, 0.25) };
    }
    if (family === "breakout") {
        if (random() < 0.5)
            return { family, variant: "donchian", period: randomInt(random, 10, 250), exitPeriod: randomInt(random, 3, 100) };
        return { family, variant: "bollinger-break", period: randomInt(random, 10, 100), deviation: randomDecimal(random, 1, 4, 0.25) };
    }
    if (random() < 0.5)
        return { family, variant: "roc", period: randomInt(random, 2, 100), threshold: randomDecimal(random, 0.25, 10, 0.25) };
...
export function crossoverStrategyGenomes(left, right, random) {
    const child = {
...
export function mutateStrategyGenome(genome, random, options) {
    const next = { direction: genome.direction, signal: { ...genome.signal }, risk: { ...genome.risk } };
    const log = [];
    ...
    if (random() < rate)
        mutateDirection(next, directions, random, log);
    if (random() < rate)
        mutateSignal(next, families, random, log);
    if (random() < rate)
        mutateRisk(next, random, log);
...
function mutateDirection(next, directions, random, log) {
...
function mutateSignal(next, families, random, log) {
    if (random() < 0.4) {
        const from = signalSummary(next.signal);
        let replacement = randomSignal(random, families);
        for (let attempt = 0; attempt < 8 && signalSummary(replacement) === from; attempt += 1)
            replacement = randomSignal(random, families);
...
function mutateSignalParameter(signal, random, log) {
    const next = { ...signal };
    if (next.variant === "ma-cross") {
        const field = pick(["maKind", "fastPeriod", "slowPeriod"], random);
        if (field === "maKind")
            replaceMaKind(next, random, log);
        else if (field === "fastPeriod")
            replaceNumber(next, field, randomDifferentInt(random, next.fastPeriod, 3, Math.min(60, next.slowPeriod - 3)), log);
        else
            replaceNumber(next, field, randomDifferentInt(random, next.slowPeriod, next.fastPeriod + 3, 250), log);
    }
    else if (next.variant === "price-ma") {
        if (random() < 0.35)
            replaceMaKind(next, random, log);
        else
            replaceNumber(next, "period", randomDifferentInt(random, next.period, 5, 250), log);
    }
    else if (next.variant === "rsi-reentry") {
        const field = pick(["period", "trigger", "exitLevel"], random);
        ...
        if (random() < 0.5)
            replaceNumber(next, "period", randomDifferentInt(random, next.period, 10, 100), log);
        else
            replaceNumber(next, "deviation", randomDifferentDecimal(random, next.deviation, 1, 4, 0.25), log);
    }
        ...
        if (random() < 0.5)
            replaceNumber(next, "period", randomDifferentInt(random, next.period, 2, 100), log);
        else
            replaceNumber(next, "threshold", randomDifferentDecimal(random, next.threshold, 0.25, 10, 0.25), log);
    }
    else {
        const field = pick(["fastPeriod", "slowPeriod", "signalPeriod"], random);
        if (field === "fastPeriod")
            replaceNumber(next, field, randomDifferentInt(random, next.fastPeriod, 3, Math.min(24, next.slowPeriod - 3)), log);
        else if (field === "slowPeriod")
            replaceNumber(next, field, randomDifferentInt(random, next.slowPeriod, Math.max(20, next.fastPeriod + 3), 80), log);
        else
            replaceNumber(next, field, randomDifferentInt(random, next.signalPeriod, 2, 30), log);
    }
    return next;
}
function mutateRisk(next, random, log) {
    const field = pick(["stopMode", "stopValue", "targetMode", "targetValue", "positionPct"], random);
    if (field === "stopMode") {
        ...
        if (value !== next.risk.stopValue)
            record(log, "bounded-step", "risk.stopValue", value, next.risk.stopValue);
    }
        ...
        if (value !== next.risk.targetValue)
            record(log, "bounded-step", "risk.targetValue", value, next.risk.targetValue);
    }
...
function replaceMaKind(signal, random, log) {
    const from = signal.maKind;
    signal.maKind = pick(ALL_MA_KINDS.filter((kind) => kind !== from), random);
    record(log, "replace", "signal.maKind", from, signal.maKind);
}
function replaceNumber(target, field, value, log, path = `signal.${String(field)}`) {
    const from = target[field];
    target[field] = value;
    record(log, "bounded-step", path, from, value);
}
function randomDifferentInt(random, current, min, max, step = 1) {
    if (max <= min)
        return min;
    let value = current;
    for (let attempt = 0; attempt < 8 && value === current; attempt += 1)
        value = randomInt(random, min, max, step);
    if (value === current)
        value = current + step <= max ? current + step : Math.max(min, current - step);
    return value;
}
function randomDifferentDecimal(random, current, min, max, step) {
    if (max <= min)
        return min;
    let value = current;
    for (let attempt = 0; attempt < 8 && value === current; attempt += 1)
        value = randomDecimal(random, min, max, step);
    if (value === current)
        value = canonicalNumber(current + step <= max ? current + step : Math.max(min, current - step));
    return value;
}
function randomRiskValue(random, mode, kind) {
    const [min, max, step] = riskRange(mode, kind);
    return randomDecimal(random, min, max, step);
}
function randomDifferentRiskValue(random, current, mode, kind) {
    const [min, max, step] = riskRange(mode, kind);
    return randomDifferentDecimal(random, current, min, max, step);
}
function clampRiskValue(value, mode, kind) {
    const [min, max] = riskRange(mode, kind);
    return canonicalNumber(clamp(value, min, max));
}
function riskRange(mode, kind) {
    if (kind === "stop")
        return mode === "percent" ? [0.5, 10, 0.25] : [0.5, 6, 0.25];
    return mode === "percent" ? [1, 30, 0.5] : [1, 12, 0.25];
}
function signalSummary(signal) {
    return JSON.stringify(signal);
}
function record(log, operator, field, from, to) {
    if (from !== to && log.length < GENERATOR_LIMITS.maxMutationLogEntries)
        log.push({ operator, field, from, to });
}
