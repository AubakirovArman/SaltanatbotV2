...
export const DEFAULT_MULTI_MARKET_RANKING_POLICY = {
    minMarkets: 2,
...
export function rankMultiMarketEvaluations(evaluations, overrides = {}) {
...
export function resolveRankingPolicy(overrides = {}) {
    ...
    for (const key of Object.keys(policy)) {
        if (key !== "minMarkets" && key !== "minTradesPerWindow")
            policy[key] = clamp(finiteOr(policy[key], DEFAULT_MULTI_MARKET_RANKING_POLICY[key]), 0, 1_000);
    }
    if (policy.trainWeight + policy.outOfSampleWeight === 0)
        policy.outOfSampleWeight = 1;
    if (policy.medianWeight + policy.worstMarketWeight === 0)
        policy.medianWeight = 1;
    return policy;
}
function rankCandidate(evaluation, policy) {
...
function scoreMarket(market, policy) {
...
function scoreWindow(metrics, policy) {
    if (!metricsFinite(metrics))
        return FAIL_SCORE;
...
function validateEvaluationSet(evaluation, uniqueMarkets, policy) {
    const flags = {
    ...
    const issues = Object.entries(flags)
        ...
        .map(([flag]) => flag);
    if (!evaluation.candidateFingerprint.trim())
        issues.push("candidateFingerprint");
    return { valid: issues.length === 0, flags, issues };
}
function uniqueByMarketId(markets) {
    const unique = new Map();
    for (const market of markets) {
        const normalizedMarketId = market.marketId.trim();
        if (!unique.has(normalizedMarketId))
            unique.set(normalizedMarketId, market);
    }
    return [...unique.values()];
}
function metricsFinite(metrics) {
...
function finiteScore(value) {
    if (!Number.isFinite(value))
        return FAIL_SCORE;
    if (value < FAIL_SCORE || value > -FAIL_SCORE)
        return FAIL_SCORE;
    return value;
}
