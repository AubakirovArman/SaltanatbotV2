...
export function validateGeneratedStrategy(ir) {
    ...
    for (const input of ir.inputs) {
        ...
        if (!input.name || input.name.length > 64 || inputNames.has(input.name) || !values.every(Number.isFinite))
            finiteInputs = false;
        inputNames.add(input.name);
        if ((input.min !== undefined && input.value < input.min) || (input.max !== undefined && input.value > input.max) || (input.min !== undefined && input.max !== undefined && input.min > input.max) || (input.step !== undefined && input.step <= 0))
            boundedInputs = false;
    }
...
function scanNodes(ir, inputNames) {
    const stack = [...ir.body];
    let nodes = 0;
    ...
    while (stack.length) {
        ...
        if (!value || typeof value !== "object")
            continue;
        nodes += 1;
        if (nodes > GENERATOR_LIMITS.maxIrNodes)
            break;
        const object = value;
        if (typeof object.k === "string" && !ALLOWED_NODE_KINDS.has(object.k))
            supportedGrammar = false;
        if (object.k === "input" && (typeof object.name !== "string" || !inputNames.has(object.name)))
